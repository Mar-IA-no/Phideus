...
de completar soporte. La enmienda explícita del presupuesto de RAM de evaluación
a 4 GiB pasó su auditoría y la ejecución se reanudó, conservando ambos fallos
y sin cambiar el cálculo.
...
La ejecución contractual de Ola 28 pasa así de veinte a veintidós suite-bank IDs distribuidos en cinco suites. Una auditoría semántica posterior mostró que el antiguo índice `22/33` reunía granularidades distintas: veintidós IDs internos de suites frente a treinta y tres bancos raw diseñados por R57/R58. Por eso once no constituye un complemento literal ni una lista de contratos pendientes. La contabilidad canónica conserva dos planos: `22` contratos ejecutados y cobertura raw `9 fully adjudicated / 20 partially covered / 4 not adjudicated`. El caso exacto de edge-weight lasso queda adjudicado para sus seis etiquetas congeladas, mientras el banco de triplet covers sólo cubre el contraste mínimo `n=5` y no la familia aleatoria mayor. El cierre amplía un firewall de identificabilidad para futuras primitives, no demuestra robustez estadística, no ejecuta modelos y no decide promoción ni GO/NO-GO.
...
La primera auditoría independiente exigió revisión mayor por nueve defectos de contrato. La corrección igualó evidencia entre brazos, separó solver oracle de solver empírico, adaptación por proceso de zero-shot, validez finita de realizabilidad global y scoring individual de casos no identificables. Una primera reauditoría detectó un residuo adicional: sin fijar inicialización, estacionariedad y pesos de mezcla, la propia ley de futuros seguía abierta. El `process_law_contract` incorporó esos campos y una auditoría final cerró entonces el snapshot de once inputs. Una revisión documental posterior descubrió que tres de esos archivos habían recibido cambios administrativos sin preservar los blobs auditados. El defecto de trazabilidad se reparó sin reescribir el pasado: se congelaron doce inputs vigentes, se preservó un tar determinista y una nueva instancia independiente verificó hashes antes y después y emitió `CLOSED` sin findings sustantivos.
...
### Octava ola — identificabilidad causal y certificación algebraica

La octava ola agregó dos informes y elevó el corpus a dieciocho investigaciones. El primero estudió equivalencia observacional e interventional, identificación de sistemas, nonlinear ICA, causal representation learning y cámaras físicas. El segundo reunió ideales, radicales, invariantes, eliminación, SMT, SOS y regresión simbólica verificada. La convergencia no fue una nueva geometría universal: fue una separación entre **qué estructura puede distinguir el experimento** y **qué consecuencia puede certificar un contrato formal**.

El programa registró `A7`, un identificador-compilador certificado, junto con los contratos `P2g/P2h`. P2g ya no ordena predicción, intervención, identificabilidad y proporcionalidad como una escalera: cruza tipo de consulta, identificabilidad estructural/práctica y anclaje físico, con regímenes de acceso separados y sin usar targets o sesiones retenidos para selección. P2h cruza `PPU±solver` y `control±solver` sobre el mismo input, IR, checker y presupuesto de solver; distingue ideal, radical algebraico/real, dominio y artefacto.

Una auditoría independiente encontró siete problemas sustantivos en la primera integración: se fusionaban estados de jurisdicciones distintas, el track causal era lineal y omitía identificabilidad práctica/contrafactual, no congelaba el acceso por régimen, la atribución arquitectura×solver era incompleta, faltaba jerarquía de unidades, la clasificación ideal/radical era ambigua y la simulación recibía una autoridad causal demasiado amplia. Todos fueron corregidos. La reauditoría confirmó las siete correcciones y agregó un finding medio: los controles algebraicos mezclaban invariancias que deben preservar el target con negativos que lo cambian o casos de dominio inválido. El contrato final los separa como `INVARIANCE-PRESERVING`, `LABEL-CHANGING-NEGATIVE` y `DOMAIN-INVALID`.

La salida candidata ya no es `valid/invalid/unknown` como clase única. Conserva por separado estado del artefacto, claim formal, solver, identificabilidad causal, autoridad física y decisión. Un artefacto rechazado no refuta la proposición, y un certificado formal no valida sus premisas físicas. La ola sigue siendo programática: no ejecuta P2g/P2h, no promueve A7 y no declara GO/NO-GO.

### Novena ola — metrología y adquisición de evidencia

La novena ola agregó dos informes y elevó el corpus a veinte investigaciones. El primero examinó mensurando, calibración, trazabilidad, incertidumbre y covarianza; el segundo, diseño óptimo, adquisición adaptativa, intervenciones y costo. La convergencia no fue una nueva fuente de verdad. Fue una distinción entre **la autoridad de una medición ya adquirida** y **la política que decidió adquirirla**.

El programa registró `A8` y los contratos `P2i/P2j`. `P2i` exige que `CALIBRATED-PROPORTIONAL` pueda reconstruirse desde indicaciones, referencia, función de calibración prospectivamente congelada, incertidumbre conjunta y dependencias compartidas. KCDB, SRM y SRD quedan como anclas externas, no como sustitutos de campañas instrumentales. `P2j` congela constructor de candidate sets, inputs permitidos, costo, safety y stopping; la campaña o episodio completo es la réplica independiente, no cada decisión de la trayectoria.

Una auditoría independiente encontró seis problemas sustantivos: pseudorreplicación de decisiones, calibración potencialmente retrospectiva, leakage en la construcción de candidatos, autoridad excesiva de certificados, falta de atribución del módulo A8 y fuentes del control coordinador no consolidadas. Todos fueron corregidos. El diseño final valida primero protocolos externos GUM/OED/system-ID y sólo después compara `LEARNED-A8` contra esos protocolos y un `SHAM/ABLATE-A8`, cruzados con PPU y control bajo inputs y presupuesto iguales. La ola sigue siendo programática: no ejecuta P2i/P2j, no promueve A8 y no declara GO/NO-GO.

## OLA 28 — SELECCIÓN DEL PRÓXIMO DISCRIMINANTE (2026-08-31)

La corrección contable dejó cuatro bancos raw de R58 todavía no adjudicados. Dos investigaciones primarias posteriores separaron las preguntas que podían confundirse: R82 estudió Neighbor Joining bajo ruido completo y R83 reconstrucción/adquisición con distancias parciales. La integración conserva B-08 como candidato principal porque permite enfrentar una garantía clásica suficiente con éxito y fallo observados sin entregar autoridad a una red.

El resultado metodológico es preciso. La cota estricta de Atteson certifica recuperación topológica dentro de su jurisdicción; superarla sólo retira ese certificado. El banco debe separar familias de ruido, convención de arista mínima, disimilitud cruda y reparada, cuartetos, splits, pesos y residual. Antes de cualquier modelo, el próximo corte coherente es checker-only.

Las alternativas arquitectónicas quedaron preservadas sin mezclarse. `N2-Q` podría puntuar resoluciones de cuartetos después del banco clásico. Una cabeza de riesgo para matrices completas requiere un tag nuevo. `N3-T` pertenece a una suite missing distinta: un minimum triplet cover de `2n-3` pares usa la topología verdadera y funciona como referencia privilegiada, no como policy deployable. Una auditoría independiente encontró cuatro defectos de trazabilidad y alcance; todos fueron resueltos. No hay plan de implementación, promoción ni GO/NO-GO.

### Ola 31 — estadística algebraica y fibras condicionales (2026-08-31)

La Ola 31 investigó un punto de contacto especialmente fértil para la geometría proporcional: una misma matriz entera `A` puede organizar tanto un modelo estadístico tórico como las fibras discretas inducidas por una estadística suficiente. Esa coincidencia ofrece una interfaz arquitectónica posible, pero no una equivalencia automática entre todas las propiedades que aparecen alrededor de ella. Pertenecer al modelo, compartir suficiente estadístico, poder recorrer una fibra y mezclar correctamente dentro de ella son afirmaciones distintas y requieren autoridades distintas.

La integración fijó por eso un contrato más estricto. El objeto de modelo y el esquema de muestreo se declaran antes de evaluar invariantes; cada fibra conserva su propia especificación, su certificado de finitud y su target. Un movimiento del kernel sólo es una posibilidad algebraica: todavía debe producir una transición factible. Una propuesta factible tampoco es el kernel Markov ejecutado: hay que conservar su masa en ambas direcciones, la aceptación Metropolis-Hastings, los fallbacks y la conectividad del soporte efectivamente recorrido. El mixing permanece como una propiedad empírica o teórica adicional, nunca como consecuencia gratuita de la conectividad.

De esta descomposición surgieron tres alternativas recuperables. `Toric Constraint Router` seleccionaría qué autoridad o solver corresponde a cada contrato; `Certified Fiber Explorer` combinaría propuestas aprendidas con aceptación y verificación externas; `Algebraic Proposal Machine` ordenaría movimientos o estrategias sin heredar la autoridad del checker. La comparación causal exige que el scorer no reciba información oracular, que los splits bloqueen por `A`, familia y `FiberSpec`, y que los controles incluyan propuestas deterministas y localmente balanceadas con presupuesto multidimensional comparable.

R94 auditó el paquete integrado y detectó problemas de finitud, fallback, trazabilidad, atribución y novedad. Las correcciones fueron reexaminadas por R95 y R96 hasta cerrar todos los findings dentro del alcance documental. La ola quedó cerrada como investigación y diseño: no se ejecutaron bancos ni modelos, no se promovió ninguna arquitectura y la decisión de abrir un experimento sigue perteneciendo al usuario.

### Ola 32 — redes de reacción y geometría proporcional dinámica (2026-08-31)

La Ola 32 estudió Chemical Reaction Network Theory como una fuente de ground truth capaz de enlazar, sin fusionar, estructura combinatoria, conservación, dinámica determinista y procesos estocásticos. El resultado central no fue una nueva distancia. Fue una geometría estratificada: realización de reacción, matrices de complejos e incidencia, hoja estequiométrica, cinética, balances, lattice de estados, clases comunicantes, composición local de saltos o canales, reloj físico y cociente por observación e intervención.

Esa estratificación volvió operativas varias cautelas. `p_jump` puede quedar identificado por el generador agregado aunque `p_channel` no lo esté; una ley estacionaria no prueba detailed balance ni mixing; la ODE sólo aproxima al CTMC dentro de un contrato de volumen, convención, scaling, horizonte y error; y una trayectoria parcial puede autorizar una clase de realizaciones sin seleccionar el mecanismo plantado. El estatuto general de la Global Attractor Conjecture quedó conservadoramente rotulado como prueba general propuesta, no como teorema cerrado para producir labels.

La auditoría independiente encontró cuatro findings altos y uno medio: certificado incompleto de detailed balance, puente ODE–CTMC insuficiente, absorción y canales duplicados mal tipados, leakage potencial del router y una taxonomía demasiado cercana a una suite ejecutada. Los cinco fueron corregidos; una reauditoría focal confirmó las resoluciones sin findings nuevos sustantivos. El atlas checker-only permanece diseñado, no materializado ni ejecutado.

Quedaron preservadas cinco alternativas arquitectónicas: `Stoichiometric Authority Router`, `Compatibility-Class Dynamics Network`, `Equivalence-Aware Reaction Proposal Network`, `Propensity-Simplex Generator` y `Stochastic Reaction Explorer`. Ninguna fue promovida. La posible singularidad está en sostener autoridad, equivalencia, abstención y cruce determinista–estocástico como tipos operativos, no en renombrar como novedosas primitives ya existentes de mass-action, neural ODE o neural CME. Cualquier materialización futura requiere un plan separado y decisión del usuario.

### Ola 33 — búsqueda adversarial de vacíos y cierre sin selección (2026-08-31)

Después de treinta y dos olas, la investigación dejó de preguntar solamente qué otro dominio podía agregarse y buscó algo más exigente: una jurisdicción de verdad que el corpus no hubiera absorbido y, al mismo tiempo, una operación aprendible estrecha capaz de sostener un contraste causal. Dos carriles independientes recorrieron esas preguntas desde extremos distintos. El primero localizó termodinámica estocástica de trayectorias y gauge curvo como brechas de autoridad matemática y física. El segundo localizó reducción de bases reticulares y composición `boxplus` bajo paridad XOR como contratos operacionales con checkers y controles específicos.
