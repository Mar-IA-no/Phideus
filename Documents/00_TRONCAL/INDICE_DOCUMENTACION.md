...
![Updated](https://img.shields.io/badge/Updated-2026--09--03-F59E0B?style=for-the-badge)
...
| **Geometría proporcional y bases de verdad** | `Documents/04_TRANSVERSAL/TEORIA_Y_FUNDAMENTOS/GEOMETRIA_PROPORCIONAL_BASES_DE_VERDAD.md` | Síntesis de cincuenta olas: invariantes, simuladores, cámaras, autoridad, benchmark clásico y primer contraste neuronal set-valued para PPU/NHG |
...
### Estado: 🟡 Frente exploratorio con `Fase 0A`, `0B`, `1 EN`, `1 ZH` y cierre `EN ↔ ZH` ya consolidados. La lectura vigente ya no es “falta la réplica”, sino una disociación precisa: positivo cross-language en `N-adapt`, null/negativo en `N-strict`

| Documento | Ubicación | Contenido |
|-----------|-----------|-----------|
| **README del frente** | `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/README.md` | Estado canónico del frente: cierre `EN↔ZH` ya consolidado, con positivo acotado a `N-adapt` y lectura `null/negativa` en `N-strict` |
| **Roadmap general** | `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/ROADMAP_VOZ_EXPRESIVA_PHIDEUS.md` | Estructura del frente por carriles y fases; `0A/0B/1 EN/ZH` ya cerradas y siguiente decisión abierta = `Fase 1.2` vs `MSP-Podcast` |
| **Plan archivado Fase 1 ZH** | `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/PLAN_FASE_1_ZH.md` | Plan canónico de la réplica `ZH`, preservado ya como fase ejecutada y absorbida en el cierre cross-language |
...
- La pregunta viva correcta ya no es cerrar `EN ↔ ZH`, sino decidir qué hacer con esa disociación: `Fase 1.2` para el cuello de `N-strict`, o salto a un dominio naturalístico donde el régimen adaptativo pueda leerse mejor.
