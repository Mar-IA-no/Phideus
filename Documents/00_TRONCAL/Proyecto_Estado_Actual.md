...
> **Núcleo arquitectónico proporcional**: el programa ejecutó contrato, smoke, desentrelazado, diagnóstico de interfaz, adaptadores congelados y fidelidad del surrogate IRLS sobre grafos de log-razones. El peso aprendido contiene localización útil para WLS, pero interfiere con IRLS. El checkpoint permitió aislar heads de `4.225/4.224` parámetros: el refit WLS mejoró IID entre `-0.0112` y `-0.0151`, pero no grouped; el refit relacional redujo su error local y empeoró IRLS IID entre `+0.0664` y `+0.0763`. La incertidumbre técnica posterior ya fue resuelta: sobre `1.151` estados de validation, `K=64` obtuvo p99 de RMSE `2,15e-6`, máximo `3,75e-6`, gradientes coincidentes con diferencias finitas y replay `7/7` exacto. Esto habilita una pérdida post-surrogate, pero no demuestra que entrenarla mejore IID o grouped. El próximo contraste CPU compara esa pérdida con el target relacional local, manteniendo IRLS canónico como evaluador externo. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya cuenta con contrato, clásicos, smoke neuronal, desentrelazado, interfaz solver-condicionada, adaptadores y surrogate IRLS CPU ejecutados y reproducidos. El próximo corte compara por CPU la pérdida relacional local contra una pérdida de potenciales post-surrogate; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
