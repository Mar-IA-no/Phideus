...
> [!IMPORTANT]
> **Energía geométrica 2026-09-15**: el [contraste de operación y pérdida](../../experiments/atencion_armonica/RESULTS_GEOMETRIC_DECISION_ENERGY.md) completó 72 entrenamientos, 2048 escenas nuevas, replay y auditorías finales. En familia deformada, la ruta geométrica mejora regret frente a Inyección bajo ambas losses, pero Geométrica-Decisión queda peor que su referencia clásica inicial (.037770 frente a .031161). No se acredita interacción especial con la loss ni ganador universal. Las revisiones están integradas; sigue diseñar el contraste bajo medición, sin promoción de arquitectura.
> **Operación y objetivo 2026-09-14**: el [diagnóstico de 2048 escenas y replay](../../experiments/atencion_armonica/RESULTS_OPERATOR_OBJECTIVE_ALIGNMENT.md) distingue orden global y decisión. En el primario deformado, Generativa tiene mayor Kendall τ-b que Extendida (0.434221 frente a 0.240753), pero mayor regret VI (0.082377 frente a 0.030361) y menor ARI. La diferencia de oracles VI/ARI observada no explica esa brecha. Sigue discriminar prospectivamente operación geométrica y aprendizaje de la decisión; no se identificó una causa ni se promovió arquitectura.
> **Evidencia generativa 2026-09-14**: el [contraste con cabeza y pérdida comunes](../../experiments/atencion_armonica/RESULTS_GENERATIVE_EVIDENCE_READER.md) completó 27 entrenamientos y cuatro tests con replay. En el primario deformado, Generativa mejora ARI frente a Desacoplada (+0.006858; IC97.5% [0.002706, 0.011171]), sin ventaja clara frente a Local (+0.002957; [−0.000668, 0.007134]). En polifonía pierde frente a Desacoplada. Extendida conserva mayor ARI en el primario como referencia de sistema, no como control causal. Ese resultado motivó el diagnóstico de operación y objetivo ahora completado. No hay promoción arquitectónica.
> **Diagnóstico de fuentes rivales 2026-09-08**: las [96 escenas y sus replays](../../experiments/atencion_armonica/RESULTS_OBSERVABLE_SOURCE_RIVALS.md) ya están completos. Bajo deformación, ampliar la familia reduce el residual mediano de 10.376 a 1.454 cents, pero las particiones exactas pasan sólo de 18 a 19 entre 23 escenas con candidatos; una de las 24 sigue sin salida. En mayor beta, la plantada ajusta mejor en las 24 escenas, aunque falta del universo observable en dos. El siguiente contraste debe aislar evidencia generativa bajo cabeza y loss comunes; este ajuste bajo ley conocida no acredita geometría neuronal aprendida ni armonía natural.
> **Corte geométrico 2026-09-08**: el [lector aprendido de particiones](../../experiments/atencion_armonica/RESULTS_LEARNED_PARTITION_READER.md) completó 36 entrenamientos y cuatro tests frescos con replay exacto. En el primario de mayor polifonía, Fuente compartida mejora ARI frente a Pares y Desacoplada, pero no muestra ventaja clara frente a Compatibilidad local; bajo familia deformada pierde frente a los tres. Local conserva menor error de cantidad de fuentes y menor masa de grupos pequeños en los cuatro escenarios. Las auditorías de evidencia y alineación cerraron sin hallazgos materiales abiertos. La mejora parcial de un lector sobre backbones congelados no demuestra nueva geometría latente ni identifica fuentes físicas. No hay arquitectura promovida.
> **Antecedente neuronal del ciclo**: el programa de [geometría armónica computable](ROADMAP_GENERAL/PROGRAMA_GEOMETRIA_ARMONICA_COMPUTABLE.md) produjo un [contraste completo](../../experiments/atencion_armonica/RESULTS_SHARED_PARTIAL_STUDY.md) de quince trainings y cinco tests, con representación y descriptores comunes para comparar pérdidas. Tras retirar el canal de amplitudes del banco, la penalización física empeoró Brier frente a BCE en las tres semillas del escenario primario; su promedio favorable frente a otros regularizadores no fue uniforme entre semillas. La lectura de particiones mostró otra ordenación y motivó el diagnóstico ahora completado. La extensión física set-valued queda pausada e incompleta; no se extrapola esta evidencia sintética a audio observado.
...
> **Ground truth proporcional / PPU**: una campaña de sesenta olas y más de ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. Las Olas 49–60 los llevaron a benchmarks clásicos, representación set-valued, políticas contextuales y un núcleo relacional con executors externos. La síntesis terminal redujo el relevo a tres líneas y `MAPPING-FEASIBILITY` descartó un factorial común por incompatibilidad de unidad, observación, target y stack decisional. El goal siguiente materializó los dos design freezes nativos. Su preflight CPU cerró `29 PASS / 1 FAIL`: el freeze relacional `GENERIC/TYPED × WLS/IRLS` fue rechazado porque la confirmación fresca K192 tuvo `191/192` convergencias y RMSE canónico p99 `0,001750`, por encima de `0,0001`; el freeze `MARGINAL/JOINT × HARD/CONTEXTUAL` pasó sus doce predicados. El runner de esta última rama ya fue implementado sobre datos abiertos: primario y replay pasaron `14/14`, las pruebas unitarias `9/9`, las mutaciones `54/54`, y R564 cerró `PASS 0/0/0`. Joint mejoró exact-set NLL pero no Brier; contextual redujo regret medio e incompatibilidad y empeoró worst regret; los controles matched no fueron evaluables por soporte común bajo. Es un diagnóstico de implementación, no evidencia prospectiva. La extensión física posterior queda pausada e incompleta; el nuevo ciclo geométrico no la retoma por inercia. EIV queda como referencia externa y el router, diferido. La campaña permanece cerrada como base de investigación y diseño, no como resolución científica. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.

> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R369 recuperó una señal de media sin intervalos compartidos; R370 mostró que ninguna señal proyecta cierre con `<=500` masters y que el firewall topology elimina 26 de 37 puntos favorables. R371 representó la tensión IID/grouped como frente y R372 abrió su mecanismo: `20→40%` nunca queda dominado, pero sólo `4/8` celdas conservan la relación entre roles; acuerdo pareado global `60,1%`. R373 dejó de subdividir el histórico y materializó la interfaz tipada `BudgetPath`: `608/608` rutas reconstruidas y `8/8` mutaciones rechazadas, sin política, utilidad, score o recomendación embebidos. R374 probó el puerto externo sobre fixtures sintéticos: `7/7` decisiones, `3/3` controles metamórficos y `12/12` rechazos adversariales; cero rutas históricas evaluadas y ninguna utilidad del usuario declarada. Esto acredita composición mecánica, no topology ni una decisión empírica. La activación espera utilidad auténtica y un freeze prospectivo dimensionado por R370. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
...
| Atención Armónica | Energía geométrica: 72 entrenamientos, 2048 escenas, replay y auditorías completos | Ventaja frente a Inyección en deformación, no mejora frente al clásico inicial. Próximo contraste bajo medición. Fases `0–0.6` históricas conservadas, sin certificar ausencia del canal global de amplitudes |
...
12. **Diseñar el contraste bajo medición desde el cierre de energía geométrica**: el [resultado del factorial](../../experiments/atencion_armonica/RESULTS_GEOMETRIC_DECISION_ENERGY.md) muestra ventaja frente a Inyección, pero ausencia de mejora frente al punto de partida clásico en deformación. Con el cierre técnico y de horizonte integrado, sigue probar si la estructura útil sobre coordenadas canónicas sobrevive a render y detección, sin otra cadena automática de correctores ni entrenamiento sobre tests abiertos. La extensión física set-valued sigue pausada e incompleta; promoción y GO/NO-GO siguen siendo decisiones del usuario.
...
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/ROADMAP_ATENCION_ARMONICA.md` | Contraste neuronal completo y diagnóstico geométrico post-hoc como continuidad; Stage B/detección preservados como alternativas |
...
*Documento revisado el 2026-09-07. El corte experimental general se conserva y la campaña proporcional cerró su fase bibliográfica acumulativa. El nuevo ciclo de investigación-acción persigue un contraste geométrico, sin declarar completa la extensión física set-valued pausada. CPU se usa cuando es proporcionada; GPU local requiere aviso y disponibilidad confirmada, y los trabajos remotos pertinentes están habilitados cuando toleren la cola y tengan protocolo y recursos verificados. Esto no constituye una PPU validada, un techo ni una decisión GO/NO-GO.*
