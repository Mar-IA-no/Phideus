...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y tres interfaces de riesgo. R363 mostró que topology transporta ajuste OOF pero no política bajo cobertura simultánea; R364 redujo esa conservatividad sin separar topology de controles. R365 reaplicó selected-action a dos cohortes: el firewall topology cambió `1/4→4/4`, `7/12` signos matched fueron inestables y los dos favorables proyectaron `4.993–8.900` masters. R366 igualó la oportunidad de actuar: con alpha public-base compartido y seis presupuestos, topology−permuted fue adverso en ambas cohortes en `37/72` celdas, inestable en `22/72` y favorable en `13/72`; sólo raw-generic conservó una franja balanceada favorable en `1–5%`, sin intervalos resueltos y con inversión posterior. El alpha fue `0,25` en `93,9–99,6%` de las vistas. El próximo diagnóstico CPU es mean-only: decidir si la cola debe limitarse a incertidumbre en vez de ordenar acciones. No se abre un freeze fresco por repetición. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
