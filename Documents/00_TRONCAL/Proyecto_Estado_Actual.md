...
> [!IMPORTANT]
> **Actualizado**: 2026-08-17 (auditoría documental; conserva el corte experimental consolidado al 2026-07-02)
...
*Documento revisado el 2026-08-17. El corte experimental sigue siendo el consolidado al 2026-07-02: Gate 5B completamente cerrado con `d4a4=84.0%±2.7pp` sobre 5 training seeds; Gate 6 con `Exp A` y `Exp B` cerrados negativamente y `Exp C` como única línea downstream abierta; Gate 8 cerrado `5/5`; Escalón 2 con null mecanístico inicial cerrado y `S2-P3` completado en su primera pasada; Voz Expresiva con el contraste `EN↔ZH` ya cerrado y una transferencia positiva acotada a `N-adapt`; Atención Armónica con `Fase 0`, `0.5` y `0.6` cerradas, pair-state fuerte, `triangle` positivo en `OOD-poly` y clusterers globales deployables que extraen parcialmente esa ventaja; Gate 10 completado `9/9` con lectura final `concat > FiLM/pca >> attn_bias`; Escalón 3 con baseline dual y primera línea geométrica `P5/P6` consolidada; Escalón 4 como expansión fisiológica futura; y el libro HIT como formulación larga pública del programa.*
