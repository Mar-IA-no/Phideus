...
> [!IMPORTANT]
> **Actualizado**: 2026-09-06 (Olas 26–60 integradas; `MAPPING-FEASIBILITY` descartó el factorial común vigente y habilitó dos contrastes nativos coordinados, con replay `148/148` y auditoría R544 `PASS 0/0/0`; sin promover arquitectura ni declarar techo; corte histórico general consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de sesenta olas y más de ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. Las Olas 49–60 los llevaron a benchmarks clásicos, representación set-valued, políticas contextuales y un núcleo relacional con executors externos. La síntesis terminal redujo el relevo a tres líneas y R533 exigió anteponer `MAPPING-FEASIBILITY`. Ese gate ya fue ejecutado por CPU: M1–M4 fallaron porque no existe unidad, observación, target ni stack decisional común; M5 y las hojas nativas R1–R6/S1–S6 pasaron, para un total `13 PASS / 4 FAIL`. La salida técnica `BIFURCATE_NATIVE_CONTRASTS` conserva un contraste `GENERIC/TYPED × WLS/IRLS` y otro `marginal/joint × hard/contextual`, coordinados en lineage y replay pero no fusionados. EIV queda como referencia externa y el router, diferido. Los dos runs coincidieron en `148/148` archivos core y `153/153` científicos; la suite v4 cubrió 67 mutaciones y R544 verificó el paquete canónico con `PASS 0/0/0`. La campaña queda cerrada como base de investigación y diseño, no como resolución científica. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-06. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a sesenta olas y cerró su fase acumulativa. `MAPPING-FEASIBILITY` resolvió por CPU que el factorial común vigente no conserva unidad, observación, target ni stack decisional, mientras ambos contrastes nativos son ejecutables; R544 auditó el paquete `PASS 0/0/0`. El modo Colab queda retirado. CPU continúa autónoma cuando es el recurso proporcionado; toda etapa para la cual GPU sea necesaria o materialmente más eficiente se detiene antes de ejecutarse, se informa por Telegram con objetivo, duración y VRAM estimados, y espera indicación del usuario. Esto no constituye una PPU validada, un techo ni una decisión GO/NO-GO.*
