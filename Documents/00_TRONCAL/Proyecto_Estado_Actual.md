...
> **Núcleo arquitectónico proporcional**: el programa ejecutó contrato, smoke, desentrelazado, diagnóstico de interfaz, adaptadores congelados, fidelidad del surrogate y contraste de pérdidas sobre grafos de log-razones. `K=64` reproduce el IRLS auditado; al entrenar la misma head, post-IRLS supera al target local en los ocho slices entre `-0,0428` y `-0,0551` IID y entre `-0,0084` y `-0,0118` grouped. Esa corrección, sin embargo, todavía empeora frente a `observed|unit` en todo IID entre `+0,0222` y `+0,0259`; grouped mejora en tres brazos y queda incierto en `closure_typed`. Los `20.192` solves convergieron y el replay igualó `32/32` artefactos. El target local estaba desalineado, pero una corrección siempre activa tampoco es una interfaz universal. El próximo diseño CPU es un gate residual con identidad exacta y observables públicos. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya ejecutó y reprodujo por CPU la cadena hasta el contraste de pérdidas post-IRLS. El próximo corte diseña un gate residual que preserve identidad y decida desde observables públicos cuándo corregir; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
