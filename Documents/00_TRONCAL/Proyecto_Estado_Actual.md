...
> [!IMPORTANT]
> **Actualizado**: 2026-09-06 (Olas 26–60 integradas; el transporte sin refit mejoró frente a hard pero no separó controles matched, R531 normalizó replay científico 36/36 y R533 validó el relevo experimental finito, sin promover arquitectura ni declarar techo; corte histórico general consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de sesenta olas y más de ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. La Ola 49 materializó un benchmark clásico sellado; las Olas 50–52 separaron conjunto, elección y utilidad contractual; la Ola 53 transportó incertidumbre marginal; y la Ola 54 mostró que un posterior conjunto mejora NLL, cardinalidad e interacciones sin garantizar una mejor decisión. La Ola 55 probó una compuerta escalar y seleccionó `hard_only`. La Ola 56 mostró que una compuerta Ridge contextual reduce regret medio pero pierde accuracy y empeora worst regret. La Ola 57 separó propuesta de valor y autorización de daño: el guard mejoró frente al proposer fijo en accuracy (`+0.0069`) y worst regret (`-0.0283`), pero frente al hard-set la mejora de regret fue pequeña (`-0.0038`), la compatibilidad cedió (`-0.0025`) y los intervalos de regret y cola cruzaron cero. Sólo `2/5` shams alcanzaron los mínimos, de modo que el patrón terminal quedó indeterminado; las condiciones 1–3 ya eran falsas, la 4 pasó y el replay fue exacto `23/23 + 13/13`. La Ola 58 abrió ese draw como diagnóstico de diseño: entre 36 candidatos y 24 probes nominó la política HGB/HGB con guard de incompatibilidad por regret medio, mientras un guard HGB de harm preservó una señal de cola más fuerte; 21 elegibles siguieron en Pareto, los IDs colapsaron a 19 firmas y JOINT fue idéntico a SEQUENTIAL. La Ola 59 llevó ambas políticas a un draw fresco con hard, proposer-only y cinco controles de desplazamiento condicional máximo por target. Incompatibilidad mejoró frente a hard en compatibilidad (`+0.0069`) y regret (`-0.0119`), pero su diferencia de regret contra controles tuvo IC95 superior `+0.00084`; cerró `7/8`, agregado falso. Harm mejoró worst regret frente a hard (`-0.0300`), pero no preservó compatibilidad —IC95 inferior `-0.00151`— ni separó sus controles —IC95 superior `+0.00347`—; cerró `6/8`, agregado falso. El replay fue exacto y la auditoría recompuso `192/192` arrays; el resultado debilita el bracket vigente sin refutar toda separación proposer/guard ni declarar un techo. La Ola 60 transportó las dos pipelines congeladas a otro draw sin refit ni recalibración: los ocho intervalos frente a hard fueron favorables, pero incompatibility no separó regret de controles matched —IC95 superior `+0.00155`— y harm no separó worst regret —IC95 superior `+0.00947`—; ambos patrones permanecieron falsos. R531 conservó el `MISMATCH 35/36` histórico y activó la vista normalizada `36/36` sin cambiar la ciencia. La síntesis terminal reduce el relevo a tres líneas y antepone un gate `MAPPING-FEASIBILITY`; R533 lo reauditoró `PASS 0/0/0`. En el carril arquitectónico local, el factorial `RAW/CLOSURE × GENERIC/TYPED` ya ejecutó dos seeds y replay exacto: el tipado y la mezcla de caminos mejoran corrección relacional y parte de WLS, pero decoder directo e IRLS no conservan la ventaja. El desentrelazado posterior mostró por CPU que el paquete corregido×aprendido mejora WLS en los ocho slices primarios y degrada IRLS en los seis evaluables. El diagnóstico de interfaz cerró la causa inmediata: WLS usa la asignación localizada de los pesos, mientras IRLS ve interferir esa base exógena con su reponderación residual; ni temperatura ni ridge pública transportaron de manera estable a grouped. Los adaptadores head-only confirmaron modularidad pero no transporte, y el diagnóstico siguiente validó `K=64` como surrogate numéricamente fiel de IRLS sobre `1.151` estados IID de validation. La salida sigue caracterizada como solver-condicionada y no como interfaz universal. El patrón acumulado separa `contrato material del target -> autoridad de relación/equivalencia -> manifest/attestation -> proposer o selector -> oracle/checker -> conjunto identificado con incertidumbre -> política de decisión -> abstención -> audit/replay -> reader`. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-06. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a sesenta olas y cerró su fase acumulativa. La Ola 60 transportó las dos pipelines HGB/HGB sin refit: mejoraron frente a hard, pero no separaron controles matched; R531 verificó replay científico normalizado 36/36. La síntesis auditada preserva tres líneas y abre un goal finito cuyo primer gate es MAPPING-FEASIBILITY. El modo Colab queda retirado. CPU continúa autónoma cuando es el recurso proporcionado; toda etapa para la cual GPU sea necesaria o materialmente más eficiente se detiene antes de ejecutarse, se informa por Telegram con objetivo, duración y VRAM estimados, y espera indicación del usuario. Esto no constituye una PPU validada, un techo ni una decisión GO/NO-GO.*
