...
> [!IMPORTANT]
> **Actualizado**: 2026-09-01 (Olas 26–42 de ground truth proporcional integradas y auditadas en su alcance; la Ola 42 separó autoridad, realizabilidad, identificabilidad, gauge, firma y aporte aprendido sobre `29` casos y `37` URLs, y preservó un proposer geométrico tipado bajo solver/checker externos sin seleccionar arquitectura, métrica, firma, suite, dominio o modelo; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y dos olas y noventa investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–40 construyeron contratos para objeto, unidades, experimento, operador, identificabilidad, transformación, grounding, revisión y autoridad; las Olas 26 y 28 añadieron anclas checker-only reproducibles, sin convertirlas en una primitive neuronal. La Ola 41 redujo el producto Clifford a su tensor de estructura exacto y dejó abierta sólo una hipótesis de sesgo inductivo frente a un gemelo tensorial igualado. La Ola 42 retrocedió un paso decisivo: antes del ejecutor deben estar autorizadas la observación, la métrica admisible, el gauge, el target y el tipo de output. Sus `29` casos, `37` URLs, `18` celdas y `12` hard negatives muestran que realizabilidad, identificabilidad, representante, firma, aporte aprendido y transferencia son claims distintos. Ninguno de los `23` brazos arquitectónicos relevados aisló la contribución aprendida bajo control completamente matched. El patrón acumulado es `contrato externo -> proposer tipado -> solver/canonicalizer/checker -> reader`, con output `point`, `class`, `identified set` o `UNKNOWN`; no una mega-arquitectura. `H41-X1`, B-08, CRNT y las demás alternativas permanecen recuperables, no priorizadas. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-01. El corte experimental general sigue siendo el consolidado al 2026-07-02; la novedad posterior es documental y programática: cuarenta y dos olas de ground truth proporcional quedaron integradas como base estratificada. Las Olas 26 y 28 aportaron anclas checker-only reproducibles; las Olas 29–40 ampliaron contratos de convención, jerarquía, acción, grounding y revisión sin promover una primitive común. La Ola 41 redujo Clifford a su tensor exacto; la Ola 42 estableció que métrica, firma, gauge y output requieren autoridad externa antes del proposer. Los patrones acumulados permanecen recuperables, pero no promovidos. No constituyen una PPU validada ni una decisión GO/NO-GO.*
