...
> [!IMPORTANT]
> **Actualizado**: 2026-09-03 (Olas 26–50 de ground truth proporcional integradas; la Ola 50 ejecutó y reprodujo el primer contraste neuronal matched entre salida exclusiva y compatibilidad set-valued; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cincuenta olas y ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. La Ola 49 materializó un benchmark clásico sellado con cuatro familias catalogadas, dos fuera de catálogo, EIV y abstención; en lockbox in-catalog, EIV elevó cobertura de `0.839` a `0.969`, mientras la abstención detectó `0.496` del OOD abstention-required con `0.005` de falsa abstención in-catalog. La Ola 50 ejecutó sobre un paquete fresco el contraste neuronal matched que ese banco habilitaba. En `NEAR_RIVAL`, `sigmoid_set` mejoró el recall del conjunto sobre `softmax_partial` en `+0.1148`, IC 97,5% `[+0.0693,+0.1582]`; el top-1 quedó casi igual en el punto (`-0.0013`), pero no alcanzó por `0.00125` el margen de no inferioridad. El patrón conjunto preregistrado fue falso y el ancho excedió levemente su límite de transferencia (`0.2565 > 0.25`). El replay exacto bajo exclusiones runtime declaradas y la auditoría independiente sostienen la integridad técnica. El patrón acumulado separa `contrato material del target -> autoridad de relación/equivalencia -> manifest/attestation -> proposer o selector -> oracle/checker -> conjunto identificado -> política de decisión -> audit/replay -> reader`. Surge como candidata una interfaz de conjunto identificado más decisión posterior, pero no hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-03. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta olas. La Ola 50 ejecutó el contraste neuronal matched abierto por la Ola 49: la salida sigmoid conservó más alternativas compatibles, pero el patrón conjunto falló por el margen top-1 y pagó un exceso leve de ancho. La recuperación técnica y el replay quedaron auditados. La separación conjunto-identificado/política-de-decisión pasa a ser candidata recuperable, no arquitectura promovida. No constituye una PPU validada ni una decisión GO/NO-GO.*
