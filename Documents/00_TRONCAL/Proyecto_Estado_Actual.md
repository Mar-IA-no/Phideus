...
> [!IMPORTANT]
> **Actualizado**: 2026-09-03 (Olas 26–52 de ground truth proporcional integradas; la Ola 52 probó y reprodujo en CPU el transporte de una representación set-valued bajo políticas ordinales cambiantes; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cincuenta y dos olas y ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. La Ola 49 materializó un benchmark clásico sellado con cuatro familias catalogadas, dos fuera de catálogo, EIV y abstención; en lockbox in-catalog, EIV elevó cobertura de `0.839` a `0.969`, mientras la abstención detectó `0.496` del OOD abstention-required con `0.005` de falsa abstención in-catalog. La Ola 50 ejecutó el contraste neuronal matched que ese banco habilitaba: `sigmoid_set` mejoró el recall `NEAR_RIVAL` sobre `softmax_partial` en `+0.1148`, pero el patrón conjunto preregistrado fue falso por top-1 y ancho. La Ola 51 mostró que congelar el conjunto y agregar una cabeza de elección no mejora por sí solo el sistema. La Ola 52 introdujo una autoridad ordinal contractual: la política explícita elevó compatibilidad (`+0.0543`) y redujo regret (`-0.0407`) frente al selector directo, aunque no mejoró concluyentemente la exactitud (`+0.0051`, IC95 cruzando cero) y su éxito contrafactual (`0.677`) quedó bajo el criterio `0.80`. Corrida y réplica CPU coincidieron en `497/497` arrays y `1.548/1.548` tensores. El patrón acumulado separa `contrato material del target -> autoridad de relación/equivalencia -> manifest/attestation -> proposer o selector -> oracle/checker -> conjunto identificado -> política de decisión -> audit/replay -> reader`. La interfaz conceptual gana una función operacional, pero esta receta no queda promovida: el próximo discriminante debe transportar incertidumbre del conjunto hacia una decisión robusta. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-03. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y dos olas. La Ola 52 mostró que una autoridad ordinal contractual vuelve efectiva la separación entre conjunto y decisión para reducir incompatibilidad y regret, pero no produjo una ventaja concluyente de exactitud ni suficiente control contrafactual. La interfaz conceptual queda mejor determinada: el problema siguiente es transportar incertidumbre, no agregar otra cabeza sobre un conjunto tratado como exacto. No constituye una PPU validada ni una decisión GO/NO-GO.*
