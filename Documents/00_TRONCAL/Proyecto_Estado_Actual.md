...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R367 aisló el aporte de la cola: mean-only supera `mu+u_topology` en `50/72`. R368 separó ranking y elegibilidad, pero el filtro topology sólo habilitó `31/2.032` decisiones en A y `7/2.040` en B y perdió frente a mean-only en `57/72`. R369 atribuyó entonces la señal del ranker: topology-mean supera reduced en `43/72`, target-shuffled en `45/72` y topology-permuted en `37/72`, sobre todo grouped/balanceado; contra el último control no hay intervalos compartidos resueltos y el firewall revierte la comparación a `20/72` adverso frente a `11/72` favorable. La primitive recuperable parcial es la media topology, no su cola, pero localización incremental y deployment siguen abiertos. El próximo diagnóstico CPU audita potencia y no linealidad de selección antes de otra realización. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
