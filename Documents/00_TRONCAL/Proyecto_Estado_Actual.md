...
> [!IMPORTANT]
> **Actualizado**: 2026-09-07 (el dual native freeze cerró `SET_VALUED_FREEZE_ONLY_VALID`: 29/30 predicados, replay exacto, 61/61 mutaciones y R557 `PASS 0/0/0`; el surrogate relacional K192 no superó su confirmación predeclarada y la continuación CPU es implementar el runner set-valued sin abrir todavía un draw nuevo; sin promover arquitectura ni declarar techo; corte histórico general consolidado al 2026-07-02)
...
12. **Continuar el relevo experimental de geometría proporcional por la única rama habilitada**: el dual native freeze rechazó el surrogate relacional K192 y dejó `SET_VALUED_FREEZE_ONLY_VALID`. El próximo trabajo implementa y audita por CPU el runner `MARGINAL/JOINT × HARD/CONTEXTUAL` contra fixtures y artefactos ya abiertos, sin crear todavía el draw fresco ni abrir el monitor; el router sólo se reconsidera si una primitive sobrevive a sus controles.
...
*Documento revisado el 2026-09-07. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a sesenta olas y cerró su fase acumulativa. El dual native freeze resolvió por CPU que sólo la rama set-valued está lista para implementar su runner: la rama relacional falló su confirmación K192 predeclarada y no se retuneó sobre ese draw. R557 auditó el paquete `PASS 0/0/0`. El modo Colab queda retirado. CPU continúa autónoma cuando es el recurso proporcionado; toda etapa para la cual GPU sea necesaria o materialmente más eficiente se detiene antes de ejecutarse, se informa por Telegram con objetivo, duración y VRAM estimados, y espera indicación del usuario. Esto no constituye una PPU validada, un techo ni una decisión GO/NO-GO.*
