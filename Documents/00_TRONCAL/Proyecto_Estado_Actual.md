...
> [!IMPORTANT]
> **Actualizado**: 2026-08-30 (Ola 21 de ground truth proporcional integrada y auditada; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y dos investigaciones independientes y dos carriles reconstruidos por el coordinador, organizados en veintiuna olas, no encontró una geometría universal ni un corpus único. Propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura candidata separa objeto/unidades, operador, constitución, identificabilidad, solver, reader y autoridad. La decimoctava separó invariancia de separación orbital y estabilidad topológica de suficiencia o autoridad de filtración; la decimonovena agregó suites P0 para autoridad armónica/orbital y estado predictivo; la vigésima distinguió, en el espacio y en la dinámica, entre representación, operador, semigrupo o generador, solver, query y autoridad. La vigesimoprimera mostró que una respuesta externa puede identificar un punto, un cociente, una región alcanzable, un conjunto compatible o sólo `UNKNOWN`, y registró `Response-Quotient Atlas` como candidata no numerada para mantener separadas inversión tipada, compatibilidad de candidatos y autoridad física. Los protocolos están auditados y programáticos, no ejecutados. `A10` y `A11` siguen como especializaciones; `A14` sigue como interfaz; no hay `A19`, GO/NO-GO ni arquitectura promovida.
