...
> [!IMPORTANT]
> **Actualizado**: 2026-09-01 (Olas 26–46 de ground truth proporcional integradas y auditadas en su alcance; la Ola 46 convirtió las piezas dispersas de identidad, lineage, evaluación sellada y atribución por etapas en cinco blueprints prospectivos de un harness tipado, sin hallar un bundle material, seleccionar stack o promover arquitectura; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y seis olas y ciento dos investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–40 construyeron contratos para objeto, unidades, experimento, operador, identificabilidad, transformación, grounding, revisión y autoridad; las Olas 26 y 28 añadieron anclas checker-only reproducibles, sin convertirlas en una primitive neuronal. La Ola 41 redujo el producto Clifford a su tensor de estructura exacto y dejó abierta sólo una hipótesis de sesgo inductivo frente a un gemelo tensorial igualado. La Ola 42 estableció que observación, métrica admisible, gauge, target y tipo de output requieren autoridad antes del ejecutor. La Ola 43 materializó el paso intermedio: observación y ley autorizadas producen una relación característica que witnesses externos deben factorizar y tipar antes de habilitar una geometría. La Ola 44 convirtió sus gaps experimentales en un contrato de benchmark con lineage graph, split atómico, propuesta cruda hasheada, repair delta, access graph de checker/solver, status estructural, objeto point/set, policy y unidad inferencial independiente. La Ola 45 contrastó ese contrato con `40` source-cases, `26` familias efectivas y `48` URLs de autoridad material, sellado y atribución de etapas. Sus `19` contratos adjudicados, `497` predicados y `285` hard negatives no produjeron un bundle tricarril completo: CSP7 conserva compatibilidad histórica parcial y los demás cruces son anti-joins o piezas de un solo carril. La Ola 46 investigó cómo construir prospectivamente ese contrato: integró `34` component cases, `49` URLs, `52` claims, `36/36` joins materiales y `408` hard negatives entre identidad/lineage, evaluación sellada y crédito por etapas. Ningún componente cubre una celda constitutiva completa ni apareció un join tricarril. La contribución es un posible harness tipado de autoridad, descompuesto en cinco blueprints recuperables, no un stack ni una arquitectura promovida. El patrón acumulado es `contrato externo -> manifest de identidad/lineage/access -> proposer de relación con firewall de crédito -> seal/window/query -> factorizer/checker/repair -> status/objeto/policy -> audit/replay -> reader`, no una mega-arquitectura. `H41-X1`, H43-A–F, H44-A–F, los cinco blueprints de Ola 46, B-08, CRNT y las demás alternativas permanecen recuperables, no priorizadas. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-01. El corte experimental general sigue siendo el consolidado al 2026-07-02; la novedad posterior es documental y programática: cuarenta y seis olas de ground truth proporcional quedaron integradas como base estratificada. Las Olas 26 y 28 aportaron anclas checker-only reproducibles; las Olas 29–40 ampliaron contratos de convención, jerarquía, acción, grounding y revisión sin promover una primitive común. La Ola 41 redujo Clifford a su tensor exacto; la Ola 42 estableció autoridad antes del proposer; la Ola 43 derivó la geometría de una relación característica autorizada y tipada; la Ola 44 especificó cómo un benchmark debe preservar lineage, crédito por etapa y abstención; la Ola 45 mostró que las piezas públicas de autoridad, sellado y crédito todavía no coexisten en un contrato indivisible; y la Ola 46 convirtió esas piezas en interfaces y blueprints prospectivos sin transferirles autoridad ni seleccionar un stack. Los patrones acumulados permanecen recuperables, pero no promovidos. No constituyen una PPU validada ni una decisión GO/NO-GO.*
