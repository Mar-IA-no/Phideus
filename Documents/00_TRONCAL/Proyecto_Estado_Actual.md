...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R368 separó ranking y elegibilidad, pero el filtro topology perdió frente a mean-only en `57/72`. R369 recuperó una señal en la media: topology-mean supera topology-permuted en `37/72`, sobre todo grouped/balanceado, aunque sin intervalos compartidos resueltos. R370 mostró que ninguna de esas señales proyecta cierre con `<=500` masters —mínimo `522`, mediana `5.552`— y que el firewall topology elimina 26 de 37 puntos favorables. R371 reemplazó ese selector binario por el frente Pareto IID/grouped: la retención selection→adjudication promedia `0,771`, pero el Jaccard A/B sólo `0,542`; topology domina a reduced `8/8`, divide `4/8` dominante y `4/8` incomparable frente a public-base, y queda incomparable con permuted en `86/128` arenas. La primitive recuperable parcial es una ruta set-valued de ranking medio, no una política ni la cola. Sigue una auditoría CPU de dominancia pareada entre presupuestos; no hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya representa por CPU la tensión IID/grouped como un frente de políticas. Ese frente transporta parcialmente y continúa sin acreditar valor topológico incremental frente a controles. Toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
