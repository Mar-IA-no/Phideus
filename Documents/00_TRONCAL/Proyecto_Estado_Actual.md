...
> [!IMPORTANT]
> **Actualizado**: 2026-09-04 (Olas 26–55 integradas; el núcleo local validó por CPU un surrogate IRLS `K=64` con replay byte-exacto; conserva el corte experimental consolidado al 2026-07-02)
...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya llegó a una calibración mixta prospectiva con adjudicación fresca. Dos realizaciones disjuntas aportaron `251` masters IID/grouped para calibrar y otros `251` para adjudicar después del freeze. El gate público mejoró grouped frente a identity entre `-0,00156` y `-0,00239` en los cuatro brazos; en IID los cuatro intervalos cruzaron cero. El promedio balanceado fue favorable en todo el factorial y quedó resuelto en raw-generic (`-0,00098`) y raw-typed (`-0,00115`). Tres brazos grouped superaron al gate IID histórico y tres a shuffles, pero ninguna comparación demostró que las quince features superen al gate reducido de dos medidas de escala. Los `40.160/40.160` solves convergieron y el replay igualó `51/51` artefactos. Existe señal pública parcial y reproducible de routing, no una política favorable por mecanismo ni una interfaz universal. El próximo diseño CPU debe usar realizaciones nuevas para calibrar abstención bajo restricción de no-daño IID; no se retoca el test abierto. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya ejecutó y reprodujo por CPU una calibración mixta con adjudicación fresca. El próximo corte diseña otra realización para una abstención con restricción de no-daño IID; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
