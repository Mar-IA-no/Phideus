...
> [!IMPORTANT]
> **Actualizado**: 2026-09-01 (Olas 26–43 de ground truth proporcional integradas y auditadas en su alcance; la Ola 43 materializó la cadena observación/ley autorizada → relación característica → factorizer/checker externo → output geométrico tipado sobre `29` casos y `51` URLs, sin seleccionar arquitectura, métrica, firma, suite, dominio o modelo; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y tres olas y noventa y tres investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–40 construyeron contratos para objeto, unidades, experimento, operador, identificabilidad, transformación, grounding, revisión y autoridad; las Olas 26 y 28 añadieron anclas checker-only reproducibles, sin convertirlas en una primitive neuronal. La Ola 41 redujo el producto Clifford a su tensor de estructura exacto y dejó abierta sólo una hipótesis de sesgo inductivo frente a un gemelo tensorial igualado. La Ola 42 estableció que observación, métrica admisible, gauge, target y tipo de output requieren autoridad antes del ejecutor. La Ola 43 materializó el paso intermedio que faltaba: observación y ley autorizadas producen una relación característica; su factorización, hiperbolicidad y compatibilidad deben ser adjudicadas por witnesses externos antes de habilitar una geometría. Sus `29` source-cases, `51` URLs, `15` celdas (`10 COVERED / 5 CONTRACTUAL-GAP`), `44` relaciones y `12` hard negatives sostienen outputs tipados como clase conforme/proyectiva, cono, multicono, variedad higher-order, degeneración, conjunto identificado o `NONE_AUTHORIZED`. El patrón acumulado es `contrato externo -> proposer de relación -> factorizer/checker -> router geométrico -> reader`, no una mega-arquitectura. El carril experimental todavía carece de cuatro de cinco piezas decisivas y `H41-X1`, H43-A–F, B-08, CRNT y las demás alternativas permanecen recuperables, no priorizadas. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
