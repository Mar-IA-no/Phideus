...
> **Atención Armónica**: el frente nuevo ya dejó de ser solo rediseño. El `sweep` `v2.1` pasó, el `final_pool` quedó congelado con gate `PASS`, el smoke supervisado confirmó aprendibilidad sin saturación y el training decisivo de `Fase 0` ya entró en curso. Eso **todavía no lo vuelve canónico** dentro del programa: la pregunta abierta ya no es si el dataset hace trampa, sino si el lift eventual de `B` se sostiene específicamente contra `B-local`, `B-shuffle` y `B-minus`, es decir, si hay atribución limpia a transitividad/`triangle` y no solo a pair-state genérico.
...
| Atención Armónica | Incubación activa | `v2.1` ya pasó `sweep` y gate final sobre `final_pool`; el training decisivo de `Fase 0` ya corre, pero el frente sigue metodológicamente abierto hasta cerrar la atribución `B vs B-local / B-shuffle / B-minus` |
...
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/README.md` | Estado canónico local del frente: gate `v2.1` ya pasado, `final_pool` congelado y training decisivo en curso, con atribución todavía pendiente |
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/ROADMAP_ATENCION_ARMONICA.md` | Roadmap del frente incubado: el cuello ya no es el dataset sino la atribución causal del lift entre pair-state genérico y transitividad/`triangle` |
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/PLAN_FASE_0_v2_1.md` | Plan operativo vigente de `Fase 0`: diseño `v2.1`, combo congelada, gate `PASS`, smoke supervisado ya hecho y controles obligatorios todavía abiertos |
