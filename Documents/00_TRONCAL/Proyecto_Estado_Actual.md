...
> [!IMPORTANT]
> **Actualizado**: 2026-08-30 (Ola 25 de ground truth proporcional integrada, reauditada y respaldada; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cincuenta investigaciones independientes y dos carriles reconstruidos por el coordinador, organizada en veinticinco olas, no encontró una geometría universal ni un corpus único. Propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura candidata separa objeto/unidades, experimento, operador, constitución, identificabilidad, solver, reader y autoridad. Las Olas 20–24 distinguieron operador, respuesta, experimento, verdad set-valued y transformación autorizada. La Ola 25 agrega que tampoco existe una geometría única de conjuntos: clase de objeto, ambiente, observación, regularidad, operación y query determinan qué métrica o topología tiene jurisdicción. `Set Geometry Authority Contract` queda como interfaz externa anterior al encoder, con schema/checker sin modelos y comparación posterior de readers sobre inputs byte-identical. La reauditoría cerró sus findings, pero los bancos siguen bloqueados hasta materializar y congelar sus referencias clásicas, hashes, hardware y costos. `A10` y `A11` siguen como especializaciones; `A14` sigue como interfaz; no hay `A19`, GO/NO-GO ni arquitectura promovida.
...
*Documento revisado el 2026-08-30. El corte experimental sigue siendo el consolidado al 2026-07-02; la novedad posterior es documental y programática: veinticinco olas de ground truth proporcional quedaron integradas como base estratificada. Completación, exchange, cocientes de forma, sincronización y closures conservan sus contratos propios; representación orbital, filtración, autoridad armónica, estado predictivo, autoridad de operadores, transformación set-valued y jurisdicción de geometrías de conjuntos permanecen como suites diagnósticas. `Response-Quotient Atlas`, `Experiment-Relative Geometry Atlas`, `Identified-Set Authority Stack`, `Guaranteed Set Transformer Stack` y `Set Geometry Authority Contract` amplían el atlas con outputs y jurisdicciones tipadas, pero siguen siendo hipótesis no promovidas. Los protocolos continúan sin ejecución y no se declaró GO/NO-GO.*
