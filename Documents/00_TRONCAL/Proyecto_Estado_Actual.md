...
> **Ground truth proporcional / PPU**: una campaña de veintidós investigaciones independientes, organizadas en diez olas, quedó integrada. No encontró una geometría universal ni un corpus único; propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura candidata separa objeto/unidades, operador, constitución, identificabilidad, solver, reader y autoridad. La octava ola agregó `A7/P2g/P2h`, un identificador-compilador certificado; la novena, `A8/P2i/P2j`, con contratos metrológicos y de adquisición. La décima agregó `A9/P2k`, un operador tipado entre escalas que separa `KNOWN-KERNEL-TRANSPORT` de `KERNEL-DISCOVERY`, deja los oráculos fuera del estimando y exige convergencia en tamaño, truncación y base. La psicofísica de magnitud quedó como transferencia externa `P5e`: factorial de representación × ruido × decisión, con controles de componentes/rango/generador y sin autoridad ontológica. No son experimentos ejecutados; no hay GO/NO-GO ni arquitectura promovida.
