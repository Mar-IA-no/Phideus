...
> **Escalón 2**: ya no está solo “abierto” en abstracto ni en su primera fase descriptor-guided. `S2-P0` y `S2-P1` quedaron completos sobre French Lombard `v1.1` con `38` speakers (`20F/18M`), `9,120` clips y ~`20h` reales en repo. Ya existen `data/lombard/manifest.json`, `data/lombard/segment_index.json` (`108,536` segmentos), `data/lombard/alignment_audit.json` y `data/lombard/p1_results/p1_results_noise0.json`; el split canónico quedó en `28/5/5`, el piloto limpio usa `noise0`, `lag_correction_samples=0` y `voiced_threshold=0.1494`, y el baseline lineal ya dio `CCA S=64.4%` contra `7.8%` random. `S2-P2-control` (`D0` neural, `29.1M` params) ya cerró con `S=77.8% @ ep25`, `CI=[72.0%, 80.8%]`. La fase `S2-P2-main` por concatenación también ya cerró y devolvió una lectura negativa útil: `V4-lin=67.8%`, `H-series=59.8%`, `A4-16k=77.8%=D0`. En `S2-P2.5`, la primera fase attention-based ya dejó sus tres brazos originales: `V4-lin-attnbias=70.6% @ e25`, `H-series-xattn=73.4% @ e29` y `A4-16k-xattn=78.4% @ e10` como corte todavía provisional. Con eso, el frente ya no corre como un único ensayo lineal sino como un factorial `3x2` que busca desconfundir descriptor y mecanismo, siempre bajo la taxonomía A/B/C/D y el preregistro [PREDICCIONES_EPISTEMOLOGICAS_P25.md](../01_FRENTES_ACTIVOS/ESCALON_2/S2_P2/PREDICCIONES_EPISTEMOLOGICAS_P25.md), que fija bootstrap pareado sobre `Delta`, matriz de predicciones y guardrails para nulls.
> **Gate 9 / revisión A10**: también quedó formalizada una rama retrospectiva secundaria sobre armonía natural en música. `A7r/A9r` ya están planificados como piloto preregistrado para releer la deuda natural-harmonic de Escalón 1 con el mecanismo ganador (`reverse cross-attention`), y la revisión `A10` abre variantes continuas ontology-free (`A10d/A10e`) para no presuponer la ontología JI como punto de llegada. Esta rama no desplaza el foco principal del programa: primero se cierra y lee `S2-P2.5`; recién después se decide cuánto valor agrega reabrir armonía natural en MAESTRO.
...
| Gate 9 natural harmony | Planificado / oportunista | `A7r/A9r` ya quedaron preregistrados; la revisión `A10` abre variantes continuas ontology-free (`A10d/A10e`) y controles explícitos; la rama se ejecuta sin desplazar `S2-P2.5` ni Gate 6 |
| Escalón 2 | Activo | `S2-P0` y `S2-P1` completos; `S2-P2-control` neural ya cerrado (`S=77.8% @ ep25`); `S2-P2-main` concat ya leído; `S2-P2.5` Fase 1 ya cerrada y factorial `3x2` corriendo |
...
3. **Leer Gate 6 como frente ya activo**: `Exp C` ya dejó una referencia local completa (`a4r`, `best_F1=0.1570 @ ep50`) y los arrays UNC `1144720/1144721` ya están corriendo como validación real de `Exp B/Exp A`.
...
7. **Sostener Escalón 2 como frente abierto también en la capa attention-guided**: `S2-P2-control` ya cerró sobre `noise0`, la fase concat ya devolvió un no parcial, `S2-P2.5` Fase 1 ya mostró que el mecanismo importa y la pregunta inmediata pasa a ser cómo se reparte ese efecto una vez cerrado el factorial `3x2`.
...
10. **Mantener Gate 9 como reapertura retrospectiva y no como pivote prematuro**: `A7r/A9r` y la revisión `A10` sirven para releer la deuda natural-harmonic dentro de música, pero solo después de fijar la lectura factorial de `S2-P2.5`.
...
*Documento actualizado al corte operativo 2026-03-11 (Gate 5B completamente cerrado; Gate 6 ya con `preflight v6` exitoso y `Exp A+B` submitidos en UNC; Gate 8 ya cerrado `5/5` con `pca=82.6%`; Escalón 2 ya con `S2-P2.5` fase 1 cerrada y factorial `3x2` corriendo como etapa attention-based; y Gate 9 / revisión `A10` ya formalizados como rama retrospectiva secundaria sobre armonía natural).*
