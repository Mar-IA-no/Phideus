...
> [!IMPORTANT]
> **Actualizado**: 2026-09-03 (Olas 26–53 de ground truth proporcional integradas; la Ola 53 probó y reprodujo en CPU una política de regret esperado sobre incertidumbre marginal y localizó su límite en la dependencia conjunta entre familias; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cincuenta y tres olas y ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–48 construyeron contratos para objeto, unidades, equivalencia, identificabilidad, transformación, autoridad material y autoridad de la relación. La Ola 49 materializó un benchmark clásico sellado con cuatro familias catalogadas, dos fuera de catálogo, EIV y abstención; la Ola 50 mostró que `sigmoid_set` conserva mejor el conjunto compatible sin resolver top-1; la Ola 51 descartó la separación two-stage simple; y la Ola 52 volvió operativa una autoridad ordinal contractual, con más compatibilidad y menos regret pero sin ventaja concluyente de accuracy. La Ola 53 transportó incertidumbre marginal hacia regret esperado. Mejoró compatibilidad frente al conjunto duro (`+0.0262`), pero perdió accuracy (`-0.0628`) y no redujo regret de forma concluyente. La abstención sí ordenó riesgo (`0.1223 -> 0.0798` con cobertura efectiva `0.7162`), mientras correlaciones residuales de hasta `0.38` y error de cardinalidad cercano a `0.43` localizaron el límite en la independencia Bernoulli. El próximo discriminante mantiene el encoder congelado y modela un posterior conjunto sobre las quince regiones compatibles. El patrón acumulado separa `contrato material del target -> autoridad de relación/equivalencia -> manifest/attestation -> proposer o selector -> oracle/checker -> conjunto identificado con incertidumbre -> política de decisión -> abstención -> audit/replay -> reader`. No hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-03. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y tres olas. La Ola 53 mostró que transportar marginales calibradas mejora compatibilidad y habilita abstención útil, pero no supera a la política sobre conjunto duro y distorsiona dependencias y cardinalidad. El siguiente discriminante cambia la distribución sobre conjuntos completos, no el encoder. No constituye una PPU validada ni una decisión GO/NO-GO.*
