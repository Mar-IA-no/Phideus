...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R368 separó ranking y elegibilidad, pero el filtro topology perdió frente a mean-only en `57/72`. R369 recuperó una señal en la media, aunque sin intervalos compartidos; R370 mostró que ninguna señal proyecta cierre con `<=500` masters y que el firewall topology elimina 26 de 37 puntos favorables. R371 reemplazó ese selector por el frente Pareto IID/grouped: retención `0,771`, Jaccard A/B `0,542` e incomparabilidad con permuted en `86/128` arenas. R372 abrió su mecanismo: `20→40%` nunca queda dominado, pero sólo `4/8` celdas conservan la relación entre roles; el acuerdo pareado global es `60,1%` y A/B `67,9–70,2%`. La primitive parcial es una ruta set-valued de ranking medio, no una política ni la cola. El histórico dejó de subdividirse; sigue el diseño CPU de una interfaz tipada `BudgetPath`, con utilidad externa. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya representa por CPU la tensión IID/grouped como una ruta de políticas anidadas. La ruta transporta parcialmente y continúa sin acreditar una elección ni valor topológico incremental robusto. Toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
