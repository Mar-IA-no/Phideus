...
> [!IMPORTANT]
> **Actualizado**: 2026-09-03 (Olas 26–55 integradas; el núcleo local ejecutó toda la cadena CPU hasta adaptadores con tronco congelado y replay byte-exacto; conserva el corte experimental consolidado al 2026-07-02)
...
> **Núcleo arquitectónico proporcional**: el programa ejecutó contrato, smoke, desentrelazado, diagnóstico de interfaz y adaptadores congelados sobre grafos de log-razones. El peso aprendido contiene localización útil para WLS, pero interfiere con IRLS. El checkpoint permitió aislar heads de `4.225/4.224` parámetros: el refit WLS mejoró IID entre `-0.0112` y `-0.0151`, pero no grouped; el refit relacional redujo su error local y empeoró IRLS IID entre `+0.0664` y `+0.0763`. Corrida y replay igualaron `31/31` artefactos deterministas en unos `304 s` y `0.822 GiB`. La modularidad es viable, no así los objetivos head-only ensayados. El próximo paso CPU debe diseñar un target o surrogate alineado con robustez residual. No hay arquitectura promovida ni GO/NO-GO; reentrenamiento integral y GPU quedan en cola.
