...
> **Ground truth proporcional / PPU**: una campaña de veinte investigaciones independientes, organizadas en nueve olas, quedó integrada. No encontró una geometría universal ni un corpus único; propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura candidata separa objeto/unidades, operador, constitución, identificabilidad, solver, reader y autoridad. La octava ola agregó `A7/P2g/P2h`, un identificador-compilador certificado. La novena agregó contratos metrológicos y de adquisición: `CALIBRATED-PROPORTIONAL` exige mensurando, referencia, calibración prospectivamente congelada, incertidumbre conjunta y covarianza; una política adaptativa debe fijar candidate set, acceso, costo, safety y campaña como unidad inferencial. `A8/P2i/P2j` permanece candidata: primero se valida como protocolo externo GUM/OED/system-ID y sólo después se compara un módulo aprendido contra ese protocolo y un sham/ablación. No son experimentos ejecutados; no hay GO/NO-GO ni arquitectura promovida.
