...
> [!IMPORTANT]
> **Actualizado**: 2026-09-03 (Olas 26–49 de ground truth proporcional integradas; la Ola 49 ejecutó en CPU el primer benchmark clásico sellado de discriminación relacional set-valued; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y nueve olas y ciento ocho investigaciones independientes no encontró una geometría universal ni un corpus único. Organizó fuentes analíticas, simulación generativa, cámaras físicas y evidencia externa bajo falsación adversarial y adjudicación ciega. Las Olas 20–40 construyeron contratos para objeto, unidades, experimento, operador, identificabilidad, transformación, grounding, revisión y autoridad; las Olas 26 y 28 añadieron anclas checker-only reproducibles. Las Olas 41–48 separaron ejecución, autoridad geométrica, relación característica, crédito, autoridad material del target y autoridad de la relación. La Ola 49 convirtió la cabeza discriminante C48-D en un benchmark clásico sellado: `4.992` fixtures por split, `3.072` de calibración, cuatro familias catalogadas y dos fuera de catálogo. En lockbox in-catalog, EIV elevó cobertura de `0.839` a `0.969`; la abstención conformal acertó en `0.496` de los OOD abstention-required (`0.199` de abstención sobre todo el fuera de catálogo) con `0.005` de falsa abstención in-catalog; forzar `PROP` sólo cubrió `0.222` in-catalog. La ejecución final pasó checker y `13/13` mutaciones. R284 auditó el piloto finalista; R285 auditó específicamente el artefacto completo, reprodujo `14/14` archivos y verificó `15/15` snapshots contra `d638ad8`. El patrón acumulado separa `contrato material del target -> autoridad de relación/equivalencia -> manifest/attestation -> proposer o selector -> oracle/checker -> status/set/abstención -> audit/replay -> reader`. El resultado habilita un contraste neuronal matched, pero no hay `A19`, nuevo `P2*`, arquitectura promovida ni decisión GO/NO-GO.
...
*Documento revisado el 2026-09-03. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cuarenta y nueve olas. La Ola 49 añadió ejecución: materializó y auditó un benchmark clásico set-valued con EIV, abstención, lockbox sellado y replay exacto. La cobertura alta no equivale a singleton, y la detección OOD todavía es incompleta. El próximo discriminante posible es neuronal y matched, pero los patrones acumulados permanecen recuperables, no promovidos. No constituyen una PPU validada ni una decisión GO/NO-GO.*
