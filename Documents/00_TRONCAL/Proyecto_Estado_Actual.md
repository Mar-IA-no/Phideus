...
> **Decisión operativa vigente**: (1) tratar `Test02` como cierre causal del argumento de capacidad, (2) leer `13G-B` como resultado negativo/generativo genérico y usar `Test11` para sostener el hallazgo mecanístico del cuello de proyección, (3) tratar la rama `Transkun+A4` de Gate 6 como cierre negativo útil (`Exp A` y `Exp B`) y conservar `Exp C` como única línea downstream abierta, (4) leer Gate 8 ya como línea cerrada con una señal positiva real sobre proyección (`pcd > pca > pcd-zero > pcm > ctrl`), (5) tratar Escalón 2 como frente principal ya con null mecanístico inicial cerrado y `S2-P3` **implementado/en ejecución**, (6) usar Gate 9 / `A10` como evidencia retrospectiva ya disponible pero subordinada, y (7) leer Gate 10 ya como contraste causal cerrado donde el mecanismo domina sobre el descriptor (`concat > FiLM/pca >> attn_bias`).
