...
> **Voz Expresiva Phideus**: el frente exploratorio de voz ya salió de la pura incubación conceptual y cerró sus dos primeras fases sobre `ESD` English. `Fase 0A` mostró señal específica de la familia ratio-based frente al control no-ratio (`A: eta²=0.385` en `Hseries_d5_mean` frente a `C: eta²=0.076`), y `Fase 0B` dejó una lectura más exigente: bajo `N-strict` ningún stack descriptor-only valida generalización honesta a hablante nuevo, pero bajo `N-adapt` la familia `A` sí muestra especificidad robusta frente al control (`A-only > C-only`, `A+D > C+D`, `C+D < D-only`) y una mejora pequeña sobre `eGeMAPS` que alcanza para justificar `Fase 1` con `WavLM`, no para declarar todavía transferencia cerrada a voz.
...
| Voz Expresiva Phideus | Activo exploratorio | `Fase 0A` y `Fase 0B` ya cerradas sobre `ESD` English; señal específica de la familia `A` frente al control `C` bajo adaptación mínima por hablante, pero sin validación fuerte todavía en `N-strict`; siguiente paso = `Fase 1` con `WavLM` |
...
| `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/README.md` | Estado canónico del frente de voz: `Fase 0A` y `0B` cerradas, señal específica bajo `N-adapt` y `Fase 1` definida como siguiente prueba |
| `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/ROADMAP_VOZ_EXPRESIVA_PHIDEUS.md` | Roadmap del frente de voz: Carril A ya con `0A/0B` cerradas; siguiente pregunta viva = `WavLM` + inyección descriptorial en `N-strict` |
