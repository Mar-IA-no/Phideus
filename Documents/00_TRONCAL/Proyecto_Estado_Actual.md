...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R365 mostró que selected-action no transporta a tamaño realizable; R366 igualó oportunidad de actuar y encontró topology−permuted adverso en `37/72` celdas e inestable en `22/72`. R367 aisló el aporte de la cola: mean-only supera `mu+u_topology` en ambas cohortes en `50/72`. R368 separó ranking y elegibilidad, pero topology sólo habilitó `31/2.032` decisiones en A y `7/2.040` en B, perdió frente a mean-only en `57/72` y no superó al control permutado. La candidata más simple conserva `mu` como ranker y presupuesto/firewall como decisión agregada; la cola aprendida queda fuera del camino primario hasta acreditar valor incremental. El próximo diagnóstico CPU atribuye la señal de `mu` contra medias public-base, reduced y permutadas. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
