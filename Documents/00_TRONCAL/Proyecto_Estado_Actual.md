...
> **Atención Armónica**: el frente nuevo ya cerró `Fase 0` y `Fase 0.5`. `Fase 0` dejó un resultado dual: el **pair-state** es el salto grande (`B-minus ≫ A-rich`), y la estructura del **triangle** no domina `IID` ni `OOD-regime` frente a `B-local`, pero sí gana en `OOD-poly` bajo métricas threshold-free (`B > B-local`, `ΔAUC +0.053`, CI excluye 0). `Fase 0.5` corrigió la lectura del caveat: el problema de `B` en `OOD-poly` **no** era la transferencia de `τ` (`gap_dist≈0`), sino la fragilidad de `connected-components`; con `agglo_true_k`, `B` pasa a ser el mejor modelo en esa celda (`ARI 0.605` vs `0.482` en `B-local`). La implicancia vigente es precisa: la representación relacional de `B` generaliza mejor, pero el clusterer actual no sabe extraerla bien. El siguiente cuello ya no es calibrar `τ`, sino diseñar una lectura global de la partición.
...
| Atención Armónica | Fase 0 y 0.5 cerradas | `v2.1` pasó gate final y `Fase 0` cerró `54/54`: pair-state aporta fuerte; `triangle` gana `OOD-poly` en `AUC/AP` frente a `B-local`; `Fase 0.5` mostró que el cuello es `connected-components`, no la calibración de `τ` |
...
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/README.md` | Estado canónico local del frente: `Fase 0` y `0.5` cerradas, resultado dual pair-state/triangle y cuello actual del clusterer |
...
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/ROADMAP_ATENCION_ARMONICA.md` | Roadmap del frente incubado: `Fase 0.5` ya cerrada y foco siguiente desplazado desde `τ` hacia clusterers globales antes de CQT |
