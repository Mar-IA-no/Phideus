...
> **Escalón 2**: el frente ya no debe describirse como "factorial corriendo". `S2-P0`, `S2-P1`, `S2-P2-control` y `S2-P2-main` siguen cerrados como antes, y `S2-P2.5` ya pasó también por su lectura estadística preregistrada: `V4-lin-attnbias=70.6% @ e25`, `V4-lin-xattn=77.0% @ e15`, `H-series-xattn=73.4% @ e29`, `H-series-attnbias=78.0% @ e29`, `A4-16k-attnbias=77.8% @ e20` y `A4-16k-xattn(30ep)=78.0% @ e25` no produjeron lift defendible sobre `D0=77.8%`, aunque `V4-lin + attn_bias` sí quedó claramente por debajo y la interacción descriptor × mecanismo siguió siendo visible. `S2-P2.5b` ya también cerró sus `3/3` brazos `pca`: `H-series-pca=77.4% @ e25`, `A4-16k-pca=77.2% @ e25` y `V4-lin-pca=74.6% @ e29`. Ninguno superó a `D0`, y `V4-lin-pca` volvió a quedar claramente por debajo. El paso inmediato ya no es correr `pca`, sino reinyectar esos tres brazos en la misma lectura bootstrap contra `D0` para decidir si el null mecanístico queda suficientemente cerrado.
...
| Próximo paso | reinyectar los tres brazos `pca` en la misma lectura bootstrap contra `D0` y decidir si el null mecanístico ya quedó suficientemente cerrado |

Escalón 2 todavía no tiene claim positivo de descriptor, pero ya tampoco está en “posibilidad abstracta” ni en simple baseline. Tiene población congelada, split por speaker, segmentación canónica, auditoría de sincronía, baseline lineal, baseline neural, una fase concat cerrada, un factorial attention-based ya interpretado y una fase `pca` ya cerrada `3/3`. La lectura prudente del corte es que, bajo `attn_bias`, `xattn` y `pca`, los descriptores testeados no mejoraron retrieval sobre `D0`; la lectura operativa siguiente es integrar esos tres mecanismos en una misma lectura bootstrap antes de decidir si persiste una ambigüedad real o si el null mecanístico ya quedó suficientemente cerrado.
...
| Escalón 2 | Activo | `S2-P0` y `S2-P1` completos; `S2-P2-control` neural ya cerrado (`S=77.8% @ ep25`); `S2-P2-main` concat ya leído; `S2-P2.5` ya interpretado; `S2-P2.5b/pca` ya completo `3/3`; bootstrap final pendiente |
...
7. **Sostener Escalón 2 como frente principal ya interpretado y todavía abierto en lectura final**: `S2-P2-control` ya cerró sobre `noise0`, la fase concat ya devolvió un no parcial, `S2-P2.5` ya fue leído bajo preregistro y `S2-P2.5b/pca` ya completó sus `3/3` brazos; lo que queda es decidir si la integración bootstrap alcanza para cerrar el null mecanístico sin reruns.
...
| `Documents/01_FRENTES_ACTIVOS/ESCALON_2/README.md` | Estado canónico de Escalón 2 (`S2-P0/P1` completos, `D0` ya cerrado, concat ya evaluado, `S2-P2.5` ya interpretado y `S2-P2.5b/pca` ya completo `3/3`) |
