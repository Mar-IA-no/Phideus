...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R368 separó ranking y elegibilidad, pero el filtro topology perdió frente a mean-only en `57/72`. R369 recuperó una señal en la media: topology-mean supera topology-permuted en `37/72`, sobre todo grouped/balanceado, aunque sin intervalos compartidos resueltos. R370 mostró que ninguna de esas señales proyecta cierre con `<=500` masters —mínimo `522`, mediana `5.552`— y que el bootstrap masters×controles resuelve `0/72`. De los 37 puntos favorables, 26 se pierden tras deployment; aplicar sólo el firewall topology vuelve `31/72` celdas adversas, mientras los firewalls de control aislados conservan `44/72` favorables. La primitive recuperable parcial es la media topology, no su cola ni su selector actual. No se abre otro freeze: el siguiente diagnóstico CPU mapea el frente Pareto IID-daño/grouped-beneficio sin fijar utilidad. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
