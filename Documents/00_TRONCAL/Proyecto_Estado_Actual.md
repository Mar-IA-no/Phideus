...
> **Gate 6 AMT**: la línea downstream ya dejó atrás la fase de "42 jobs submitidos". `Exp 0` sigue fijando el baseline `Transkun`; `Exp C` local `a4r` mantiene `best_F1=0.1570 @ ep50`; y el último cierre UNC dejó el frente mucho más nítido: **`Exp B` ya puede leerse como cierre negativo** porque `20/27` tareas bastaron para mostrar que fine-tuning y `A4-degraded` convergieron a la misma banda del baseline degradado, con deltas entre `+0.0011` y `-0.0005`, mientras **`Exp A` quedó reducido a un screening mínimo** con baseline `seed=42` ya fijado en `F1=0.3186` y criterio GO/NO-GO de `+0.01` antes de reabrir la grilla completa.
...
> **Escalón 2**: el frente ya no debe describirse como "factorial corriendo". `S2-P0`, `S2-P1`, `S2-P2-control`, `S2-P2-main`, `S2-P2.5` y `S2-P2.5b` ya quedaron absorbidos en una misma lectura mecanística: `concat`, `attn_bias`, `xattn` y `pca` dieron `12/12` condiciones `≈ D0` o peores, con `V4-lin + attn_bias` y `V4-lin-pca` claramente por debajo. Ese null mecanístico inicial ya puede darse por cerrado con CI. La novedad del corte ya no es solo la decisión de `S2-P3`, sino su **apertura efectiva**: `WavLM-Large` frozen ya quedó implementado como encoder speech de referencia, la precomputación `noise0` ya existe y `P3-D0` ya está en ejecución local, todavía sin lectura de resultados. El trabajo siguiente pasa a ser completar los brazos con descriptor y luego el diagnóstico comparativo `P2 vs P3` sobre representaciones.
> **Gate 9 / revisión A10 / Gate 10**: la rama retrospectiva ya dejó de ser solo plan. `A7r` cerró con `70.4% @ e29` y `A9r` con `71.6% @ e30`. `A10a-e` ya quedaron formalmente cerrados en una banda muy estrecha: `a10ar=70.6% @ e28`, `a10br=70.0% @ e29`, `a10cr=69.2% @ e29`, `a10dr=70.2% @ e30` y `a10er` con **mejor observado `71.8% @ e27`** y cierre final `70.2% @ e30`. La lectura no cambió: bajo `reverse cross-attention` el mecanismo comprime diferencias descriptoriales; por eso **Gate 10** ya pasó de “listo para UNC” a **en curso parcial**. Al corte de notas más reciente, `8/9` arms alcanzaron `e10` y dejaron una señal provisoria nítida: `FiLM/pca` quedó arriba (`a7=70.4%`, `a10a=68.8%`, `a10d=68.6%`), `concat` en banda intermedia (`52.2-63.6%`) y `attn_bias` abajo (`44.6-49.0%` en los brazos visibles). Ningún arm llegó todavía a `30ep`, así que la lectura sigue siendo parcial.
> **Escalón 3**: ya dejó de ser solo línea conceptual. `E3-P0` ya materializó el banco canónico del frente con generador reproducible en `experiments/escalon3/generate_lissajous_dataset.py` y `6,016` scenes en `data/escalon3/scenes/`: `train/val/test=2144/448/480`, `ratio_ood=768`, `scale_ood=1024`, `equiv_ood=1152`. `train/val/test` quedó puro sobre ratios reducidos; las formas no reducidas quedaron fuera de training como `equivalence-OOD`. La tesis del frente sigue siendo separar **storage**, **retrieval** y **activation**, pero el siguiente paso real ya no es “fijar v0.1”: es correr `P1/P2` sobre este banco antes de abrir probes `phi`.
...
> **Libro HIT**: el repositorio ya tiene una consolidación teórica larga en `manifiesto_HIT_Beancon_Phideus/`. La arquitectura editorial quedó renombrada a `ARQUITECTURA_LIBRO.md`, el manuscrito absorbió el nuevo `Chapter 10` sobre el `activation problem`, y la síntesis conceptual rápida quedó reordenada al final del libro como `Appendix F`.
...
> **Decisión operativa vigente**: (1) tratar `Test02` como cierre causal del argumento de capacidad, (2) leer `13G-B` como resultado negativo/generativo genérico y usar `Test11` para sostener el hallazgo mecanístico del cuello de proyección, (3) leer `Exp B` de Gate 6 como negativo útil y dejar `Exp A` en screening mínimo antes de escalarlo, (4) leer Gate 8 ya como línea cerrada con una señal positiva real sobre proyección (`pcd > pca > pcd-zero > pcm > ctrl`), (5) tratar Escalón 2 como frente principal ya con null mecanístico inicial cerrado y `S2-P3` **implementado/en ejecución**, (6) usar Gate 9 / `A10` como evidencia retrospectiva ya disponible pero subordinada, y (7) **sostener Gate 10 hasta cierre comparable @30ep** antes de cerrar la discusión descriptor × mecanismo en la rama retrospectiva.
> **Encuadre estrategico**: Gate 5A deja de ser barrido bloqueante y queda absorbido operacionalmente por Gate 8; Gate 6 AMT conserva su rol downstream, pero ya con una poda metodológica real tras el negativo de `Exp B`; Gate 7.1 ya no es campaña pendiente sino evidencia para acotar hipótesis; Escalón 2 pasa del cierre mecanístico inicial a `S2-P3` ya implementado sobre `WavLM-Large` frozen; Gate 10 nace como mecanismo de desconfusión dentro de Escalón 1 retrospectivo; Escalón 3 pasa a ser Lissajous como banco sintético ya abierto en `E3-P0`, con convergencia potencial con Beacon pero todavía sin `P1/P2`; y Escalón 4 queda como expansión fisiológica posterior.
...
| Próximo paso | sostener `P3-D0` ya abierto con `WavLM-Large` frozen, completar los brazos con descriptor y luego hacer el diagnóstico comparativo `P2 vs P3` |

Escalón 2 todavía no tiene claim positivo de descriptor, pero ya tampoco está en “posibilidad abstracta” ni en simple baseline. Tiene población congelada, split por speaker, segmentación canónica, auditoría de sincronía, baseline lineal, baseline neural, una fase concat cerrada, un factorial attention-based ya interpretado y una fase `pca` ya cerrada `3/3`. La lectura prudente del corte es que, bajo `concat`, `attn_bias`, `xattn` y `pca`, los descriptores testeados no mejoraron retrieval sobre `D0`; por eso el frente ya abrió `S2-P3` como contraste de régimen foundation-encoder, con `WavLM-Large` frozen implementado y `P3-D0` corriendo, mientras la ambigüedad pasa de descriptor vs mecanismo a descriptor vs encoder.
...
| Gate 10 mechanism sweep | En curso parcial UNC | Piloto `3x3` (`a7`, `a10a`, `a10d` × `concat/pca/attn_bias`) ya con `8/9` arms @ `e10`; lectura provisoria `pca > concat > attn_bias`, cierre final pendiente @ `30ep` |
| Escalón 2 | Activo | `S2-P0` y `S2-P1` completos; `S2-P2-control`, `S2-P2-main`, `S2-P2.5` y `S2-P2.5b` ya integrados en un null mecanístico inicial cerrado; `S2-P3` ya implementado con `WavLM-Large` frozen y `P3-D0` en ejecución |
| Escalón 3 | Activo temprano (`E3-P0`) | Audio XY <-> figuras de Lissajous como banco sintético ya materializado (`6,016` scenes) para ratios visibles, `storage / retrieval / activation` y convergencia potencial con Beacon |
...
7. **Sostener Escalón 2 como frente principal ya reencuadrado**: `S2-P2-control`, concat, `P2.5` y `P2.5b` ya absorbieron el primer contraste mecanístico del frente; lo que sigue ya no es decidir `S2-P3`, sino dejar correr `P3-D0`, abrir los tres brazos con descriptor y consolidar luego la comparación `P2 vs P3`.
...
10. **Mantener Gate 9 como reapertura retrospectiva ya informativa y Gate 10 como contraste ya abierto**: `A7r/A9r` y `A10` ya dejaron datos suficientes para sospechar que el mecanismo domina; Gate 10 ya empezó a observar ese punto y no conviene cerrarlo antes de `30ep`.
11. **Tratar Escalón 3 como frente ya abierto en `E3-P0`**: el banco canónico ya existe y ya no corresponde seguir describiéndolo como idea o dataset futuro. El trabajo inmediato del frente pasa a ser bundling, `P1` y `P2`, dejando `phi` y la Activation Arena para el gate `E3-P4`.
...
| `Documents/01_FRENTES_ACTIVOS/ESCALON_2/README.md` | Estado canónico de Escalón 2 (`S2-P0/P1` completos, null mecanístico inicial cerrado y `S2-P3` ya implementado/en ejecución) |
...
| `Documents/01_FRENTES_ACTIVOS/ESCALON_3/README.md` | Estado canónico de Escalón 3: `E3-P0` ya materializado, `P1/P2` pendientes |
...
| `manifiesto_HIT_Beancon_Phideus/Harmonic_Information_Theory_Foundations.md` | Libro de trabajo HIT: formulación larga del programa, ya con `activation problem`, `Phideus`, `Beacon` y apéndices teóricos consolidados |
| `manifiesto_HIT_Beancon_Phideus/ARQUITECTURA_LIBRO.md` | Blueprint editorial del libro HIT, útil como mapa de capítulos, apéndices y relaciones entre probes |
...
*Documento actualizado al corte operativo 2026-03-21 (Gate 5B completamente cerrado; Gate 6 ya con `Exp B` leído como negativo útil y `Exp A` en screening pendiente; Gate 8 ya cerrado `5/5`; Escalón 2 ya con null mecanístico inicial cerrado y `S2-P3` implementado/en ejecución; Gate 9 / `A10` ya con datos retrospectivos; Gate 10 ya en curso parcial en UNC; Escalón 3 ya con `E3-P0` materializado como banco sintético Lissajous; Escalón 4 reubicado como expansión fisiológica futura; y el libro HIT ya consolidado como formulación larga del programa).*
