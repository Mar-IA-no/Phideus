...
> **Voz Expresiva Phideus**: el frente ya no está en “English cerrado, ZH pendiente”, sino un paso más adelante. `Fase 0A`, `0B` y `1 EN` ya habían dejado señal descriptorial específica, baseline `WavLM-large` fuerte y un caso positivo robusto para `concat` en `N-strict`; ahora además la réplica `ZH` ya fue corrida completa (`240/240` runs) con el manifest corregido. Eso **todavía no equivale** a una validación translingüística cerrada: falta rehacer `EN N-adapt` con `fix B2` en `1_en_calibfix/`, consolidar los reportes intra-EN, intra-ZH y cross-language, y leer el caveat de `0A ZH` donde la especificidad ratio pooled se invierte (`A/C=0.69` vs `2.88` en EN). La pregunta viva correcta del frente ya no es “si WavLM sirve” ni “si hay que correr ZH”, sino si la lectura positiva de `Fase 1` **sobrevive al cierre analítico EN↔ZH** antes de reclamar estabilidad más amplia o saltar a `MSP-Podcast`.
> **Atención Armónica**: el frente nuevo cerró su `Fase 0` sobre el pool sintético `v2.1` (`54/54` trainings). La lectura dejó de ser “training en curso” y pasó a ser un resultado dual: el **pair-state** es el salto grande (`B-minus ≫ A-rich`), y la estructura del **triangle** no mejora `IID` ni `OOD-regime` frente a `B-local`, pero sí gana en `OOD-poly` bajo métricas threshold-free (`B > B-local`, `ΔAUC +0.053`, CI excluye 0). El caveat metodológico queda igualmente fuerte: `ARI@τ_val` colapsa para `B` en `OOD-poly` pese a su mejor ranking, lo que separa representación de calibración. Ese caveat ya quedó aislado como **Fase 0.5**: post-audit de calibración antes de pasar a CQT/audio. El frente recibe un **GO acotado**, no una promoción canónica sin reservas.
...
| Voz Expresiva Phideus | Activo exploratorio | `Fase 0A`, `0B` y `1 EN` ya cerradas, `ZH` full LOSO ya corrido; `concat` sigue como caso positivo robusto en `N-strict`, pero el cierre translingüístico queda pendiente de `1_en_calibfix/` + reportes intra/cross-language |
| Atención Armónica | Fase 0 cerrada / Fase 0.5 planificada | `v2.1` pasó gate final y `Fase 0` cerró `54/54`: pair-state aporta fuerte; `triangle` gana `OOD-poly` en `AUC/AP` frente a `B-local`; `Fase 0.5` audita calibración `ARI@τ_val` antes de CQT |
...
| `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/README.md` | Estado canónico del frente de voz: `Fase 1 EN` ya cerrada, `ZH` ya corrido, y cierre translingüístico todavía pendiente de consolidación analítica |
| `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/ROADMAP_VOZ_EXPRESIVA_PHIDEUS.md` | Roadmap del frente de voz: Carril A ya con `0A/0B/1 EN` cerradas y `ZH` ya ejecutado; siguiente pregunta viva = lectura consolidada `EN ↔ ZH` con recipe alineada |
| `Documents/01_FRENTES_ACTIVOS/Voz_Expresiva_Phideus/PLAN_FASE_1_ZH.md` | Plan archivado de la réplica `ZH`, ya con nota de estado post-training y pendientes de cierre analítico |
...
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/README.md` | Estado canónico local del frente: `Fase 0` cerrada, resultado dual pair-state/triangle y caveat de calibración |
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/Explicacion_arq_RNA_codex.md` | Explicación conceptual de la arquitectura: plano token/par, `triangle update`, geometría relacional y caminos derivados |
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/ROADMAP_ATENCION_ARMONICA.md` | Roadmap del frente incubado: `Fase 0.5` como calibración previa a CQT, luego `Fase 1a/1b` |
| `Documents/01_FRENTES_ACTIVOS/Atencion_Armonica/PLAN_FASE_0_5_CALIBRACION.md` | Plan del post-audit de calibración: re-run con matrices/checkpoints, calibradores y reglas deployables |
...
*Documento actualizado al corte operativo 2026-06-28 (Gate 5B completamente cerrado con `d4a4=84.0%±2.7pp` sobre 5 training seeds; Gate 6 ya con `Exp A` y `Exp B` cerrados negativamente en la rama `Transkun+A4`, dejando `Exp C` como única línea downstream abierta; Gate 8 ya cerrado `5/5`; Escalón 2 ya con null mecanístico inicial cerrado y `S2-P3` completado en su primera pasada; el frente `Voz Expresiva Phideus` ya con `Fase 0A`, `0B` y `1 EN` cerradas y `ZH` ya corrido, pero todavía pendiente de cierre translingüístico por `1_en_calibfix/` + reportes; Atención Armónica ya cerró `Fase 0` con pair-state fuerte, `triangle` positivo en `OOD-poly` threshold-free y caveat de calibración `ARI@τ_val`; Gate 9 / `A10` ya con datos retrospectivos; Gate 10 ya completado `9/9` con lectura final `concat > pca >> attn_bias`; Escalón 3 ya con baseline dual y primera línea geométrica `P5/P6` consolidada; Escalón 4 reubicado como expansión fisiológica futura; y el libro HIT ya público y consolidado como formulación larga del programa).*
