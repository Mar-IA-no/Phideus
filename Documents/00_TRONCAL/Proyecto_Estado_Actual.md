...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó una abstención bajo restricción de no-daño IID. Policy selection usó `247` masters pareados y rechazó todos los thresholds no triviales: full, reduced y `64/64` shuffles eligieron identidad. En la adjudicación fresca de `249` masters, safe quedó idéntico al baseline, mientras el comparador unconstrained volvió a mejorar grouped y el promedio balanceado en `4/4`, con IID incierto. La auditoría posterior cruzó esa selección con dos adjudicaciones abiertas: full proyecta `368–3.504` masters bajo efecto fijo, pero ningún candidato mantiene punto IID no positivo en las tres realizaciones y los cuatro son positivos en la más reciente; grouped y balanceado conservan signo favorable en ambos transportes. El replay del diagnóstico igualó `4/4` artefactos. La abstención funcionó como firewall y la potencia no explica por sí sola el routing. El próximo diseño CPU debe probar observables de localización topológica contra un control de escala matched sobre realizaciones nuevas. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya ejecutó por CPU la abstención segura y su auditoría de potencia/transporte. El próximo corte diseña localización topológica con control de escala matched; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
