...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó una abstención bajo restricción de no-daño IID. Policy selection usó `247` masters pareados y rechazó todos los thresholds no triviales: full, reduced y `64/64` shuffles eligieron identidad. En la adjudicación fresca de `249` masters, safe quedó idéntico al baseline, mientras el comparador unconstrained volvió a mejorar grouped y el promedio balanceado en `4/4`, con IID incierto. La ventaja predicha correlacionó positivamente con beneficio grouped y negativamente con beneficio IID; un threshold monotónico no ordena ambos mecanismos por igual. Los `39.680/39.680` solves convergieron y el replay igualó `50/50` artefactos. La abstención funcionó como firewall, no como solución de routing. El próximo paso CPU audita potencia desde los crudos antes de elegir entre más masters y observables de localización topológica. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya ejecutó y reprodujo por CPU una abstención segura sobre realizaciones frescas. El próximo corte audita potencia desde estados preservados; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
