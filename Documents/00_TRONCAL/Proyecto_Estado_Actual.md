...
> **Ground truth proporcional / PPU**: una campaña de cincuenta y seis investigaciones independientes y dos carriles reconstruidos por el coordinador, organizada en veintiocho olas, no encontró una geometría universal ni un corpus único. Propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura emergente separa objeto/unidades, experimento, operador, constitución, identificabilidad, solver, reader y autoridad. Las Olas 20–25 tiparon operador, respuesta, experimento, verdad y transformación set-valued, y jurisdicción geométrica. Las Olas 26–28 avanzaron sobre tres contratos externos: `Intrinsic Object Authority Contract` para espacios métricos-medidos, cocientes y estratos; `Positive/Operator Authority Contract` para conos y razones operatoriales; y `Hierarchy/Tree Authority Contract` para ultramétricas, tree metrics y jerarquías. La Ola 26 aportó cuatro anclas checker-only ejecutadas y reproducidas byte a byte; sus otras diecisiete entradas siguen no adjudicadas. La Ola 27 cerró documentalmente veintidós bancos, todavía no implementados. La Ola 28 dio un paso programático adicional: de sus treinta y tres bancos diseñados, veinte ya fueron implementados y ejecutados con aritmética exacta. A los trece bancos iniciales, los dos falsadores sobre no-identificación p-ádica y dimensión del tight span, y las dos fronteras de proyección y resolución se añadió una cuarta suite: una presentación finita del building de Bruhat–Tits sobre `Q2`, un gauge que conserva la jerarquía ordinal mientras cambia matrices y alturas, y un preflight que rechaza datos no métricos antes de toda inferencia estructural. La suite cerró `3/3`, rechazó diecisiete mutaciones y reprodujo sus veinte fixtures desde el manifest serializado. La auditoría independiente detectó y corrigió una autoridad de certificado falsificable, una replay incompleta y una política destructiva de `RUN_ID`; la reauditoría cerró sin findings. Los trece bancos restantes siguen no adjudicados. El resultado acumulado favorece un dispatch tipado de primitives estrechas con executor/checker externos, no una mega-arquitectura. `A10` y `A11` siguen como especializaciones; `A14` sigue como interfaz; no hay `A19`, nuevo `P2*`, GO/NO-GO ni arquitectura promovida.
