...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y cuatro interfaces de riesgo. R369 recuperó una señal de media sin intervalos compartidos; R370 mostró que ninguna señal proyecta cierre con `<=500` masters y que el firewall topology elimina 26 de 37 puntos favorables. R371 representó la tensión IID/grouped como frente y R372 abrió su mecanismo: `20→40%` nunca queda dominado, pero sólo `4/8` celdas conservan la relación entre roles; acuerdo pareado global `60,1%`. R373 dejó de subdividir el histórico y materializó la interfaz tipada `BudgetPath`: `608/608` rutas reconstruidas y `8/8` mutaciones rechazadas, sin política, utilidad, score o recomendación embebidos. Esto acredita una frontera estructural, no topology ni una decisión. Sigue una prueba CPU del puerto de utilidad sobre fixtures sintéticos; el uso empírico espera utilidad declarada. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya dispone de una interfaz checker-valid para representar la ruta IID/grouped sin embebir una decisión. La ruta empírica transporta parcialmente y continúa sin acreditar elección ni valor topológico incremental robusto. Toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
