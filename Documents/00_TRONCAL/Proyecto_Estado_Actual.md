...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya llegó desde el contrato hasta un gate residual de identidad exacta. La pérdida post-IRLS corrigió el desajuste del target local, y el gate posterior usó quince observables públicos para elegir entre cinco intensidades. Mejoró grouped frente a identity entre `-0,00168` y `-0,00242` en los cuatro brazos, y superó controles shuffled matched; en IID tres intervalos cruzaron cero y `raw_typed` empeoró `+0,00116`. El alpha constante y el gate reducido eligieron siempre identidad. Los `25.240` solves convergieron y el replay igualó `17/17` artefactos. Existe señal pública parcial de competencia, pero validation IID no produce una política conjuntamente favorable. El próximo diseño CPU debe crear calibración IID/grouped y test frescos antes de ampliar capacidad. No hay arquitectura promovida ni GO/NO-GO; GPU queda en cola.
...
*Documento revisado el 2026-09-04. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a cincuenta y cinco olas y el núcleo local ya ejecutó y reprodujo por CPU la cadena hasta un gate residual público. El próximo corte diseña una realización fresca con calibración IID/grouped y test fresco; toda etapa GPU permanece en cola. Esto no constituye una PPU validada ni una decisión GO/NO-GO.*
