...
> [!IMPORTANT]
> **Actualizado**: 2026-08-31 (Olas 26–32 de ground truth proporcional integradas y auditadas en su alcance; las Olas 31–32 tienen cierre documental, sin suites ni modelos ejecutados; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de sesenta y cinco investigaciones independientes y dos carriles reconstruidos por el coordinador, organizada en treinta y tres olas, no encontró una geometría universal ni un corpus único. Propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura emergente separa objeto/unidades, experimento, operador, constitución, identificabilidad, solver, reader y autoridad. Las Olas 20–25 tiparon operador, respuesta, experimento, verdad, transformación set-valued y jurisdicción geométrica. Las Olas 26–28 avanzaron sobre tres contratos externos: `Intrinsic Object Authority Contract` para espacios métricos-medidos, cocientes y estratos; `Positive/Operator Authority Contract` para conos y razones operatoriales; y `Hierarchy/Tree Authority Contract` para ultramétricas, tree metrics y jerarquías. La Ola 26 aportó cuatro anclas checker-only ejecutadas y reproducidas byte a byte; sus otras diecisiete entradas siguen no adjudicadas. La Ola 27 cerró documentalmente veintidós bancos, todavía no implementados. La Ola 28 ejecutó veintidós suite-bank IDs en cinco suites exactas: trece contratos iniciales; dos falsadores sobre no-identificación p-ádica y dimensión del tight span; dos fronteras de proyección y resolución; tres contratos sobre building de Bruhat–Tits, gauge de presentación y preflight no métrico; y dos contratos de autoridad bajo observación parcial. El último corte demuestra que una máscara puede identificar pesos dada una topología sin identificar globalmente la topología, y que dos máscaras de igual cardinalidad pueden tener autoridad opuesta según qué relaciones observan. Cerró `2/2` baselines, `22/22` mutaciones, `8/8` guards y replay offline con resultados byte-identical; la reauditoría final no dejó findings sustantivos. Una auditoría posterior corrigió la contabilidad: esos veintidós contratos no son un subconjunto de treinta y tres IDs homogéneos, porque el denominador histórico pertenece al atlas raw R57/R58 y las suites comprimen y cruzan sus bancos. La cobertura raw canónica es `9 fully adjudicated / 20 partially covered / 4 not adjudicated`. La Ola 29 especificó los guards convencionales B-03/B-06/B-16; la Ola 30 dejó auditado, pero no implementado, el plan checker-only de B-08. La Ola 31 incorporó estadística algebraica y fibras condicionales, separando pertenencia al modelo, estadístico suficiente, conectividad, irreducibilidad, estacionariedad y mixing. La Ola 32 incorporó CRNT como geometría estratificada de realización, compatibilidad, cinética, balances, lattice estocástica, composición local, reloj e identificabilidad observacional. Sus cinco alternativas arquitectónicas quedan preservadas, pero la suite checker-only sigue sin materializar ni ejecutar. La Ola 33 no produjo convergencia independiente sobre una misma candidata: preservó termodinámica de trayectorias y gauge curvo como brechas de autoridad de dominio, y reducción reticular y `boxplus` como contratos operacionales todavía no cross-validados. Ese cierre no refuta las cuatro vías, pero tampoco selecciona una, abre una suite ni promueve una arquitectura. El resultado acumulado favorece un dispatch tipado de primitives estrechas con executor/checker externos, equivalencia y abstención explícitas, no una mega-arquitectura. B-08, la materialización CRNT y las arquitecturas derivadas son alternativas recuperables, no prioridades ni decisiones ya tomadas. `A10` y `A11` siguen como especializaciones; `A14` sigue como interfaz; no hay `A19`, nuevo `P2*`, GO/NO-GO ni arquitectura promovida.
