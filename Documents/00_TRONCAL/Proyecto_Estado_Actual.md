...
> [!IMPORTANT]
> **Actualizado**: 2026-08-30 (Ola 23 de ground truth proporcional integrada, reauditable y respaldada; conserva el corte experimental consolidado al 2026-07-02)
...
> **Ground truth proporcional / PPU**: una campaña de cuarenta y seis investigaciones independientes y dos carriles reconstruidos por el coordinador, organizados en veintitrés olas, no encontró una geometría universal ni un corpus único. Propuso cuatro fuentes de evidencia —oráculos analíticos, simulación generativa, cámaras físicas y evidencia empírica externa— más falsación adversarial y adjudicación ciega como funciones transversales. La arquitectura candidata separa objeto/unidades, experimento, operador, constitución, identificabilidad, solver, reader y autoridad. La vigésima distinguió entre representación, operador, semigrupo o generador, solver, query y autoridad; la vigesimoprimera mostró que una respuesta puede identificar un punto, un cociente, una región, un conjunto compatible o sólo `UNKNOWN`; la vigesimosegunda agregó que un mismo cociente no implica igual información y que la visibilidad pertenece a componentes posición-dirección. La vigesimotercera mostró que una respuesta set-valued exige mantener separados conjunto identificado poblacional, witnesses internos, aproximaciones exteriores, autoridad numérica e inferencia muestral. `Identified-Set Authority Stack` formaliza esa separación y reserva al checker y al ledger la autoridad que el módulo aprendido no puede conferirse a sí mismo. Los protocolos están auditados y programáticos, no ejecutados. `A10` y `A11` siguen como especializaciones; `A14` sigue como interfaz; no hay `A19`, GO/NO-GO ni arquitectura promovida.
...
*Documento revisado el 2026-08-30. El corte experimental sigue siendo el consolidado al 2026-07-02; la novedad posterior es documental y programática: veintitrés olas de ground truth proporcional quedaron integradas como base estratificada. Completación, exchange, cocientes de forma, sincronización y closures conservan sus contratos propios; representación orbital, filtración, autoridad armónica, estado predictivo y autoridad de operadores permanecen como suites diagnósticas. `Response-Quotient Atlas`, `Experiment-Relative Geometry Atlas` e `Identified-Set Authority Stack` amplían el atlas con outputs y jurisdicciones tipadas, pero siguen siendo hipótesis no promovidas. Los protocolos continúan sin ejecución y no se declaró GO/NO-GO.*
