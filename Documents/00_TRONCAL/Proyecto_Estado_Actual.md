...
> **Núcleo arquitectónico proporcional**: la cadena CPU ya probó abstención, potencia, localización topológica y tres interfaces de riesgo. R365 mostró que selected-action no transporta a tamaño realizable; R366 igualó oportunidad de actuar y encontró topology−permuted adverso en `37/72` celdas e inestable en `22/72`. R367 aisló el aporte de la cola: mean-only supera `mu+u_topology` en ambas cohortes en `50/72`, la cola gana sólo `2/72` y el promedio entre presupuestos favorece mean-only en `10/12` brazo×slice. La distribución candidata queda separada: `mu` ordena y la cola calibra riesgo/abstención sin reordenar. El próximo diseño CPU es una interfaz two-stage con ranking congelado por `mu`; no se abre otro freeze ni se entrena una cabeza nueva antes de probarla. No hay arquitectura promovida ni GO/NO-GO y la GPU sigue en cola.
