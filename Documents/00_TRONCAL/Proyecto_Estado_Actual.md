...
> [!IMPORTANT]
> **Actualizado**: 2026-09-07 (el runner nativo `MARGINAL/JOINT × HARD/CONTEXTUAL` cerró su preflight CPU sobre histórico abierto: primario y replay 14/14, unitarios 9/9, mutaciones 54/54, replay exacto y R564 `PASS 0/0/0`; el próximo paso diseña la envolvente prospectiva físicamente separada sin crear todavía el draw; sin promover arquitectura ni declarar techo; corte histórico general consolidado al 2026-07-02)
...
12. **Preparar el contraste prospectivo de la única rama habilitada**: el runner CPU `MARGINAL/JOINT × HARD/CONTEXTUAL` ya cerró sobre fixtures y artefactos abiertos. El próximo trabajo diseña y audita schemas finales, workers por fase, permisos, source freeze, escrow, receipts, recovery, replay y costo, sin crear todavía el draw fresco ni abrir el monitor; el router sólo se reconsidera si una primitive sobrevive a sus controles.
...
*Documento revisado el 2026-09-07. El corte experimental general sigue siendo el consolidado al 2026-07-02; la campaña proporcional llegó a sesenta olas y cerró su fase acumulativa. El runner set-valued habilitado por el dual native freeze ya pasó su preflight histórico y R564 lo auditó `PASS 0/0/0`; todavía no existe un draw nuevo. El modo Colab queda retirado. CPU continúa autónoma cuando es el recurso proporcionado; toda etapa para la cual GPU sea necesaria o materialmente más eficiente se detiene antes de ejecutarse, se informa por Telegram con objetivo, duración y VRAM estimados, y espera indicación del usuario. Esto no constituye una PPU validada, un techo ni una decisión GO/NO-GO.*
