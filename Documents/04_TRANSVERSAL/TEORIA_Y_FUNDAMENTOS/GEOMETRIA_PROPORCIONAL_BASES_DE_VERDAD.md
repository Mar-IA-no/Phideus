...
La Ola 55 convirtió esa deuda en una prueba de interfaz. En vez de reemplazar
siempre la acción dura por la acción bayesiana, conservó la primera como ancla y
permitió el reemplazo sólo cuando la reducción de riesgo estimada superaba un
umbral global. El selector primario eligió no reemplazar nunca: los umbrales que
reducían regret perdían demasiada exactitud y los que preservaban exactitud ya
no mejoraban regret. Una selección sobre una población más amplia volvió a
encontrar reducción de regret, pero cambió simultáneamente el signo de varios
contrastes y pagó una caída de accuracy. La interfaz fue, por tanto, sensible a
qué población autorizaba el selector.

Esto afina la geometría decisional buscada. No basta con disponer de una
distribución mejor sobre regiones compatibles ni con reducirla a una magnitud
global de ventaja. La transición hacia la acción parece depender de la posición
dentro del posterior, del desacuerdo entre readers, de la cardinalidad, de la
estabilidad entre realizaciones y de la política concreta bajo la cual se mide
el costo. La siguiente hipótesis arquitectónica es una compuerta contextual de
baja capacidad que opere sobre esas coordenadas sin apropiarse de la autoridad
del target ni de la utilidad. Que esa compuerta aprenda a discriminar overrides
útiles sería evidencia sobre la interfaz; no convertiría por sí sola el
posterior en geometría física.

La Ola 56 sometió esa hipótesis a una realización fresca. La compuerta
contextual sí recuperó una diferencia que el escalar no había podido ordenar:
redujo regret frente a advantage-only y frente a cinco asignaciones shuffled,
con intervalos orientados en la dirección esperada. Frente a la política dura
también redujo regret medio y elevó compatibilidad. El contexto, por tanto, no
fue una decoración del predictor ni una mera reexpresión de la ventaja escalar.

Esa señal no alcanzó la forma conjunta exigida. La pérdida de accuracy frente a
la acción dura excedió el margen preregistrado; la mejora frente al shuffled
quedó por debajo de la magnitud mínima; y el peor regret empeoró aun cuando el
promedio mejoraba. La compuerta produjo más overrides perjudiciales que
beneficiosos, aunque estos últimos tuvieron mayor magnitud. El problema deja de
ser la existencia de información contextual y pasa a ser su jurisdicción: una
estimación de valor medio puede proponer un cambio sin estar autorizada para
resolver qué riesgos de cola son aceptables.

Esta separación sugiere una arquitectura más precisa. El proposer puede estimar
la ganancia esperada, mientras un guard distinto modela probabilidad o cuantil
de daño y conserva la acción ancla cuando la autorización no alcanza. La prueba
de esa composición requiere otra realización fresca y controles de capacidad
igualada; no puede diseñarse sobre el monitor ya abierto. Su posible utilidad
pertenece a la interfaz de decisión y no asciende por sí sola a geometría física
o proporcional.

La Ola 57 ejecutó esa separación sobre un draw nuevo. El guard retiró daño del
proposer con la máscara de propuestas fija: mejoró accuracy y worst regret con
intervalos completamente favorables, sin pagar un aumento material de regret
medio. Pero el sistema compuesto no desplazó al hard-set bajo el patrón
congelado. La mejora de regret fue pequeña, la compatibilidad cedió y los
intervalos de regret y cola cruzaron cero. Además, sólo dos de cinco controles
de capacidad igualada alcanzaron el Hamming ponderado mínimo; el contraste
contra el promedio shuffled quedó no evaluable y el patrón terminal permaneció
indeterminado.

El resultado preserva la distinción proposer/guard porque localiza una
contribución incremental real, pero rechaza el salto desde esa contribución a
una política globalmente superior. También expone una deuda del aparato de
falsación: las permutaciones futuras deben satisfacer sus mínimos por
construcción, no por descarte posterior. Ese arreglo exige otro draw y tampoco
basta por sí solo, porque las condiciones frente al hard ya fallaron. La
pregunta siguiente es si un estimando de riesgo conjunto o de cola puede
conservar la compatibilidad y aumentar la magnitud del beneficio sin apropiarse
de la autoridad de la utilidad.
