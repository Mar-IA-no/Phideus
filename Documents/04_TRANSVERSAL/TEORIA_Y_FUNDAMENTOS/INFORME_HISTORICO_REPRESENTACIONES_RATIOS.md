...
- Las Olas 59–60 no agregan ni reclasifican un descriptor. Cambian, en cambio, el roadmap de decisión set-valued: dos guards HGB que mejoraron métricas frente al hard-set no separaron controles de igual desplazamiento en el primer draw; transportados después sin refit, recalibración ni reselección, conservaron ocho direcciones favorables frente a `hard`, pero tampoco separaron sus controles matched en el segundo draw. `MAPPING-FEASIBILITY` resolvió luego que la representación relacional, la referencia EIV y la decisión set-valued no comparten unidad, observación, target ni stack decisional. El design freeze posterior conservó las dos jurisdicciones y produjo una asimetría informativa: el surrogate relacional K192 no superó su confirmación fresca, mientras el contrato `MARGINAL/JOINT × HARD/CONTEXTUAL` quedó listo sólo para implementar su runner. No corresponde forzar un factorial común ni leer ese fallo numérico como reclasificación de un descriptor. Nada de esto reescribe el estatuto de `A4`, `D4`, `V4-lin`, `H-series` ni la taxonomía descriptorial.
...
Las Olas 59–60 vuelven concreta esa exigencia fuera de la taxonomía
descriptorial. Una
...
lección sobrevivió al transporte sin recalibración: la eficacia local frente a
`hard` reapareció, pero la atribución volvió a fallar. El gate posterior separó
los objetos y autoridades de representación y decisión y mostró que el
factorial común vigente no es interpretable. La autoridad de fases sí permanece
bien delimitada; lo que falla es la identidad semántica de unidad, observación,
target y decisión. El cambio modifica el diseño experimental, pero no reescribe
la historia de qué descriptores validaron mecánica ni cuáles siguen poniendo a
prueba la tesis de armonía natural.

El dual native freeze vuelve esa separación operativa. La rama relacional no
falló por carecer de lenguaje proporcional, sino porque el surrogate K192 no
reprodujo con la precisión predeclarada al executor canónico en una
confirmación nueva. La rama set-valued, en cambio, superó su preflight de
schemas, fases, controles y estimandos, pero todavía no produjo evidencia de
training. El relevo pasa por construir ese runner sin convertir una validez
contractual en una victoria empírica.
