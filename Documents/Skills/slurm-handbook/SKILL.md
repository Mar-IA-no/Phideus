...
### IMPORTANTE: Audio de largo variable rompe `torch.stack`
- **Contexto**: en Gate 6 aparecieron batches con slices de audio de distinto largo porque MAESTRO v3 mezcla archivos a 44.1 kHz y 48 kHz.
- **Sintoma**: `RuntimeError: stack expects each tensor to be equal size`
- **Causa**: el training loop hace `torch.stack` directo sobre audio crudo, asumiendo que todos los items del batch tienen la misma longitud.
- **Solucion**: truncar a `min_len` o paddear ANTES de `torch.stack`:
  ```python
  min_len = min(s.shape[0] for s in slices_raw)
  audio = torch.stack([s[:min_len] for s in slices_raw])
  ```
- **Regla**: si el collate no paddea, nunca asumir largo fijo en audio crudo.

### IMPORTANTE: `torch.utils.checkpoint` puede exigir gradiente en el input
- **Contexto**: algunos modelos usan checkpointing interno. Con wrappers o backbones parcialmente congelados, el backward puede fallar si ningun input entra con `requires_grad=True`.
- **Sintoma**: `element 0 of tensors does not require grad and does not have a grad_fn`
- **Solucion**: despues de mover el tensor a device, habilitar gradiente en la entrada cuando el modelo lo requiera:
  ```python
  audio = audio.to(device).requires_grad_(True)
  ```
- **Uso correcto**: aplicarlo cuando el modelo/checkpointing lo necesita; no como receta universal.
...
2. **Verificar que pasa**:
   ...
   - Si hay audio variable o modelos tipo Transkun: verificar batch real + primer backward, no solo imports y shapes

3. **Calibrar** `--time` del job real con la medicion

**Leccion practica (Gate 6 / Transkun)**: un preflight util tiene que atravesar dataloader, collate y primer backward. Los bugs de audio variable y de `torch.utils.checkpoint` no aparecen con chequeos superficiales.
