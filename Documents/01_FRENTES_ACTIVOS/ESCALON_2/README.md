...
![Status](https://img.shields.io/badge/Status-S2--P3_Completed-0A7E3B?style=for-the-badge)
![Focus](https://img.shields.io/badge/Focus-Speech↔EGG-1F6FEB?style=for-the-badge)
![Updated](https://img.shields.io/badge/Updated-2026--04--03-F59E0B?style=for-the-badge)
...
> **Corte nuevo**: `S2-P3` ya dejó de ser apertura y pasó a ser resultado. El régimen foundation-encoder con `WavLM-Large` frozen ya cerró sus `4` brazos en `noise0`: `P3-D0=78.8% @ ep15`, `P3-A4-16k-pca=78.2% @ ep25`, `P3-V4-lin-pca=76.8% @ ep28` y `P3-H-series-pca=75.6% @ ep25`. El cambio de encoder elevó levemente el baseline respecto de `P2-D0=77.8%`, pero ningún brazo descriptor-guided superó a `P3-D0`.
> **Próximo paso único**: cerrar el diagnóstico comparativo `P2 vs P3` (`CKA`, probes lineales, lectura representacional) y decidir si el frente necesita una nueva iteración de descriptores o si el null debe leerse ya como estable bajo ambos regímenes de encoder.
...
| Interpretación `P3` | `data/lombard/p3_interpretation/` | cierre canónico del régimen `P3` (`4` brazos, métricas, CIs y deltas vs `P3-D0`) |
...
- La capa activa ya no es abrir `S2-P3`, sino leer su cierre comparativo contra `P2` y decidir si el régimen foundation cambia de verdad el estatuto del null descriptorial.
...
- El siguiente contraste ya no debe preguntarse si falta “correr P3”, sino si la comparación `P2 vs P3` cambia la lectura representacional del null o confirma su estabilidad bajo dos regímenes de encoder.

## Próximos pasos

1. Consolidar la lectura comparativa `P2 vs P3` usando `data/lombard/p3_interpretation/` como cierre canónico del régimen foundation.
2. Ejecutar la comparación posthoc `P2 vs P3` (`paired delta CI`, `CKA`, probes lineales) antes de abrir nuevas variantes.
3. Decidir recién después si `V4-log`, `V4-lin+H` o ramas laterales como `A10d/A10e` merecen recursos adicionales.
4. Si no aparece una hipótesis nueva defendible, tratar el null descriptorial de Escalón 2 como estable bajo ambos regímenes de encoder ya probados.
