...
El siguiente paso ya no es una `Fase 0.5` abstracta de “calibración de `τ`”, porque esa auditoría ya se hizo y dejó una corrección metodológica más fuerte: el problema de `B` en `OOD-poly` no estaba en `τ`, sino en `connected-components` como lector de la matriz de pares. La ventaja representacional de `B` sí aparece; y `Fase 0.6` ya agregó la pieza que faltaba para decir algo más fuerte: clusterers globales deployables (`spectral` y `agglo` con `k` estimado) ya recuperan una ventaja real de `B` sobre `B-local` en `OOD-poly`. Lo que falta ahora no es un umbral mejor, sino cerrar el gap que deja la subestimación de `k`.

Por eso, antes de saltar a audio real, el paso más razonable ya no es “probar algún clusterer global” en abstracto, porque eso ya ocurrió. La decisión real es si conviene abrir un **Stage B** donde cambie no la red pairwise sino la lectura de su salida mediante una cabeza explícita de `k/partición`, o si se pasa directo a `Fase 1a`: renderizar las mezclas sintéticas, detectar picos con CQT y usar esos picos detectados como tokens. Ahí entran picos faltantes, picos corridos, picos fusionados y espurios, pero todavía con ground truth controlado. Audio real/stems queda como una fase posterior.
