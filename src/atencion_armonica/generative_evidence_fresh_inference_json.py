"""JSON-equality successor of the frozen fresh-test inference module.

Static copy: only six tuple/list boundary comparisons use canonical encoded
bytes. Scientific operations, schemas and paths are unchanged. Execution
requires the separately audited JSON amendment supervisor; the original
freeze and both failed execution chains remain immutable.
...
def _fixed_json(store, relative, value):
    ...
    if path.exists():
        ref = store.reference(path)
        if encoded(store.json(ref)) != encoded(value):
...
def _run(split, *, freeze_ref, device, check, progress, root):
    ...
    for scene_id, (observation, feature) in enumerate(zip(observations, records)):
        ...
        if reused_fit:
            ...
            if store.json(fit_ref)["origin"] != origin or encoded(fit["inventory"]) != encoded(scene["inventory"]):
...
def _verify_sources_and_choices(store, split, index, choices, frozen, check):
    ...
    for scene_id, (observation, feature, source_ref, choice_record) in enumerate(
        ...
        check()
        scene = _scene_from_sources(split, scene_id, observation, feature, matrices)
        if encoded(VerifiedBytes(ROOT).json(source_ref)) != encoded(_source_record_value(
                store, split, scene_id, scene, source["features"], source["logits"])):
            raise ValueError("fresh source scene or candidate pool differs")
        fit, fit_ref = store.load_fit(split, scene_id)
        if fit["observation"] != observation or encoded(fit["inventory"]) != encoded(scene["inventory"]):
        ...
        wanted = {"scene_id": scene_id, "identity": identity, "source": source_ref,
                  "fit": _root_ref(store.path(fit_ref["path"])), "choice": choice}
        if encoded(choice_record) != encoded(wanted):
