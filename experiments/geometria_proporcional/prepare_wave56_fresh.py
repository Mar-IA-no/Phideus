...
WAVE60_HARD_SET_RECOVERY_SOURCES = {
    "preparer": PREPARER_RELATIVE,
    "test": WAVE60_TEST_RELATIVE,
}
WAVE60_HARD_SET_UNCHANGED_SOURCES = (
    WAVE60_RUNNER_RELATIVE,
    "src/geometria_proporcional/wave60_frozen_policy_transport.py",
    "experiments/geometria_proporcional/_wave60_phase_worker.py",
    (
...
def parse_wave60_revise_audit_report(
    path: Path,
    *,
    audit_id: str,
    scope: str,
    ...
    """Validate one historical REVISE authority without upgrading its verdict."""
    text = path.read_text(encoding="utf-8")
    blocks = re.findall(r"```json\s*\n(.*?)\n```", text, flags=re.DOTALL)
    if len(blocks) != 1:
        raise RuntimeError("Wave 60 REVISE audit must contain one JSON block")
    try:
        authority = json.loads(blocks[0])
    except json.JSONDecodeError as exc:
        raise RuntimeError("Wave 60 REVISE audit JSON is invalid") from exc
    if "schema_version" in authority:
        expected = {
        ...
        expected = {
            ...
            "Target": target,
            "Verdict": "REVISE",
            "Findings": {key.upper(): value for key, value in findings.items()},
        }
    if authority != expected:
        raise RuntimeError("Wave 60 historical REVISE authority drifted")


def validate_wave60_bound_document(
    repo_root: Path,
    ...
    *,
    ...
    _require_keys(binding, {"commit", "path", "sha256"}, label)
    path, _ = require_repo_artifact(repo_root, binding["path"], binding["sha256"])
    introduction = git_introduction_commit(repo_root, binding["path"])
    if introduction != binding["commit"]:
        raise RuntimeError(f"{label} introduction commit drifted")
    if git_changed_paths(repo_root, introduction) != {binding["path"]}:
        raise RuntimeError(f"{label} commit contains unrelated paths")
    if git_blob_sha256(repo_root, introduction, binding["path"]) != binding["sha256"]:
        raise RuntimeError(f"{label} blob differs from its binding")
    require_direct_parent(repo_root, introduction, expected_parent, label)
    return path
...
def validate_wave60_audit_commit(
    ...
    *,
    ...
    authority = parse_wave60_audit_report(path, scope=scope, target=target)
    if (
        expected_audit_id is not None
        and authority["audit_id"] != expected_audit_id
    ):
...
def validate_wave60_invalid_preparation_amendment_audit(
    ...
    """Authenticate the R494 audit of the invalid-preparation amendment."""
...
def validate_wave60_invalid_preparation_implementation_suffix(
    ...
    *,
    ...
    r481_resolution_implementation: dict[str, Any],
    r483_audit: dict[str, Any],
    r483_resolution_plan: dict[str, Any],
    r484_audit: dict[str, Any],
    r483_resolution_implementation: dict[str, Any],
    r485_audit: dict[str, Any],
    r485_resolution_plan: dict[str, Any],
    r486_audit: dict[str, Any],
    r486_resolution_plan: dict[str, Any],
    ...
    r489_resolution_implementation: dict[str, Any],
    r491_audit: dict[str, Any],
    r491_resolution_plan: dict[str, Any],
    r492_audit: dict[str, Any],
    ...
    _require_keys(
        ...
        {"commit", "parent", "changed_sources"},
        "Wave 60 R481 resolution implementation",
    )
    if (
        r481_resolution_implementation["parent"] != r482_audit["commit"]
        or set(r481_resolution_implementation["changed_sources"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES)
        or git_changed_paths(repo_root, r481_resolution_implementation["commit"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.values())
    ):
        raise RuntimeError("Wave 60 R481 resolution implementation drifted")
    require_direct_parent(
        repo_root,
        r481_resolution_implementation["commit"],
        r482_audit["commit"],
        "Wave 60 R481 resolution implementation",
    )
    for label, relative in WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.items():
        expected = {
            ...
            "new_sha256": git_blob_sha256(
                repo_root, r481_resolution_implementation["commit"], relative
            ),
        }
        if r481_resolution_implementation["changed_sources"].get(label) != expected:
            raise RuntimeError(
                f"Wave 60 R481 resolution {label} source binding drifted"
            )
    ...
    _require_keys(
        ...
        "Wave 60 R483 implementation audit",
    )
    if (
        r483_audit["audit_id"] != "R483"
        or r483_audit["scope"]
        != "INVALID_PREPARATION_RECOVERY_IMPLEMENTATION"
        or r483_audit["verdict"] != "REVISE"
        or r483_audit["findings"] != {"high": 0, "medium": 1, "low": 0}
    ):
        raise RuntimeError("Wave 60 R483 REVISE authority drifted")
    r483_path = validate_wave60_bound_document(
        repo_root,
        {key: r483_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R483 implementation audit",
        expected_parent=r481_resolution_implementation["commit"],
    )
    parse_wave60_revise_audit_report(
        r483_path,
        audit_id="R483",
        scope="INVALID_PREPARATION_RECOVERY_IMPLEMENTATION",
        target={
            "implementation_commit": r481_resolution_implementation["commit"]
        },
        findings={"high": 0, "medium": 1, "low": 0},
    )
    ...
    validate_wave60_bound_document(
        ...
        label="Wave 60 R483 resolution plan",
        expected_parent=r483_audit["commit"],
    )
    _require_keys(
        r484_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R484 resolution audit",
    )
    if (
        r484_audit["audit_id"] != "R484"
        or r484_audit["verdict"] != "PASS"
        or r484_audit["findings"] != {"high": 0, "medium": 0, "low": 0}
    ):
        raise RuntimeError("Wave 60 R484 PASS authority drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": r484_audit["commit"],
            "audit_path": r484_audit["path"],
            "audit_sha256": r484_audit["sha256"],
        },
        scope="INVALID_PREPARATION_RECOVERY_R483_RESOLUTION_PLAN",
        target={
            "plan_commit": r483_resolution_plan["commit"],
            "plan_sha256": r483_resolution_plan["sha256"],
        },
        expected_parent=r483_resolution_plan["commit"],
    ...
    _require_keys(
        ...
        {"commit", "parent", "changed_sources"},
        "Wave 60 R483 resolution implementation",
    )
    if (
        r483_resolution_implementation["parent"] != r484_audit["commit"]
        or set(r483_resolution_implementation["changed_sources"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES)
        or git_changed_paths(repo_root, r483_resolution_implementation["commit"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.values())
    ):
        raise RuntimeError("Wave 60 R483 resolution implementation drifted")
    require_direct_parent(
        repo_root,
        r483_resolution_implementation["commit"],
        r484_audit["commit"],
        "Wave 60 R483 resolution implementation",
    )
    for label, relative in WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.items():
        expected = {
            ...
            "new_sha256": git_blob_sha256(
                repo_root, r483_resolution_implementation["commit"], relative
            ),
        }
        if r483_resolution_implementation["changed_sources"].get(label) != expected:
            raise RuntimeError(
                f"Wave 60 R483 resolution {label} source binding drifted"
            )
    ...
    _require_keys(
        ...
        "Wave 60 R485 implementation audit",
    )
    if (
        r485_audit["audit_id"] != "R485"
        or r485_audit["scope"]
        != "INVALID_PREPARATION_RECOVERY_IMPLEMENTATION"
        or r485_audit["verdict"] != "REVISE"
        or r485_audit["findings"] != {"high": 0, "medium": 1, "low": 0}
    ):
        raise RuntimeError("Wave 60 R485 REVISE authority drifted")
    r485_path = validate_wave60_bound_document(
        repo_root,
        {key: r485_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R485 implementation audit",
        expected_parent=r483_resolution_implementation["commit"],
    )
    parse_wave60_revise_audit_report(
        r485_path,
        audit_id="R485",
        scope="INVALID_PREPARATION_RECOVERY_IMPLEMENTATION",
        target={
            "implementation_commit": r483_resolution_implementation["commit"]
        },
        findings={"high": 0, "medium": 1, "low": 0},
    )
    ...
    validate_wave60_bound_document(
        ...
        label="Wave 60 R485 resolution plan",
        expected_parent=r485_audit["commit"],
    )
    _require_keys(
        r486_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R486 resolution audit",
    )
    if (
        r486_audit["audit_id"] != "R486"
        or r486_audit["verdict"] != "REVISE"
        or r486_audit["findings"] != {"high": 0, "medium": 2, "low": 0}
    ):
        raise RuntimeError("Wave 60 R486 REVISE authority drifted")
    r486_path = validate_wave60_bound_document(
        repo_root,
        {key: r486_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R486 resolution audit",
        expected_parent=r485_resolution_plan["commit"],
    )
    parse_wave60_revise_audit_report(
        r486_path,
        audit_id="R486",
        ...
        target={
            "plan_commit": r485_resolution_plan["commit"],
            "plan_sha256": r485_resolution_plan["sha256"],
        },
    ...
    validate_wave60_bound_document(
        ...
        label="Wave 60 R486 resolution plan",
        expected_parent=r486_audit["commit"],
    )
    _require_keys(
        r487_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R487 resolution audit",
    )
    if (
        r487_audit["audit_id"] != "R487"
        or r487_audit["verdict"] != "REVISE"
        or r487_audit["findings"] != {"high": 0, "medium": 1, "low": 0}
    ):
        raise RuntimeError("Wave 60 R487 REVISE authority drifted")
    r487_path = validate_wave60_bound_document(
        repo_root,
        {key: r487_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R487 resolution audit",
        expected_parent=r486_resolution_plan["commit"],
    )
    parse_wave60_revise_audit_report(
        r487_path,
        audit_id="R487",
        ...
        target={
            "plan_commit": r486_resolution_plan["commit"],
            "plan_sha256": r486_resolution_plan["sha256"],
        },
        findings={"high": 0, "medium": 1, "low": 0},
    )
    ...
    validate_wave60_bound_document(
        ...
        label="Wave 60 R487 resolution plan",
        expected_parent=r487_audit["commit"],
    )
    _require_keys(
        r488_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R488 resolution audit",
    )
    if (
        r488_audit["audit_id"] != "R488"
        or r488_audit["verdict"] != "PASS"
        or r488_audit["findings"] != {"high": 0, "medium": 0, "low": 0}
    ):
        raise RuntimeError("Wave 60 R488 PASS authority drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": r488_audit["commit"],
            "audit_path": r488_audit["path"],
            "audit_sha256": r488_audit["sha256"],
        },
        scope="INVALID_PREPARATION_RECOVERY_R487_RESOLUTION_PLAN",
        target={
            "plan_commit": r487_resolution_plan["commit"],
            "plan_sha256": r487_resolution_plan["sha256"],
        },
        expected_parent=r487_resolution_plan["commit"],
        expected_audit_id="R488",
    )
    ...
    _require_keys(
        ...
        {"commit", "parent", "changed_sources"},
        "Wave 60 R489 resolution implementation",
    )
    if (
        r489_resolution_implementation["parent"] != r490_audit["commit"]
        or set(r489_resolution_implementation["changed_sources"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES)
        or git_changed_paths(repo_root, r489_resolution_implementation["commit"])
        != set(WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.values())
    ):
        raise RuntimeError("Wave 60 R489 resolution implementation drifted")
    require_direct_parent(
        repo_root,
        r489_resolution_implementation["commit"],
        r490_audit["commit"],
        "Wave 60 R489 resolution implementation",
    )
    for label, relative in WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.items():
        expected = {
            ...
            "new_sha256": git_blob_sha256(
                repo_root, r489_resolution_implementation["commit"], relative
            ),
        }
        if r489_resolution_implementation["changed_sources"].get(label) != expected:
            raise RuntimeError(
                f"Wave 60 R489 resolution {label} source binding drifted"
            )
    ...
    _require_keys(
        ...
        "Wave 60 R491 implementation audit",
    )
    if (
        r491_audit["audit_id"] != "R491"
        or r491_audit["scope"]
        != "INVALID_PREPARATION_RECOVERY_IMPLEMENTATION"
        or r491_audit["verdict"] != "REVISE"
        or r491_audit["findings"] != {"high": 0, "medium": 1, "low": 0}
    ):
        raise RuntimeError("Wave 60 R491 REVISE authority drifted")
    r491_path = validate_wave60_bound_document(
        repo_root,
        {key: r491_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R491 implementation audit",
        expected_parent=r489_resolution_implementation["commit"],
    )
    parse_wave60_revise_audit_report(
        r491_path,
        audit_id="R491",
        scope="INVALID_PREPARATION_RECOVERY_IMPLEMENTATION",
        target={
            "implementation_commit": r489_resolution_implementation["commit"]
        },
        findings={"high": 0, "medium": 1, "low": 0},
    )
    ...
    validate_wave60_bound_document(
        ...
        label="Wave 60 R491 resolution plan",
        expected_parent=r491_audit["commit"],
    )
    _require_keys(
        r492_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R492 resolution audit",
    )
    if (
        r492_audit["audit_id"] != "R492"
        or r492_audit["verdict"] != "PASS"
        or r492_audit["findings"] != {"high": 0, "medium": 0, "low": 0}
    ):
        raise RuntimeError("Wave 60 R492 PASS authority drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": r492_audit["commit"],
            "audit_path": r492_audit["path"],
            "audit_sha256": r492_audit["sha256"],
        },
        scope="INVALID_PREPARATION_RECOVERY_R491_RESOLUTION_PLAN",
        target={
            "plan_commit": r491_resolution_plan["commit"],
            "plan_sha256": r491_resolution_plan["sha256"],
        },
        expected_parent=r491_resolution_plan["commit"],
        expected_audit_id="R492",
    )
    ...
    if (
        final_implementation["audit_id"] != "R493"
    ...
    for relative in WAVE60_SOURCE_LAW_SOURCES:
        if git_blob_sha256(
            repo_root, final_implementation["commit"], relative
        ) != git_blob_sha256(repo_root, r475_implementation_commit, relative):
            raise RuntimeError(
                f"Wave 60 accepted recovery crossed scientific source: {relative}"
            )
    validate_wave60_audit_commit(
        repo_root,
        final_implementation,
...
def validate_wave60_final_config_authority(
    ...
    if recovery is not None:
        ...
        if amendment_schema in {
            ...
            if is_static_protocol_recovery or is_hard_set_recovery:
...
def require_canonical_repo_file(repo_root: Path, relative: str, label: str) -> Path:
    """Resolve one literal regular file without traversal or symlink aliases."""
    repository = repo_root.resolve(strict=True)
    candidate = Path(relative)
    if (
        candidate.is_absolute()
    ...
    metadata = resolved.lstat()
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise RuntimeError(f"{label} is not one physical file")
    return resolved


def validate_wave60_hard_set_request_alias(
    request: dict[str, Any], contract: dict[str, Any]
) -> None:
    source_paths = request.get("source_paths")
    source_hashes = request.get("source_sha256")
    alias = contract["request_alias"]
    if not isinstance(source_paths, dict) or not isinstance(source_hashes, dict):
        raise RuntimeError("Wave 60 hard-set request source maps are absent")
    if (
        source_paths.get(alias) != contract["source_path"]
        or source_hashes.get(alias) != contract["source_sha256"]
        or sum(value == contract["source_path"] for value in source_paths.values())
        != 1
    ):
        raise RuntimeError("Wave 60 hard-set request alias binding drifted")


def validate_wave60_invalid_preparation_hard_set_contract(
    amendment: dict[str, Any],
    config: dict[str, Any],
    *,
    repo_root: Path = REPO_ROOT,
) -> float:
    """Authenticate manifest -> request -> alias -> snapshot before materializing."""
    contract = amendment.get("hard_set_contract")
    expected = {
    ...
    if contract != expected:
        raise RuntimeError("Wave 60 hard-set contract drifted")
    authority = require_canonical_repo_directory(
        repo_root,
        contract["source_authority_path"],
        "Wave 60 hard-set source authority",
    )
    if config.get("source_law_authority", {}).get("path") != contract[
        "source_authority_path"
    ] or config["source_law_authority"].get(
        "source_authority_manifest_sha256"
    ) != contract[
        "source_authority_manifest_sha256"
    ]:
        raise RuntimeError("Wave 60 hard-set authority is not config-bound")
    manifest_path = authority / "source_authority_manifest.json"
    if sha256_file(manifest_path) != contract["source_authority_manifest_sha256"]:
        raise RuntimeError("Wave 60 hard-set authority manifest drifted")

    import run_wave60_frozen_policy_transport as wave60_transport_runner

    if wave60_transport_runner.REPO_ROOT.resolve(strict=True) != repo_root.resolve(
        strict=True
    ):
        raise RuntimeError("Wave 60 source-law validator repository root drifted")
    wave60_transport_runner.validate_source_authority(
        authority,
        config["source_law_authority"],
        config,
    )

    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    request_relative = contract["source_law_request_relative"]
    request_path = authority / request_relative
    request_metadata = request_path.lstat()
    request_record = {
        "bytes": request_metadata.st_size,
        "group": request_metadata.st_gid,
        "mode": f"{stat.S_IMODE(request_metadata.st_mode):04o}",
        "owner": request_metadata.st_uid,
        "sha256": sha256_file(request_path),
    }
    if (
        stat.S_ISLNK(request_metadata.st_mode)
        or not stat.S_ISREG(request_metadata.st_mode)
        or request_record["sha256"] != contract["source_law_request_sha256"]
        or manifest.get("files", {}).get(request_relative) != request_record
    ):
        raise RuntimeError("Wave 60 hard-set request is not manifest-bound")
    request = json.loads(request_path.read_text(encoding="utf-8"))
    validate_wave60_hard_set_request_alias(request, contract)

    source_path = require_canonical_repo_file(
        repo_root, contract["source_path"], "Wave 60 hard-set snapshot"
    )
    if sha256_file(source_path) != contract["source_sha256"]:
        raise RuntimeError("Wave 60 hard-set snapshot hash drifted")
    snapshot = json.loads(source_path.read_text(encoding="utf-8"))
    tau = snapshot.get("hard_set_tau")
    if (
        isinstance(tau, bool)
        or not isinstance(tau, (int, float))
        or not np.isfinite(float(tau))
        or float(tau) != 0.5
    ):
        raise RuntimeError("Wave 60 hard-set snapshot value drifted")
    return float(tau)
...
def has_wave60_authorized_nested_invalid_preparation_escrow(
    source: Path,
    config: dict[str, Any],
    amendment_arg: Path | None,
    *,
    repo_root: Path,
) -> bool:
    """Narrow preflight allowance; full signature/inventory checks follow."""
    if amendment_arg is None:
        return False
    ...
    try:
        ...
        if amendment_arg.resolve(strict=True) != canonical:
            return False
    ...
    return (
        ...
        and amendment.get("prior_attempt_container")
        == recovery["prior_attempt_container"]
        and source == prior_primary
        and has_escrow(source / "failed_preparation")
    )
...
def validate_invocation(
    ...
    *,
    ...
    if args.recovery_secrets_from:
        ...
        if not has_escrow(source) and not has_wave60_authorized_nested_invalid_preparation_escrow(
            source,
            config,
            amendment_arg,
            repo_root=repo_root,
    ...
    if amendment_schema in {
        ...
        source_partition = (
            ...
            == WAVE60_INVALID_PREPARATION_RECOVERY_AMENDMENT_SCHEMA
                ...
                else WAVE60_HARD_SET_RECOVERY_SOURCES
            )
        )
        ...
        for label, relative in source_partition.items():
            ...
            if implementation.get("changed_sources", {}).get(label) != expected:
                raise RuntimeError(
                    f"Wave 60 {label} recovery delta differs from amendment"
                )
        head = _git_output(repo_root, "rev-parse", "HEAD")
        if execution_contract.get("git_commit") != head:
            raise RuntimeError("Wave 60 execution contract is not bound to current HEAD")
    elif changed:
...
def _validate_wave60_invalid_preparation_recovery_origin(
    amendment: dict[str, Any],
    config: dict[str, Any],
    *,
    repo_root: Path,
    ...
    """Authenticate the terminal v1 root and its nested pre-truth draw."""
    prior = require_canonical_repo_directory(
        repo_root,
        amendment["prior_attempt_container"],
        "Wave 60 invalid-preparation prior attempt",
    )
    pair = prior / "pair"
    primary = prior / "primary"
    replay = prior / "replay"
    ...
    pair_failure = json.loads(pair_failure_path.read_text(encoding="utf-8"))
    if (
        pair_status.get("terminal") != "PAIR_ABORTED_PRE_TRUTH"
        or pair_status.get("primary_terminal") != "INVALID_PREPARATION"
        or pair_status.get("replay_terminal") != "INVALID_PREPARATION"
        or pair_status.get("any_truth_accessed") is not False
        or pair_status.get("recovery_allowed") is not True
        or sha256_file(pair_failure_path) != amendment["prior_pair_failure_sha256"]
        or pair_failure.get("schema_version") != "wave60-pair-failure-v1"
        or pair_failure.get("terminal") != "PAIR_ABORTED_PRE_TRUTH"
        or pair_failure.get("truth_accessed") is not False
        or pair_failure.get("recovery_allowed") is not True
        or pair_failure.get("authority_binding_sha256")
        != sha256_file(pair / "pair_status.json")
    ):
        raise RuntimeError(
            "Wave 60 invalid-preparation origin is not an authorized pre-truth abort"
        )
    for role, root in (("primary", primary), ("replay", replay)):
        failure = json.loads((root / "FAILURE.json").read_text(encoding="utf-8"))
        inventory = json.loads(
            (root / "failure_inventory.json").read_text(encoding="utf-8")
        )
        if (
            failure.get("run_role") != role
            or failure.get("terminal") != "INVALID_PREPARATION"
            or failure.get("truth_accessed") is not False
            or failure.get("recovery_allowed") is not True
            or inventory.get("terminal") != "INVALID_PREPARATION"
            or inventory.get("last_complete_phase") != "INITIALIZED"
        ):
            raise RuntimeError(f"Wave 60 {role} invalid-preparation state drifted")

    observed = physical_tree_inventory(primary)
    if observed != amendment["origin_inventory"]:
        raise RuntimeError("Wave 60 invalid-preparation origin inventory drifted")
    ...
    for path in sorted(primary.rglob("*")):
        ...
        if stat.S_ISREG(metadata.st_mode):
            ...
            if metadata.st_nlink != 1 or inode in seen_inodes:
                raise RuntimeError("Wave 60 invalid-preparation origin has a hardlink")
            seen_inodes.add(inode)

    draw = primary / "failed_preparation"
    draw_metadata = draw.lstat()
    if (
        stat.S_ISLNK(draw_metadata.st_mode)
        or not stat.S_ISDIR(draw_metadata.st_mode)
        or stat.S_IMODE(draw_metadata.st_mode) != 0o700
        or draw_metadata.st_uid != 0
        or draw_metadata.st_gid != 0
    ):
        raise PermissionError(
            "Wave 60 invalid-preparation draw must be root-owned mode 0700"
        )
    required_modes = {
        ESCROW_NAME: "0600",
        FREEZE_NAME: "0644",
        "benchmark/manifest.json": "0600",
    }
    records = _inventory_records_by_path(physical_tree_inventory(draw))
    for relative, expected_mode in required_modes.items():
        record = records.get(relative)
        if (
            record is None
            ...
            raise PermissionError(
                f"Wave 60 invalid-preparation draw metadata drifted: {relative}"
            )
    preserved = amendment["preserved_draw_sha256"]
    for relative, expected in preserved.items():
        record = records.get(relative)
        if record is None or record.get("sha256") != expected:
            raise RuntimeError(f"Wave 60 preserved nested draw drifted: {relative}")
    escrow = read_escrow(draw)
    if (
        records[ESCROW_NAME]["sha256"]
        != amendment["escrow_origin"]["escrow_sha256"]
        or records[FREEZE_NAME]["sha256"]
        != amendment["escrow_origin"]["pre_generation_freeze_sha256"]
        or records["benchmark/manifest.json"]["sha256"]
        != amendment["escrow_origin"]["benchmark_manifest_sha256"]
        ...
        raise RuntimeError("Wave 60 invalid-preparation escrow origin drifted")
    benchmark_manifest = json.loads(
        (draw / "benchmark/manifest.json").read_text(encoding="utf-8")
    )
    expected_draw = {
        ESCROW_NAME,
        FREEZE_NAME,
    ...
    if (
        ...
        raise RuntimeError("Wave 60 invalid-preparation primary binding drifted")
    return primary, draw, observed


def _validate_wave60_invalid_preparation_recovery_amendment(
    amendment_path: Path,
    source: Path,
    execution_contract: dict[str, Any],
    mode: str,
    *,
    ...
    if mode not in {"recovery", "replay"}:
        raise RuntimeError(
            "Wave 60 invalid-preparation amendment is recovery/replay-only"
        )
    config = execution_contract["prospective_config"]
    recovery = config["attempt"]["recovery"]
    canonical = (repo_root / recovery["amendment_path"]).resolve(strict=True)
    if amendment_path.resolve(strict=True) != canonical:
        raise RuntimeError("Wave 60 invalid-preparation amendment path drifted")
    amendment_sha256 = sha256_file(canonical)
    if amendment_sha256 != recovery["amendment_sha256"]:
        raise RuntimeError("Wave 60 invalid-preparation amendment hash drifted")
    amendment = json.loads(canonical.read_text(encoding="utf-8"))
    if canonical_json_sha256(amendment) != amendment_sha256:
        raise RuntimeError("Wave 60 invalid-preparation amendment is not canonical JSON")
    _require_keys(
        amendment,
        {
            "schema_version",
        ...
        "Wave 60 invalid-preparation recovery amendment",
    )
    expected_prior_audit = {
        "commit": "2a10b6cb5a88fd2af4ca2f5f8230a296f59c8948",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "477_wave60_frozen_policy_transport_config_audit.md"
        ),
        "sha256": (
            "2c0c8bd9defe0fbc4901b5bc5130e2d27de1a5a47fd61331e2eff40682f5556d"
        ),
    }
    if (
        ...
        or amendment["recovery_kind"] != "INVALID_PREPARATION"
        or amendment["prior_attempt_container"]
        != recovery["prior_attempt_container"]
        or amendment["prior_pair_failure_sha256"]
        != recovery["prior_pair_failure_sha256"]
        or amendment["prior_config_audit"] != expected_prior_audit
        ...
        raise RuntimeError("Wave 60 invalid-preparation amendment authority drifted")
    _require_keys(
        ...
        "Wave 60 invalid-preparation escrow origin",
    )
    if amendment["unledgered_preparation_debit"] != {
        "seconds": 60.0,
        ...
        raise RuntimeError("Wave 60 invalid-preparation debit drifted")
    expected_population = {
        "rows": 4992,
        "total_unique_pair_tokens": 1152,
        "eligible_unique_pair_tokens": 768,
    ...
    if amendment["population_contract"] != {
        ...
        raise RuntimeError("Wave 60 invalid-preparation population contract drifted")
    if not isinstance(amendment["origin_inventory"], list):
        raise RuntimeError("Wave 60 invalid-preparation inventory is absent")

    rejected_plan = amendment["rejected_plan"]
    if rejected_plan != {
        ...
        raise RuntimeError("Wave 60 rejected recovery plan binding drifted")
    validate_wave60_bound_document(
        repo_root,
        rejected_plan,
        label="Wave 60 rejected recovery plan",
        expected_parent=expected_prior_audit["commit"],
    )

    rejected_audit = amendment["rejected_plan_audit"]
    _require_keys(
        rejected_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 rejected recovery plan audit",
    )
    if (
        rejected_audit["commit"]
        != "979c835bc2b2f08182e23e919f265f4fe2bc480a"
        or rejected_audit["path"]
        != (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "478_wave60_invalid_preparation_recovery_plan_audit.md"
        )
        or rejected_audit["sha256"]
        != "ef4be8f02f2bf40f005c63c9cf61e5932ddd52bb36fb343aab15f457dceb1eb8"
        or rejected_audit["audit_id"] != "R478"
        or rejected_audit["verdict"] != "REVISE"
        or rejected_audit["findings"] != {"high": 0, "medium": 2, "low": 0}
    ):
        raise RuntimeError("Wave 60 R478 audit binding drifted")
    rejected_audit_path = validate_wave60_bound_document(
        repo_root,
        {key: rejected_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R478 audit",
        expected_parent=rejected_plan["commit"],
    )
    parse_wave60_revise_audit_report(
        rejected_audit_path,
        audit_id="R478",
        scope="RECOVERY_PLAN",
        target={
            "plan_commit": rejected_plan["commit"],
            "plan_sha256": rejected_plan["sha256"],
        },
        findings=rejected_audit["findings"],
    )

    r478_plan = amendment["r478_resolution_plan"]
    if r478_plan != {
        ...
        raise RuntimeError("Wave 60 R478 resolution binding drifted")
    validate_wave60_bound_document(
        repo_root,
        r478_plan,
        label="Wave 60 R478 resolution plan",
        expected_parent=rejected_audit["commit"],
    )
    r478_audit = amendment["r478_resolution_plan_audit"]
    _require_keys(
        r478_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R479 audit",
    ...
    if (
        r478_audit["commit"]
        != "3f20c79db2311cfefebc90dd049b4862d2a04a11"
        or r478_audit["path"]
        != (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "479_wave60_invalid_preparation_recovery_r478_resolution_plan_audit.md"
        )
        or r478_audit["sha256"]
        != "4089ef7cd4812714fa731efae45719fc1685dafb767e1e92c26574abdddb9fa4"
        or r478_audit["audit_id"] != "R479"
        or r478_audit["verdict"] != "REVISE"
        or r478_audit["findings"] != {"high": 0, "medium": 1, "low": 0}
    ):
        raise RuntimeError("Wave 60 R479 audit binding drifted")
    r478_audit_path = validate_wave60_bound_document(
        repo_root,
        {key: r478_audit[key] for key in ("commit", "path", "sha256")},
        label="Wave 60 R479 audit",
    ...
    parse_wave60_revise_audit_report(
        ...
        findings=r478_audit["findings"],
    )

    r479_plan = amendment["r479_resolution_plan"]
    if r479_plan != {
        ...
        raise RuntimeError("Wave 60 R479 resolution binding drifted")
    validate_wave60_bound_document(
        repo_root,
        r479_plan,
        label="Wave 60 R479 resolution plan",
        expected_parent=r478_audit["commit"],
    )
    r479_audit = amendment["r479_resolution_plan_audit"]
    _require_keys(
        r479_audit,
        {"commit", "path", "sha256", "audit_id", "verdict", "findings"},
        "Wave 60 R480 audit",
    ...
    if (
        r479_audit["commit"]
        != "abb8fd2e8c9119e46fabee2aa15405ceb146d4b2"
        or r479_audit["path"]
        != (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "480_wave60_invalid_preparation_recovery_r479_resolution_plan_audit.md"
        )
        or r479_audit["sha256"]
        != "d4860501a98acfe71659f49b0fbeb89c1c8fec7d3fcdd1fa4444664c7e6aaad3"
        or r479_audit["audit_id"] != "R480"
        or r479_audit["verdict"] != "PASS"
        or r479_audit["findings"] != {"high": 0, "medium": 0, "low": 0}
    ):
        raise RuntimeError("Wave 60 R480 audit binding drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": r479_audit["commit"],
            "audit_path": r479_audit["path"],
            "audit_sha256": r479_audit["sha256"],
        },
    ...
    r481_resolution_implementation = amendment[
        "r481_resolution_implementation"
    ]
    if r481_resolution_implementation != {
        ...
        "changed_sources": {
            "preparer": {
                ...
                "new_sha256": (
                    "fd3a3809fc98c825cf1c3159b6af8602db50c59a529d7a32b12f2148f687cfb5"
                ),
            },
            "test": {
                "path": WAVE60_TEST_RELATIVE,
                ...
                "new_sha256": (
                    "d8e5c78554f5eb5ae1be5472f42ba15761e7034f305591a8f2c7931f63f7ce62"
                ),
            },
        },
    }:
        raise RuntimeError("Wave 60 rejected R483 implementation binding drifted")
    r481_resolution_implementation_audit = amendment[
        "r481_resolution_implementation_audit"
    ]
    if r481_resolution_implementation_audit != {
        "commit": "b944cac9fbcbb3f28e2d7753161c58c978868226",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 rejected R483 audit binding drifted")
    r483_plan = amendment["r483_resolution_plan"]
    if r483_plan != {
        ...
        "sha256": (
            "0c22469829176132efb9f039ab261f8cc93821f440117f54669f20d3623bd352"
        ),
    }:
        raise RuntimeError("Wave 60 R483 resolution plan binding drifted")
    r483_plan_audit = amendment["r483_resolution_plan_audit"]
    if r483_plan_audit != {
        "commit": "f26404044cd87cc14deea22cb0d14fa54b6134ae",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "484_wave60_invalid_preparation_recovery_r483_resolution_plan_audit.md"
        ),
        "sha256": (
            "2b8453e1ce541a7c6ce24d9ecc9b85d6b19c8bf870775ebf55e18cc6f9f09b21"
        ),
        "audit_id": "R484",
        "verdict": "PASS",
        "findings": {"high": 0, "medium": 0, "low": 0},
    }:
        raise RuntimeError("Wave 60 R484 resolution audit binding drifted")

    r483_resolution_implementation = amendment[
        "r483_resolution_implementation"
    ]
    if r483_resolution_implementation != {
        "commit": "f32ba2bb5a6f38b4a3e9e9afbafb9393430f6719",
        "parent": "f26404044cd87cc14deea22cb0d14fa54b6134ae",
        "changed_sources": {
            "preparer": {
                "path": PREPARER_RELATIVE,
                "old_sha256": (
                ...
                "new_sha256": (
                    "b21d89af10021904347563997b3ec1e13292558cfc30519bd2c5f4d8cc8d7f32"
                ),
            },
            "test": {
                "path": WAVE60_TEST_RELATIVE,
                ...
                "new_sha256": (
                    "979054582ca1a61e6834f19d65e8d39954b431e1482f2c72a14af830c0363fdd"
                ),
            },
        },
    }:
        raise RuntimeError("Wave 60 rejected R485 implementation binding drifted")
    r483_resolution_implementation_audit = amendment[
        "r483_resolution_implementation_audit"
    ]
    if r483_resolution_implementation_audit != {
        "commit": "b038cf91fffe66689b103e1b3a28503b6975d645",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 rejected R485 audit binding drifted")
    r485_plan = amendment["r485_resolution_plan"]
    if r485_plan != {
        "commit": "0c07b10abf7831dc6577c39637f3b68c2a3a02b2",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/waves/"
        ...
        raise RuntimeError("Wave 60 R485 resolution plan binding drifted")
    r485_plan_audit = amendment["r485_resolution_plan_audit"]
    if r485_plan_audit != {
        "commit": "2049eff3b411024e6b4fd444f2b975ae76c27f3e",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 R486 resolution audit binding drifted")
    r486_plan = amendment["r486_resolution_plan"]
    if r486_plan != {
        ...
        raise RuntimeError("Wave 60 R486 resolution plan binding drifted")
    r486_plan_audit = amendment["r486_resolution_plan_audit"]
    if r486_plan_audit != {
        "commit": "50e5bd3cbfeb32491b71c24ee2b52c9b2a6325fb",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 R487 resolution audit binding drifted")
    r487_plan = amendment["r487_resolution_plan"]
    if r487_plan != {
        "commit": "126ee2ca160d2bb3301a146b9850709c7aa78d1f",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/waves/"
            "WAVE_60_INVALID_PREPARATION_RECOVERY_R487_RESOLUTION_PLAN.md"
        ),
        "sha256": (
            "4c5a08398a8893ba7e49340cdb6606e2ea3b3f4d8a3785088cf0ffc6fba85449"
        ),
    }:
        raise RuntimeError("Wave 60 R487 resolution plan binding drifted")
    r487_plan_audit = amendment["r487_resolution_plan_audit"]
    if r487_plan_audit != {
        "commit": "bb6a80d4aae74683f0a4b3513cd5d44bb9fa5c6b",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
    ...
    r489_resolution_implementation = amendment[
        "r489_resolution_implementation"
    ]
    if r489_resolution_implementation != {
        "commit": "4e50425bc8c33ed668da6de21dcd2a22882268fa",
        "parent": "9503a697efe6c9c766fcae6fee4b4d7982ba6221",
        "changed_sources": {
            "preparer": {
                "path": PREPARER_RELATIVE,
                "old_sha256": (
                ...
                "new_sha256": (
                    "83525c3edd0e5b77584d1341a9b65d0d4e45996ad3b9a831f7fc09274df109b4"
                ),
            },
            "test": {
                "path": WAVE60_TEST_RELATIVE,
                ...
                "new_sha256": (
                    "11d05738e96537a0774a0a90514a615cb3c426e06c2d341fbc6585ae759de11a"
                ),
            },
        },
    }:
        raise RuntimeError("Wave 60 rejected R491 implementation binding drifted")
    r489_resolution_implementation_audit = amendment[
        "r489_resolution_implementation_audit"
    ]
    if r489_resolution_implementation_audit != {
        "commit": "413ffdb8980d8128214898797eb01f4dd8af564d",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 rejected R491 audit binding drifted")
    r491_plan = amendment["r491_resolution_plan"]
    if r491_plan != {
        "commit": "e7af03ac590e90098ff94adb09cd0181f5a083e0",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/waves/"
        ...
        raise RuntimeError("Wave 60 R491 resolution plan binding drifted")
    r491_plan_audit = amendment["r491_resolution_plan_audit"]
    if r491_plan_audit != {
        "commit": "9eeb7b0a54ad1bd73ab0df0ee768ef7c24433deb",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
        ...
        raise RuntimeError("Wave 60 R492 resolution audit binding drifted")
    ...
    validate_wave60_invalid_preparation_implementation_suffix(
        ...
        r481_resolution_implementation=r481_resolution_implementation,
        r483_audit=r481_resolution_implementation_audit,
        r483_resolution_plan=r483_plan,
        r484_audit=r483_plan_audit,
        r483_resolution_implementation=r483_resolution_implementation,
        r485_audit=r483_resolution_implementation_audit,
        r485_resolution_plan=r485_plan,
        r486_audit=r485_plan_audit,
        r486_resolution_plan=r486_plan,
        ...
        r491_audit=r489_resolution_implementation_audit,
        r491_resolution_plan=r491_plan,
        r492_audit=r491_plan_audit,
        final_implementation=implementation,
    )
    for label, relative in WAVE60_RECOVERY_IMPLEMENTATION_SOURCES.items():
        expected = {
            "path": relative,
            "old_sha256": git_blob_sha256(repo_root, r475_commit, relative),
            "new_sha256": git_blob_sha256(
                repo_root, implementation["commit"], relative
            ),
        }
        if implementation["changed_sources"].get(label) != expected:
            raise RuntimeError(f"Wave 60 recovery {label} source binding drifted")
        if (
            config["source_sha256"].get(relative) != expected["new_sha256"]
            or sha256_file(repo_root / relative) != expected["new_sha256"]
        ):
            raise RuntimeError(f"Wave 60 recovery {label} physical blob drifted")
    for relative in WAVE60_SOURCE_LAW_SOURCES:
        expected = git_blob_sha256(repo_root, r475_commit, relative)
        if (
            git_blob_sha256(repo_root, implementation["commit"], relative) != expected
    ...
    if (
        ...
        != {recovery["amendment_path"]}
        or git_blob_sha256(repo_root, amendment_commit, recovery["amendment_path"])
        != amendment_sha256
    ):
        raise RuntimeError("Wave 60 invalid-preparation amendment commit drifted")
    require_direct_parent(
        repo_root,
        amendment_commit,
        implementation["audit_commit"],
        "Wave 60 invalid-preparation amendment",
    )
    ...
    validate_wave60_invalid_preparation_hard_set_contract(
        amendment, config, repo_root=repo_root
    )
    failed, draw, observed = _validate_wave60_invalid_preparation_recovery_origin(
        amendment,
        config,
        repo_root=repo_root,
    ...
    if source.resolve(strict=True) != expected_source:
        raise RuntimeError("Wave 60 invalid-preparation source is not canonical")
    if mode == "replay":
        source_freeze = json.loads(
            (source / "preparation_freeze.json").read_text(encoding="utf-8")
        )
        provenance = source_freeze.get("recovery_provenance", {})
        if (
            provenance.get("amendment_sha256") != amendment_sha256
            or provenance.get("contract_extensions")
            != {
    ...
    if len(config_relatives) != 1:
        raise RuntimeError("Wave 60 recovery cannot identify its config source")
    config_relative = config_relatives[0]
    prior_config_commit = _git_output(
        repo_root, "rev-parse", f"{expected_prior_audit['commit']}^"
    )
    prior_config_bytes = subprocess.check_output(
        ["git", "show", f"{prior_config_commit}:{config_relative}"], cwd=repo_root
    )
    prior_config_sha256 = sha256_bytes(prior_config_bytes)
    prior_config = json.loads(prior_config_bytes)
    validate_prospective_config(prior_config)
    if (
        git_changed_paths(repo_root, prior_config_commit) != {config_relative}
        or prior_config["attempt"]["container"]
        != amendment["prior_attempt_container"]
        or prior_config["attempt"]["recovery"] is not None
        or prior_config["final_audit"]["audit_path"]
        != expected_prior_audit["path"]
    ):
        raise RuntimeError("Wave 60 prior config authority drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": expected_prior_audit["commit"],
            "audit_path": expected_prior_audit["path"],
            "audit_sha256": expected_prior_audit["sha256"],
        },
        scope="CONFIG",
        target={
    ...
    if (
        ...
        or json.loads(failed_config_path.read_text(encoding="utf-8")) != prior_config
    ):
        raise RuntimeError("Wave 60 invalid-preparation root config drifted")
    origin_contract = read_escrow(draw)["contract"]
    if origin_contract.get("prospective_config") != prior_config:
        raise RuntimeError("Wave 60 invalid-preparation escrow config drifted")
    _validate_wave60_contract_delta(
        origin_contract, execution_contract, amendment, repo_root
    )
    return {
        ...
        "reuse_source": draw if mode == "recovery" else source.resolve(strict=True),
        "repo_root": repo_root,
    }
...
def _validate_wave60_hard_set_plan_chain(
    repo_root: Path, amendment: dict[str, Any]
) -> None:
    """Authenticate the corrected v4 plan and its independent R501 PASS."""
    plan = {
        "commit": "a9bb0cfd1062da5076f8cbdd8c2d64978270dce7",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/waves/"
            "WAVE_60_HARD_SET_AUTHORITY_RECOVERY_PLAN.md"
        ),
        "sha256": (
            "6a4e95e4373f5fc5fa1c40e31e9efb090483c986b69af4d48c9738918fc5472c"
        ),
    }
    if amendment.get("plan") != plan:
        raise RuntimeError("Wave 60 hard-set recovery plan binding drifted")
    plan_path, _ = require_repo_artifact(repo_root, plan["path"], plan["sha256"])
    if (
        git_changed_paths(repo_root, plan["commit"]) != {plan["path"]}
        or git_blob_sha256(repo_root, plan["commit"], plan["path"])
        != plan["sha256"]
        or sha256_file(plan_path) != plan["sha256"]
    ):
        raise RuntimeError("Wave 60 hard-set recovery plan blob drifted")
    require_direct_parent(
        repo_root,
        plan["commit"],
        "b53349a764840bb49a8a9c1de5b445dade87f44f",
        "Wave 60 corrected hard-set recovery plan",
    )
    plan_audit = {
        "commit": "2fe5f1f83dc266e8671ca5cc8dd1580ceef6b7a1",
        "path": (
            "Biblioteca/Geometria_Proporcional_Ground_Truth/agent_reports/"
            "501_wave60_hard_set_authority_recovery_plan_audit.md"
        ),
        "sha256": (
            "efa2f2897ad0717303b91c35bd126a026b6076e8de67632d1b1a03f167fc3134"
        ),
        "audit_id": "R501",
    }
    if amendment.get("plan_audit") != plan_audit:
        raise RuntimeError("Wave 60 R501 plan audit binding drifted")
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": plan_audit["commit"],
            "audit_path": plan_audit["path"],
            "audit_sha256": plan_audit["sha256"],
        },
        ...
        expected_parent=plan["commit"],
        expected_audit_id="R501",
    )


def validate_wave60_hard_set_recovery_authority(
    repo_root: Path,
    amendment: dict[str, Any],
    config: dict[str, Any],
    prior_config: dict[str, Any],
    ...
    if (
        ...
        or set(required) != set(config.get("source_sha256", {}))
        or set(required) != set(prior_config.get("source_sha256", {}))
        or len(required) != 8
    ):
        raise RuntimeError("Wave 60 hard-set eight-source roster drifted")
    _validate_wave60_hard_set_plan_chain(repo_root, amendment)
    implementation = amendment.get("recovery_implementation")
    if not isinstance(implementation, dict):
    ...
    if (
        ...
        != list(WAVE60_HARD_SET_UNCHANGED_SOURCES)
        or set(implementation["changed_sources"])
        != set(WAVE60_HARD_SET_RECOVERY_SOURCES)
        or git_changed_paths(repo_root, implementation["commit"])
    ...
    for label, relative in WAVE60_HARD_SET_RECOVERY_SOURCES.items():
        ...
        if implementation["changed_sources"].get(label) != expected:
            raise RuntimeError(
                f"Wave 60 hard-set {label} source binding drifted"
            )
        if (
            config["source_sha256"].get(relative) != expected["new_sha256"]
            or sha256_file(repo_root / relative) != expected["new_sha256"]
            ...
            raise RuntimeError(
                f"Wave 60 hard-set {label} physical source drifted"
            )
    for relative in WAVE60_HARD_SET_UNCHANGED_SOURCES:
        expected = prior_config["source_sha256"][relative]
        if (
            git_blob_sha256(repo_root, implementation["commit"], relative)
            != expected
            ...
            raise RuntimeError(
                f"Wave 60 hard-set invariant source crossed: {relative}"
            )
    validate_wave60_audit_commit(
        repo_root,
        implementation,
...
def _validate_wave60_hard_set_recovery_origin(
    ...
    *,
    ...
    """Authenticate the sealed v3 terminal and its nested failed preparation."""
    prior = require_canonical_repo_directory(
        repo_root,
        amendment["prior_attempt_container"],
        "Wave 60 hard-set prior attempt",
    )
    if amendment["prior_attempt_container"] != (
        "data/geometria_proporcional/"
        "wave60_frozen_policy_transport_attempt_v3"
    ):
        raise RuntimeError("Wave 60 hard-set prior namespace drifted")
    pair = prior / "pair"
    primary = prior / "primary"
    replay = prior / "replay"
    from run_wave60_frozen_policy_transport import validate_pair_failure_package

    status = validate_pair_failure_package(
        prior, public_key=trusted_public_key_path
    )
    if status != json.loads((pair / "pair_status.json").read_text(encoding="utf-8")):
        raise RuntimeError("Wave 60 hard-set pair status parser drifted")
    if (
        status.get("terminal") != "PAIR_ABORTED_PRE_TRUTH"
        or status.get("primary_terminal") != "INVALID_PREPARATION"
        or status.get("replay_terminal") != "INVALID_PREPARATION"
        or status.get("any_truth_accessed") is not False
        or status.get("recovery_allowed") is not True
    ):
        raise RuntimeError("Wave 60 hard-set origin is not recoverable")
    ...
    terminal_relatives = (
        ...
        "primary/failure_inventory.json",
        "primary/failure_attestation.json",
        "primary/config.snapshot.json",
        "primary/source_bindings.json",
        "replay/FAILURE.json",
        "replay/failure_inventory.json",
        "replay/failure_attestation.json",
        "replay/config.snapshot.json",
        "replay/source_bindings.json",
    )
    observed_terminal = {
        relative: sha256_file(prior / relative) for relative in terminal_relatives
    }
    if (
        amendment.get("prior_terminal") != observed_terminal
        or amendment.get("prior_pair_failure_sha256")
        != observed_terminal["pair/FAILURE.json"]
    ):
        raise RuntimeError("Wave 60 hard-set prior terminal binding drifted")

    observed_inventory = {
        "primary": physical_tree_inventory(primary),
        "replay": physical_tree_inventory(replay),
    }
    if amendment.get("origin_inventory") != observed_inventory:
        raise RuntimeError("Wave 60 hard-set origin inventory drifted")
    for root in (primary, replay):
        ...
        for path in sorted(root.rglob("*")):
            ...
            if stat.S_ISREG(metadata.st_mode):
                inode = (metadata.st_dev, metadata.st_ino)
                if metadata.st_nlink != 1 or inode in seen_inodes:
                    raise RuntimeError("Wave 60 hard-set origin has a hardlink")
                seen_inodes.add(inode)

    prior_config_sha256 = canonical_json_sha256(prior_config)
    for role, root in (("primary", primary), ("replay", replay)):
        failure = json.loads((root / "FAILURE.json").read_text(encoding="utf-8"))
        inventory = json.loads(
            (root / "failure_inventory.json").read_text(encoding="utf-8")
        ...
        if (
            failure.get("run_role") != role
            or failure.get("terminal") != "INVALID_PREPARATION"
            or failure.get("phase") != "prepare"
            or failure.get("truth_accessed") is not False
            or failure.get("recovery_allowed") is not True
            or failure.get("authority_binding_sha256") != prior_config_sha256
            or inventory.get("terminal") != "INVALID_PREPARATION"
            or inventory.get("last_complete_phase") != "INITIALIZED"
            or inventory.get("missing_expected") != []
            or inventory.get("forbidden_present") != []
        ):
            raise RuntimeError(f"Wave 60 hard-set {role} terminal drifted")
        snapshot = root / "config.snapshot.json"
        if (
            sha256_file(snapshot) != prior_config_sha256
            or json.loads(snapshot.read_text(encoding="utf-8")) != prior_config
            or json.loads(
                (root / "source_bindings.json").read_text(encoding="utf-8")
            )
            != prior_config["source_binding"]
        ):
            raise RuntimeError(f"Wave 60 hard-set {role} frozen config drifted")
    ...
    forbidden_names = {
        ...
        WAVE59_PREPARATION_ATTESTATION_NAME,
    }
    forbidden = [
        path
        for root in (primary, replay)
    ...
    if forbidden:
        raise RuntimeError("Wave 60 hard-set origin contains forbidden preparation state")
    ...
    if (
        stat.S_ISLNK(draw_metadata.st_mode)
        or not stat.S_ISDIR(draw_metadata.st_mode)
        or (draw_metadata.st_uid, draw_metadata.st_gid, stat.S_IMODE(draw_metadata.st_mode))
        != (0, 0, 0o700)
    ):
        raise PermissionError("Wave 60 hard-set nested draw metadata drifted")
    draw_records = _inventory_records_by_path(physical_tree_inventory(draw))
    preserved = amendment["preserved_draw_sha256"]
    for relative, expected in preserved.items():
        record = draw_records.get(relative)
        if record is None or record.get("sha256") != expected:
            raise RuntimeError(f"Wave 60 hard-set preserved draw drifted: {relative}")
    escrow = read_escrow(draw)
    if (
        preserved != prior_config["attempt"]["recovery"]["preserved_draw_sha256"]
        or draw_records[ESCROW_NAME]["sha256"]
        != amendment["escrow_origin"]["escrow_sha256"]
        or draw_records[FREEZE_NAME]["sha256"]
        != amendment["escrow_origin"]["pre_generation_freeze_sha256"]
        or draw_records["benchmark/manifest.json"]["sha256"]
        != amendment["escrow_origin"]["benchmark_manifest_sha256"]
        or compact_json_sha256(escrow["contract"])
        != amendment["escrow_origin"]["contract_sha256"]
    ):
        raise RuntimeError("Wave 60 hard-set escrow origin drifted")
    benchmark_manifest = json.loads(
        (draw / "benchmark/manifest.json").read_text(encoding="utf-8")
    )
    expected_preserved = {
    ...
    if set(preserved) != expected_preserved:
        raise RuntimeError("Wave 60 hard-set preserved draw map is not closed-world")
    expected_auxiliary = {
        "generation_receipt.json",
        "preparation_error.json",
    ...
    observed_files = {
        relative
        for relative, record in draw_records.items()
        if record["type"] == "file"
    }
    if observed_files != expected_preserved | expected_auxiliary:
        raise RuntimeError("Wave 60 hard-set nested draw is not closed-world")

    preparation_error = json.loads(
        (draw / "preparation_error.json").read_text(encoding="utf-8")
    )
    if preparation_error != {
        "schema_version": "wave60-preparation-failure-v1",
        "error_type": "KeyError",
        "error_message_sha256": hashlib.sha256(
            str(KeyError("hard_set_tau")).encode("utf-8")
        ).hexdigest(),
    }:
        raise RuntimeError("Wave 60 hard-set preparation error drifted")

    access = json.loads(
        (draw / "inference/access_receipt.json").read_text(encoding="utf-8")
    )
    if (
        access.get("phase") != "wave56-visible-inference-before-any-oracle"
        or access.get("fit_operations") is not False
        or access.get("oracle_or_labels_available") is not False
        or access.get("effective_uid") != 65534
        or access.get("effective_gid") != 65534
        or access.get("sealed_truth_probe")
        != {
            "attempted": True,
            "denied_with": "PermissionError",
            "passed": True,
        }
        or access.get("allowed_splits") != list(SPLITS)
    ):
        raise RuntimeError("Wave 60 hard-set blind access receipt drifted")
    expected_outputs = {
        (f"logits/seed{seed}__{split}.npz", split)
        for split in SPLITS
        for seed in (17, 29, 43)
    }
    observed_outputs: set[tuple[str, str]] = set()
    for record in access.get("outputs", []):
        relative = record.get("path")
        if not isinstance(relative, str):
            raise RuntimeError("Wave 60 hard-set access output path drifted")
        split = Path(relative).stem.split("__")[-1]
        output_path = draw / "inference" / relative
        if (
            record.get("rows") != 4992
            or record.get("sha256") != sha256_file(output_path)
        ):
            raise RuntimeError("Wave 60 hard-set inference output drifted")
        observed_outputs.add((relative, split))
    if observed_outputs != expected_outputs:
        raise RuntimeError("Wave 60 hard-set inference output roster drifted")

    prior_recovery = amendment["prior_recovery_authority"]
    generation = json.loads(
        (draw / "generation_receipt.json").read_text(encoding="utf-8")
    )
    generation_provenance = generation.get("recovery_provenance", {})
    if (
        generation.get("oracle_materialized") is not False
        or generation.get("execution_mode") != "recovery"
        or generation_provenance.get("schema_version")
        != WAVE60_STATIC_PROTOCOL_RECOVERY_AMENDMENT_SCHEMA
        or generation_provenance.get("amendment_sha256")
        != prior_recovery["amendment_sha256"]
        or generation_provenance.get("contract_extensions")
        != {
        ...
        or access.get("stage_hashes", {}).get("wave56_config.json")
        != prior_config_sha256
        or sha256_file(draw / "recovery_amendment.json")
        != prior_recovery["amendment_sha256"]
    ):
        raise RuntimeError("Wave 60 hard-set v3 recovery provenance drifted")
    return primary, draw, observed_inventory


def validate_wave60_hard_set_prior_v2_budget(
    repo_root: Path,
    amendment: dict[str, Any],
    *,
    trusted_public_key_path: Path = PUBLIC_KEY,
) -> float:
    """Revalidate the signed v2 ledger and apply the sole conservative v3 debit."""
    if amendment.get("prior_v2_attempt_container") != (
        "data/geometria_proporcional/"
        "wave60_frozen_policy_transport_attempt_v2"
    ):
        raise RuntimeError("Wave 60 hard-set v2 namespace drifted")
    binding = {
        "prior_attempt_container": amendment["prior_v2_attempt_container"],
        "prior_pair_failure_sha256": amendment["prior_v2_terminal"][
            "pair/FAILURE.json"
        ],
        "prior_terminal": amendment["prior_v2_terminal"],
        "prior_durable_budget": amendment["prior_v2_durable_budget"],
    }
    ...
    debit = amendment.get("unledgered_preparation_debit")
    if debit != {
        "seconds": 60.0,
        "regime": "CONSERVATIVE_UNSIGNED_PREPARATION_DEBIT",
        "observed_failed_prepare_wall_seconds": 36.47,
        "observed_terminal_seal_wall_seconds": 2.29,
        "observed_external_record_authority": "TRANSCRIPT_ONLY_NOT_SIGNED_LEDGER",
        "applied_once": True,
    }:
        raise RuntimeError("Wave 60 hard-set unsigned debit contract drifted")
    prior_recovery = amendment.get("prior_recovery_authority", {})
    if (
        prior_recovery.get("prior_attempt_container")
        != amendment["prior_v2_attempt_container"]
        or prior_recovery.get("prior_pair_failure_sha256")
        != amendment["prior_v2_terminal"]["pair/FAILURE.json"]
    ):
        raise RuntimeError("Wave 60 hard-set v2 lineage drifted")
    return float(amendment["prior_v2_durable_budget"]["durable_seconds"]) + 60.0


def _validate_wave60_hard_set_authority_recovery_amendment(
    amendment_path: Path,
    source: Path,
    execution_contract: dict[str, Any],
    mode: str,
    *,
    ...
    """Validate the v4 recovery from the sealed v3 materializer failure."""
    if mode not in {"recovery", "replay"}:
        raise RuntimeError("Wave 60 hard-set amendment is recovery/replay-only")
    config = execution_contract["prospective_config"]
    recovery = config["attempt"]["recovery"]
    canonical = (repo_root / recovery["amendment_path"]).resolve(strict=True)
    if amendment_path.resolve(strict=True) != canonical:
        raise RuntimeError("Wave 60 hard-set amendment path drifted")
    amendment_sha256 = sha256_file(canonical)
    if amendment_sha256 != recovery["amendment_sha256"]:
        raise RuntimeError("Wave 60 hard-set amendment hash drifted")
    amendment = json.loads(canonical.read_text(encoding="utf-8"))
    if canonical_json_sha256(amendment) != amendment_sha256:
    ...
    if (
        ...
        raise RuntimeError("Wave 60 hard-set amendment authority drifted")
    _require_keys(
        amendment["escrow_origin"],
        {
            "contract_sha256",
        ...
        "Wave 60 hard-set escrow origin",
    )
    expected_population = {
        "rows": 4992,
        "total_unique_pair_tokens": 1152,
    ...
    if amendment["population_contract"] != {
        ...
        raise RuntimeError("Wave 60 hard-set population contract drifted")
    ...
    if len(config_relatives) != 1:
        raise RuntimeError("Wave 60 hard-set cannot identify its config source")
    config_relative = config_relatives[0]
    prior_config_commit = _git_output(
    ...
    if git_changed_paths(repo_root, prior_config_commit) != {config_relative}:
        raise RuntimeError("Wave 60 hard-set prior config commit is not exclusive")
    prior_config_bytes = subprocess.check_output(
        ["git", "show", f"{prior_config_commit}:{config_relative}"], cwd=repo_root
    )
    prior_config_sha256 = sha256_bytes(prior_config_bytes)
    try:
        ...
        raise RuntimeError("Wave 60 hard-set prior config is not JSON") from exc
    validate_prospective_config(prior_config)
    if (
        prior_config["attempt"]["version"] != 3
        or prior_config["attempt"]["container"]
        != amendment["prior_attempt_container"]
        or prior_config["final_audit"]
        != {
            "audit_id": "R500",
            "audit_path": recovery["prior_config_audit_path"],
        }
        or amendment["prior_recovery_authority"]
        != prior_config["attempt"]["recovery"]
    ):
        raise RuntimeError("Wave 60 hard-set prior config authority drifted")
    require_direct_parent(
        repo_root,
        prior_config_commit,
        prior_config["attempt"]["recovery"]["amendment_audit_commit"],
        "Wave 60 hard-set prior config",
    )
    validate_wave60_audit_commit(
        repo_root,
        {
        ...
        expected_parent=prior_config_commit,
        expected_audit_id="R500",
    )

    prior_recovery = amendment["prior_recovery_authority"]
    prior_amendment_path, _ = require_repo_artifact(
        repo_root,
        prior_recovery["amendment_path"],
        prior_recovery["amendment_sha256"],
    )
    prior_amendment = json.loads(prior_amendment_path.read_text(encoding="utf-8"))
    if (
        canonical_json_sha256(prior_amendment)
        != prior_recovery["amendment_sha256"]
        or prior_amendment.get("schema_version")
        != WAVE60_STATIC_PROTOCOL_RECOVERY_AMENDMENT_SCHEMA
        or prior_amendment.get("recovery_kind")
        != "STATIC_PROTOCOL_IDENTITY_GUARD"
    ):
        raise RuntimeError("Wave 60 hard-set prior amendment drifted")
    prior_amendment_commit = git_introduction_commit(
        repo_root, prior_recovery["amendment_path"]
    )
    if (
        git_changed_paths(repo_root, prior_amendment_commit)
        != {prior_recovery["amendment_path"]}
        or git_blob_sha256(
            repo_root, prior_amendment_commit, prior_recovery["amendment_path"]
        )
        != prior_recovery["amendment_sha256"]
    ):
        raise RuntimeError("Wave 60 hard-set prior amendment Git blob drifted")
    prior_implementation = prior_amendment.get("recovery_implementation")
    if not isinstance(prior_implementation, dict):
        raise RuntimeError("Wave 60 hard-set prior implementation is absent")
    require_direct_parent(
        repo_root,
        prior_amendment_commit,
        prior_implementation["audit_commit"],
        "Wave 60 hard-set prior amendment",
    )
    validate_wave60_audit_commit(
        repo_root,
        prior_implementation,
        scope="STATIC_PROTOCOL_IDENTITY_GUARD_RECOVERY_IMPLEMENTATION",
        target={"implementation_commit": prior_implementation["commit"]},
        expected_parent=prior_implementation["commit"],
        expected_audit_id="R498",
    )
    validate_wave60_audit_commit(
        repo_root,
        {
            "audit_commit": prior_recovery["amendment_audit_commit"],
            "audit_path": prior_recovery["amendment_audit_path"],
            "audit_sha256": prior_recovery["amendment_audit_sha256"],
        },
        scope="STATIC_PROTOCOL_IDENTITY_GUARD_RECOVERY_AMENDMENT",
        target={"amendment_sha256": prior_recovery["amendment_sha256"]},
        expected_parent=prior_amendment_commit,
        expected_audit_id="R499",
    )

    implementation = validate_wave60_hard_set_recovery_authority(
        repo_root, amendment, config, prior_config
    )
    validate_wave60_invalid_preparation_hard_set_contract(
        amendment, config, repo_root=repo_root
    )
    failed, draw, observed = _validate_wave60_hard_set_recovery_origin(
        amendment,
        prior_config,
        repo_root=repo_root,
        trusted_public_key_path=trusted_public_key_path,
    )
    expected_prior = validate_wave60_hard_set_prior_v2_budget(
        repo_root,
        amendment,
        trusted_public_key_path=trusted_public_key_path,
    )
    if expected_prior != 275.3670025616884:
        raise RuntimeError("Wave 60 hard-set cumulative prior budget drifted")
    ...
    if (
        ...
        raise RuntimeError("Wave 60 hard-set amendment commit drifted")
    require_direct_parent(
        repo_root,
        amendment_commit,
        implementation["audit_commit"],
        "Wave 60 hard-set amendment",
    )
    validate_wave60_audit_commit(
        repo_root,
        {
    ...
    if source.resolve(strict=True) != expected_source:
        raise RuntimeError("Wave 60 hard-set recovery source is not canonical")
    expected_extensions = {
    ...
    if mode == "replay":
        ...
        if (
            provenance.get("amendment_sha256") != amendment_sha256
            or provenance.get("contract_extensions") != expected_extensions
        ):
            raise RuntimeError("Wave 60 hard-set replay provenance drifted")
...
def _validate_wave60_recovery_amendment(
    ...
    *,
    ...
    if (
        ...
        return _validate_wave60_invalid_preparation_recovery_amendment(
            amendment_path,
            source,
            execution_contract,
            mode,
...
def revalidate_authorized_recovery_origin(
    ...
    if schema == WAVE60_INVALID_PREPARATION_RECOVERY_AMENDMENT_SCHEMA:
        failed, _, inventory = (
            _validate_wave60_invalid_preparation_recovery_origin(
                amendment,
    ...
    if schema == WAVE60_HARD_SET_AUTHORITY_RECOVERY_AMENDMENT_SCHEMA:
        ...
        config_relative = next(
            relative
            for relative in execution_contract["prospective_config"][
        ...
        failed, _, inventory = _validate_wave60_hard_set_recovery_origin(
            amendment,
            prior_config,
            repo_root=context["repo_root"],
            trusted_public_key_path=trusted_public_key_path,
        )
        validate_wave60_hard_set_prior_v2_budget(
            context["repo_root"],
            amendment,
            trusted_public_key_path=trusted_public_key_path,
        )
...
def resolve_wave60_materializer_config(
    config: dict[str, Any],
    recovery_context: dict[str, Any] | None,
) -> dict[str, Any]:
    """Resolve materializer-only authority before any Wave 60 output mutation."""
    schema = config.get("schema_version")
    if schema != WAVE60_CONFIG_SCHEMA:
        return config
    if recovery_context is None:
        raise RuntimeError(
            "Wave 60 materializer hard-set authority is absent before preparation"
        )
    ...
    repo_root = recovery_context.get("repo_root")
    if not isinstance(repo_root, Path):
        raise RuntimeError("Wave 60 materializer repository authority is absent")
    tau = validate_wave60_invalid_preparation_hard_set_contract(
        amendment,
        config,
        repo_root=repo_root,
    )
    materializer_config = deepcopy(config)
    if "hard_set_tau" in materializer_config:
        raise RuntimeError("Wave 60 canonical config unexpectedly embeds hard_set_tau")
    materializer_config["hard_set_tau"] = tau
...
def _wave60_invalid_preparation_unsigned_debit(
    args: argparse.Namespace,
    config: dict[str, Any],
    source: Path,
) -> float:
    """Authorize the sole conservative debit for the sealed v1 failed prepare."""
    recovery = config.get("attempt", {}).get("recovery")
    amendment_arg = getattr(args, "recovery_amendment", None)
    if (
        not isinstance(recovery, dict)
        or int(config["attempt"]["version"]) != 2
        or recovery.get("prior_attempt_container")
        != (
            "data/geometria_proporcional/"
            "wave60_frozen_policy_transport_attempt_v1"
        )
        or amendment_arg is None
    ):
        raise RuntimeError("Wave 60 unsigned preparation debit is not authorized")
    canonical = (REPO_ROOT / recovery["amendment_path"]).resolve(strict=True)
    if (
        amendment_arg.resolve(strict=True) != canonical
        or sha256_file(canonical) != recovery["amendment_sha256"]
    ):
        raise RuntimeError("Wave 60 unsigned preparation debit amendment drifted")
    amendment = json.loads(canonical.read_text(encoding="utf-8"))
    if (
        amendment.get("schema_version")
        != WAVE60_INVALID_PREPARATION_RECOVERY_AMENDMENT_SCHEMA
        or amendment.get("recovery_kind") != "INVALID_PREPARATION"
        or amendment.get("prior_attempt_container")
        != recovery["prior_attempt_container"]
        or amendment.get("unledgered_preparation_debit")
        != {
            "seconds": 60.0,
            "regime": "CONSERVATIVE_UNSIGNED_PREPARATION_DEBIT",
            "observed_external_wall_seconds": 48.51,
        ...
        raise RuntimeError("Wave 60 unsigned preparation debit contract drifted")
    primary, _, _ = _validate_wave60_invalid_preparation_recovery_origin(
        amendment,
        config,
        repo_root=REPO_ROOT,
    ...
    if source != primary:
        raise RuntimeError("Wave 60 unsigned preparation debit source drifted")
    prior = primary.parent
    forbidden = []
    for role in ("primary", "replay"):
        root = prior / role
        for name in ("preparation_receipt.json", WAVE59_PREPARATION_ATTESTATION_NAME):
            forbidden.extend(root.rglob(name))
    if forbidden:
        raise RuntimeError(
            "Wave 60 unsigned debit cannot mix with signed preparation authority"
        )
    return 60.0


def _wave60_hard_set_authority_unsigned_debit(
    args: argparse.Namespace,
    config: dict[str, Any],
    source: Path,
) -> float:
    """Bind signed v2 time plus the sole conservative debit for failed v3."""
    recovery = config.get("attempt", {}).get("recovery")
    amendment_arg = getattr(args, "recovery_amendment", None)
    if (
        not isinstance(recovery, dict)
        or int(config["attempt"]["version"]) != 4
        or recovery.get("prior_attempt_container")
        != (
            "data/geometria_proporcional/"
            "wave60_frozen_policy_transport_attempt_v3"
        ...
        raise RuntimeError("Wave 60 hard-set unsigned debit is not authorized")
    canonical = (REPO_ROOT / recovery["amendment_path"]).resolve(strict=True)
    if (
        amendment_arg.resolve(strict=True) != canonical
        or sha256_file(canonical) != recovery["amendment_sha256"]
    ):
        raise RuntimeError("Wave 60 hard-set unsigned debit amendment drifted")
    amendment = json.loads(canonical.read_text(encoding="utf-8"))
    if (
        amendment.get("schema_version")
        != WAVE60_HARD_SET_AUTHORITY_RECOVERY_AMENDMENT_SCHEMA
    ...
    prior_config_commit = _git_output(
        REPO_ROOT,
        "rev-parse",
        f"{amendment['prior_config_audit']['commit']}^",
    )
    config_relative = next(
        relative
        for relative in config["required_execution_sources"]
    ...
    primary, _, _ = _validate_wave60_hard_set_recovery_origin(
        amendment,
        prior_config,
        repo_root=REPO_ROOT,
        trusted_public_key_path=PUBLIC_KEY,
    ...
    if source != primary:
        raise RuntimeError("Wave 60 hard-set unsigned debit source drifted")
    for role in ("primary", "replay"):
        root = primary.parent / role
        if any(
            any(root.rglob(name))
            for name in (
                "preparation_receipt.json",
            ...
            raise RuntimeError(
                "Wave 60 hard-set debit cannot mix with signed v3 preparation"
            )
    return validate_wave60_hard_set_prior_v2_budget(
        REPO_ROOT,
        amendment,
        trusted_public_key_path=PUBLIC_KEY,
    )
...
def wave60_prior_preparation_elapsed(
    ...
    if mode == "recovery":
        boundary = (
            (source / "preparation_receipt.json").exists(),
            (source / WAVE59_PREPARATION_ATTESTATION_NAME).exists(),
        )
        if any(boundary) and not all(boundary):
            raise RuntimeError("Wave 60 prior preparation budget authority is incomplete")
        if not any(boundary):
            amendment_arg = getattr(args, "recovery_amendment", None)
            if amendment_arg is not None:
                recovery = config.get("attempt", {}).get("recovery")
                if isinstance(recovery, dict):
                    canonical = (REPO_ROOT / recovery["amendment_path"]).resolve(
                        strict=True
                    )
                    amendment = json.loads(canonical.read_text(encoding="utf-8"))
                    if (
            ...
            return _wave60_invalid_preparation_unsigned_debit(
                args, config, source
            )
        ...
        with budget_context as coordinator_budget:
            ...
            if mode in {"replay", "recovery"}:
                ...
                reused_escrow = validate_reused_escrow(
                    escrow_source, contract, recovery_context
                )
