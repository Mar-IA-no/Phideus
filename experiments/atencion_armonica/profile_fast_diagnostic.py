...
from experiments.atencion_armonica import profile_cached_diagnostic as prior

FILES = (
    "src/atencion_armonica/operator_diagnostic_fast.py",
    "experiments/atencion_armonica/test_operator_diagnostic_fast.py",
    "experiments/atencion_armonica/profile_fast_diagnostic.py",
    "experiments/atencion_armonica/test_profile_fast_diagnostic.py",
    "experiments/atencion_armonica/PLAN_OPERATOR_OBJECTIVE_FAST_PATH.md",
    "experiments/atencion_armonica/AMENDMENT_OPERATOR_OBJECTIVE_FAST_PROFILE.md",
)
...
def snapshot(store):
    ...
    cache_review = {"path": base.WORK+"/cache_profile_review.v1.json",
                    "sha256": "7e739af7f7da51ba12c808352e916f3564c9c7d21b2153349b1ffbde974228fb"}
    prior.accept_review(store, cache_review)
    cache_finish = {"path": "attempts/0003/finish.json",
                    "sha256": "f3cbede248f578d3480f97f2208c4adb5588da24b310a4ac6bc4db904824cc5a"}
    finish = store.json(cache_finish)
    if finish["status"] != "COMPLETE" or finish["start"]["operation"] != "profile":
        raise ValueError("cached baseline has no successful profile")
    base.equal(finish["start"]["manifest"], manifest_ref, "cached profile manifest differs")
    revision = store.json(finish["completion"]["runtime_revision"])
    base.equal(revision["review"], cache_review, "cached profile review differs")
    store.json(finish["completion"]["report"])
    return {"manifest": manifest_ref, "cache_review": cache_review, "cache_finish": cache_finish,
...
def accept_review(store, review_ref):
    ...
    if review.get("status") != "FAST_PROFILE_REVIEW_COMPLETE" or not review.get("reports"):
        raise ValueError("fast profile implementation review is not complete")
    base.equal(review["snapshot"], snapshot(store), "fast profile review does not cover current sources")
...
def previous_profile(store, current_review):
    ...
    for path in sorted((store.root/"attempts").glob("*/finish.json")):
        ...
        if old_review["snapshot"] != current_review["snapshot"]:
            continue
        if old_review.get("status") != "FAST_PROFILE_REVIEW_COMPLETE" or not old_review.get("reports"):
        ...
        if (report["status"] != "FAST_PROFILE_CANDIDATE"
...
def execute(review_ref, *, project=ROOT):
    ...
    with base.exclusive_lock(project):
        ...
        if previous_report is not None:
            print(json.dumps({"operation": "fast_profile", "status": "FIRST_SUCCESS_REUSED"}), flush=True)
        ...
        def interrupted(signum, frame):
            raise base.Paused(f"fast profile interrupted by signal {signum}")
        def timeout(signum, frame):
            raise base.BudgetExceeded("fast profile cumulative time guard reached")
        ...
        try:
            ...
            for split in base.TESTS:
                ...
                repeated, codec = fast.bundle_bytes({"extracted": extracted, "result": result, "coverage": coverage})
                encoded_at = time.monotonic()
                original_codec = {k: unit["bundle"][k] for k in codec}
                if codec != original_codec:
                    raise ValueError("fast profile codec receipt differs from v1")
                if repeated != original:
                    raise ValueError("fast profile differs from frozen scientific bytes")
                ...
                if result["candidate_count"] != old[split]["candidate_count"]:
                    raise ValueError("fast profile candidate extent differs")
            ...
            report = {"status": "FAST_PROFILE_CANDIDATE", "manifest": diagnostic.manifest_ref,
                      "runtime_revision": revision, "profiles": rows, "forecast": projection,
                      "authority": "TIMING_ONLY_NOT_ROSTER_AUTHORIZATION"}
            report_ref = publish("fast_profile.json", report)
            attempt.check()
            ...
            print(json.dumps({"operation": "fast_profile", "terminal": terminal}), flush=True)
        return report
