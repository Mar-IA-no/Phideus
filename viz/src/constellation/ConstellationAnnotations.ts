...
import { ConstellationDim, constellationDimColor, constellationDimText } from "./ConstellationDimStyle";
import { IConstellationModelLayout } from "./ConstellationModelLayout";

export function constellationBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    phDim: ConstellationDim,
    t: number,
) {
    if (t < 0.01 || phDim === ConstellationDim.None) return;

    let text = constellationDimText(phDim);
    if (!text) return;

    let color = constellationDimColor(phDim).mul(t);
...
export function drawConstellationBlockNames(render: IRenderState, layout: IConstellationModelLayout) {
