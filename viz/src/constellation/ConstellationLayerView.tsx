...
import s from './ConstellationLayerView.module.scss';
...
import { IConstellationProgramState, initConstellationProgramState, runConstellationProgram } from './ConstellationProgram';
import { ConstellationProgramStateContext, ConstellationSidebar } from './ConstellationSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpConstellationPhase } from './walkthrough/ConstellationWalkthrough';

class ConstellationCanvasRender {
    progState: IConstellationProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initConstellationProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runConstellationProgram(view, progState);
...
export function ConstellationLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<ConstellationCanvasRender | null>(null);
    ...
    useEffect(() => {
        ...
        let render = new ConstellationCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <ConstellationProgramStateContext.Provider value={canvasRender.progState}>
            <ConstellationSidebar />
        </ConstellationProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <ConstellationProgramStateContext.Provider value={canvasRender.progState}>
            <ConstellationCanvasEventSurface />
        </ConstellationProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"constellation-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const ConstellationCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(ConstellationProgramStateContext);
