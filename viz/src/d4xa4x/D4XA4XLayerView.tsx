...
import s from './D4XA4XLayerView.module.scss';
...
import { ID4XA4XProgramState, initD4XA4XProgramState, runD4XA4XProgram } from './D4XA4XProgram';
import { D4XA4XProgramStateContext, D4XA4XSidebar } from './D4XA4XSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpD4XA4XPhase } from './walkthrough/D4XA4XWalkthrough';

class D4XA4XCanvasRender {
    progState: ID4XA4XProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initD4XA4XProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runD4XA4XProgram(view, progState);
...
export function D4XA4XLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<D4XA4XCanvasRender | null>(null);
    ...
    useGlobalKeyboard(KeyboardOrder.MainPage, (ev: KeyboardEvent) => {
        ...
        if (ev.key === ' ') {
            if (wt.time >= wt.phaseLength) { jumpD4XA4XPhase(wt, 1); wt.time = 0; }
    ...
    useEffect(() => {
        ...
        let render = new D4XA4XCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <D4XA4XProgramStateContext.Provider value={canvasRender.progState}>
            <D4XA4XSidebar />
        </D4XA4XProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <D4XA4XProgramStateContext.Provider value={canvasRender.progState}>
            <D4XA4XCanvasEventSurface />
        </D4XA4XProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"d4xa4x-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const D4XA4XCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(D4XA4XProgramStateContext);
