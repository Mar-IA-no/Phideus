...
import ps from './D4XA4XSidebar.module.scss';
import { ID4XA4XProgramState } from './D4XA4XProgram';
import { ID4XA4XPhaseDef, jumpD4XA4XPhase, phaseToD4XA4XGroup, D4XA4XPhase } from './walkthrough/D4XA4XWalkthrough';
...
export let D4XA4XProgramStateContext = createContext<ID4XA4XProgramState>(null!);

export function useD4XA4XProgramState() {
    let context = useContext(D4XA4XProgramStateContext);
...
export const D4XA4XSidebar: React.FC = () => {
    let progState = useD4XA4XProgramState();
...
const D4XA4XPhaseMenu: React.FC = () => {
    let progState = useD4XA4XProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: ID4XA4XPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>d4x-a4x — Forward Cross-Attention</div>
...
const SectionHighlight: React.FC<{ top: number; height: number; width: number }> = ({ top, height, width }) => {
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={pad} y={pad} width={svgW - 2 * pad} height={svgH - 2 * pad} fill="none" stroke="#dd33cc" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
