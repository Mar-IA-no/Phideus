...
let forwardXAttColor = Vec4.fromHexColor('#dd33cc');   // magenta for forward xatt
let lossColor = Vec4.fromHexColor('#cc3333');
let sharedColor = Vec4.fromHexColor('#9933cc');

export function drawD4XA4XSectionLabels(render: IRenderState, layout: ID4XA4XModelLayout) {
    let { margin } = layout;
    ...
    // FORWARD Cross-Attention (magenta)
    {
        let x = layout.audioForwardQProj.x - margin * 1.2;
        let tl = new Vec3(x, layout.audioForwardQProj.y, 0);
        let br = new Vec3(x, layout.audioForwardNorm.y + layout.audioForwardNorm.dy, 0);
        drawSectionLabel(render, 'FORWARD XAtt', tl, br, forwardXAttColor, 10);
    }
    ...
    {
        let rightX = layout.midiForwardQProj.x + layout.midiForwardQProj.dx + margin * 1.2;
        let tl = new Vec3(rightX, layout.midiForwardQProj.y, 0);
        let br = new Vec3(rightX, layout.midiForwardNorm.y + layout.midiForwardNorm.dy, 0);
        drawSectionLabelRight(render, 'FORWARD XAtt', tl, br, forwardXAttColor, 10);
    }
