...
import { D4A4Dim, d4a4DimColor, d4a4DimText } from "./D4A4DimStyle";
...
export function d4a4BlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === D4A4Dim.None) return;

    let text = d4a4DimText(phDim);
    if (!text) return;

    let color = d4a4DimColor(phDim).mul(t);
...
export function drawD4A4BlockNames(render: IRenderState, layout: ID4A4ModelLayout) {
