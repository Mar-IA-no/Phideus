...
export function drawD4A4SectionLabels(render: IRenderState, layout: ID4A4ModelLayout) {
    let { margin } = layout;
    ...
    // CNN (frozen) — positioned away from descriptor column
    {
        ...
        drawSectionLabel(render, 'CNN (frozen)', tl, br, frozenColor, 9);
    }
    ...
    // Audio Descriptor A4 (far left)
    {
        ...
        drawSectionLabel(render, 'Audio Descriptor A4', tl, br, descriptorAmberColor, 12);
        drawSectionLabel(render, 'NO_GRAD', new Vec3(tl.x, tl.y + 18, 0), new Vec3(br.x, tl.y + 18, 0), descriptorAmberColor.mul(0.6), 8);
    }
    ...
    {
        let tl = new Vec3(layout.audioConcatGate.x - margin * 1.2, layout.audioConcatGate.y, 0);
        let br = new Vec3(layout.audioConcatGate.x - margin * 1.2, layout.audioProjLayer3.y + layout.audioProjLayer3.dy, 0);
        drawSectionLabel(render, 'Concat + Projection', tl, br, baseColor, 8);
    }
    ...
    // MIDI Descriptor D4 (far right)
    {
        ...
        drawSectionLabelRight(render, 'MIDI Descriptor D4', tl, br, descriptorGreenColor, 12);
        drawSectionLabelRight(render, 'NO_GRAD', new Vec3(rightX, tl.y + 18, 0), new Vec3(rightX, tl.y + 18, 0), descriptorGreenColor.mul(0.6), 8);
    }
    ...
    {
        let rightX = layout.midiConcatGate.x + layout.midiConcatGate.dx + margin * 1.2;
        let tl = new Vec3(rightX, layout.midiConcatGate.y, 0);
        let br = new Vec3(rightX, layout.midiProjLayer3.y + layout.midiProjLayer3.dy, 0);
        drawSectionLabelRight(render, 'Concat + Projection', tl, br, baseColor, 8);
    }
