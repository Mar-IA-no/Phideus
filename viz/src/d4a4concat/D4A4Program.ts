...
import { defaultD4A4Shape, genD4A4ModelLayout, ID4A4ModelLayout, ID4A4ModelShape } from "./D4A4ModelLayout";
import { drawD4A4Arrows } from "./D4A4Arrows";
import { drawD4A4SectionLabels } from "./D4A4SectionLabels";
import { drawD4A4BlockNames } from "./D4A4Annotations";
import { initD4A4Walkthrough, ID4A4Walkthrough, runD4A4Walkthrough } from "./walkthrough/D4A4Walkthrough";

export interface ID4A4ProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initD4A4ProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): ID4A4ProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initD4A4Walkthrough();
    let shape = defaultD4A4Shape;
    ...
    return {
        ...
        camera,
        shape,
        layout: genD4A4ModelLayout(shape),
...
export function runD4A4Program(view: IRenderView, state: ID4A4ProgramState) {
    ...
    state.layout = genD4A4ModelLayout(state.shape);
    ...
    drawD4A4Arrows(state.render, state.layout);
    drawD4A4SectionLabels(state.render, state.layout);
    drawD4A4BlockNames(state.render, state.layout);
