...
import s from '@/src/phideus/PhideusLayerView.module.scss';
...
import { ID4A4ProgramState, initD4A4ProgramState, runD4A4Program } from './D4A4Program';
import { D4A4ProgramStateContext, D4A4Sidebar } from './D4A4Sidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpD4A4Phase } from './walkthrough/D4A4Walkthrough';

class D4A4CanvasRender {
    progState: ID4A4ProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initD4A4ProgramState(canvasEl, fontAtlasData);
...
export function D4A4LayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<D4A4CanvasRender | null>(null);
    ...
    useEffect(() => {
        ...
        let render = new D4A4CanvasRender(canvasEl, fontAtlasData);
        setCanvasRender(render);
        render.markDirty();
        let resizeObserver = new ResizeObserver(() => { render.canvasSizeDirty = true; render.markDirty(); });
        resizeObserver.observe(canvasEl);
        return () => { resizeObserver.disconnect(); render.stop(); setCanvasRender(null); };
    }, [canvasEl, fontAtlasData]);

    useEffect(() => {
        if (canvasRender) { canvasRender.progState.pageLayout = layout; canvasRender.markDirty(); }
    }, [canvasRender, layout]);

    let sidebar = canvasRender && <div className={s.sidebar}>
        <D4A4ProgramStateContext.Provider value={canvasRender.progState}>
            <D4A4Sidebar />
        </D4A4ProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <D4A4ProgramStateContext.Provider value={canvasRender.progState}>
            <D4A4CanvasEventSurface />
        </D4A4ProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"d4a4-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const D4A4CanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(D4A4ProgramStateContext);
