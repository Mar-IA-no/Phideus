...
export function crossAttPhase09_AudioTransformer(args: ICrossAttWalkthroughArgs) {
    ...
    dimExcept([
        ...
        // Keep CNN visible for width contrast
        ...layout.audioCnn,
    ], 0.08);
    ...
    commentary()`These 4 Transformer layers process the output of reverse cross-attention: only ${outRef}! Compare the NARROW transformer blocks (T_stft=188 width) with the WIDE CNN blocks above (T_audio=2400 width) — that's the visual consequence of 12.8x sequence compression from reverse cross-attention.`;
    breakAfter();
    ...
    commentary()`Each layer has the full LN1 -> Q/K/V -> Attention -> Attn Out -> +Residual -> LN2 -> FFN Up (1024->4096) -> GELU -> FFN Down (4096->1024) -> +Residual structure. Self-attention cost per layer: 188^2 = 35K ops. If we had kept 2400 tokens (regular cross-attention), it would be 2400^2 = 5.76M — 163x more expensive.`;
    breakAfter();

    let t1 = afterTime(null, 2.0, 0.5);
    if (t1.active) {
        // Highlight FFN expansion
