...
export function crossAttPhase02_AudioDescriptor(args: ICrossAttWalkthroughArgs) {
    let { state, layout, tools } = args;
    let { afterTime, breakAfter, commentary, c_blockRef, dimExcept, drawDimLabels } = tools;

    setCrossAttInitialCamera(state, new Vec3(-200, 0, -350), new Vec3(290, 18, 11));
    ...
    commentary()`The final ${outRef} transposes [B, 8, 188] -> [B, 188, 8]: each of 188 STFT frames now has 8 features describing temporal change in each octave band. This [188, 8] tensor becomes the descriptor input for cross-attention.`;
    breakAfter();
