...
import s from './CrossAttLayerView.module.scss';
...
import { ICrossAttProgramState, initCrossAttProgramState, runCrossAttProgram } from './CrossAttProgram';
import { CrossAttProgramStateContext, CrossAttSidebar } from './CrossAttSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpCrossAttPhase } from './walkthrough/CrossAttWalkthrough';

class CrossAttCanvasRender {
    progState: ICrossAttProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initCrossAttProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runCrossAttProgram(view, progState);
...
export function CrossAttLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<CrossAttCanvasRender | null>(null);
    ...
    useGlobalKeyboard(KeyboardOrder.MainPage, (ev: KeyboardEvent) => {
        ...
        if (ev.key === ' ') {
            if (wt.time >= wt.phaseLength) { jumpCrossAttPhase(wt, 1); wt.time = 0; }
    ...
    useEffect(() => {
        ...
        let render = new CrossAttCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <CrossAttProgramStateContext.Provider value={canvasRender.progState}>
            <CrossAttSidebar />
        </CrossAttProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <CrossAttProgramStateContext.Provider value={canvasRender.progState}>
            <CrossAttCanvasEventSurface />
        </CrossAttProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"crossatt-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const CrossAttCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(CrossAttProgramStateContext);
