...
import { CrossAttDim, crossAttDimColor, crossAttDimText } from "./CrossAttDimStyle";
import { ICrossAttModelLayout } from "./CrossAttModelLayout";

export function crossAttBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === CrossAttDim._None) return;

    let text = crossAttDimText(phDim);
    if (!text) return;

    let color = crossAttDimColor(phDim).mul(t);
