...
import s from './RosetaLayerView.module.scss';
...
import { IRosetaProgramState, initRosetaProgramState, runRosetaProgram } from './RosetaProgram';
import { RosetaProgramStateContext, RosetaSidebar } from './RosetaSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpRosetaPhase } from './walkthrough/RosetaWalkthrough';

class RosetaCanvasRender {
    progState: IRosetaProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initRosetaProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runRosetaProgram(view, progState);
...
export function RosetaLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<RosetaCanvasRender | null>(null);
    ...
    useEffect(() => {
        ...
        let render = new RosetaCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <RosetaProgramStateContext.Provider value={canvasRender.progState}>
            <RosetaSidebar />
        </RosetaProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <RosetaProgramStateContext.Provider value={canvasRender.progState}>
            <RosetaCanvasEventSurface />
        </RosetaProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"roseta-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const RosetaCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(RosetaProgramStateContext);
