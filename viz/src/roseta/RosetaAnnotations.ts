...
import { RosetaDim, rosetaDimColor, rosetaDimText } from "./RosetaDimStyle";
 ...
 * Roseta version of blockDimension() from llm/Annotations.ts.
 */
export function rosetaBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === RosetaDim.None) return;

    let text = rosetaDimText(phDim);
    if (!text) return;

    let color = rosetaDimColor(phDim).mul(t);
 ...
 */
export function drawRosetaBlockNames(render: IRenderState, layout: IRosetaModelLayout) {
