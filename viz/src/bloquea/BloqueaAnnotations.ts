...
import { BloqueaDim, bloqueaDimColor, bloqueaDimText } from "./BloqueaDimStyle";
 ...
 * BloqueA version of blockDimension() from llm/Annotations.ts.
 */
export function bloqueaBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || baDim === BloqueaDim.None) return;

    let text = bloqueaDimText(baDim);
    if (!text) return;

    let color = bloqueaDimColor(baDim).mul(t);
 ...
 */
export function drawBloqueaBlockNames(render: IRenderState, layout: IBloqueaModelLayout) {
