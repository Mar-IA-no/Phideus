...
import ps from './HrmSidebar.module.scss';
import { IHrmProgramState } from './HrmProgram';
import { IHrmPhaseDef, jumpHrmPhase, phaseToHrmGroup, HrmPhase } from './walkthrough/HrmWalkthrough';
...
export let HrmProgramStateContext = createContext<IHrmProgramState>(null!);

export function useHrmProgramState() {
    let context = useContext(HrmProgramStateContext);
...
export const HrmSidebar: React.FC = () => {
    let progState = useHrmProgramState();
...
const HrmPhaseMenu: React.FC = () => {
    let progState = useHrmProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: IHrmPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>HRM Architecture</div>
...
const SectionHighlight: React.FC<{ top: number; height: number; width: number }> = ({ top, height, width }) => {
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={pad} y={pad} width={svgW - 2 * pad} height={svgH - 2 * pad} fill="none" stroke="#339966" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
