...
import { HrmDim, hrmDimColor, hrmDimText } from "./HrmDimStyle";
import { IHrmModelLayout } from "./HrmModelLayout";

export function hrmBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === HrmDim.None) return;

    let text = hrmDimText(phDim);
    if (!text) return;

    let color = hrmDimColor(phDim).mul(t);
...
function drawBlockNameLabel(render: IRenderState, blk: IBlkDef, opacity: number) {
    ...
    let color: Vec4;
    switch (blk.t) {
        case 'w': color = new Vec4(0.1, 0.4, 0.25, opacity); break;
    ...
    // h_context gets special orange color
    if (blk.special === 1) {
        color = new Vec4(0.8, 0.47, 0.2, opacity);
    }

    // halt/continue gets special red color
    if ((blk.special as number) === 2) {
        color = new Vec4(0.8, 0.27, 0.27, opacity);
    }
...
export function drawHrmBlockNames(render: IRenderState, layout: IHrmModelLayout) {
