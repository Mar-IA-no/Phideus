...
import { defaultHrmShape, genHrmModelLayout, IHrmModelLayout, IHrmModelShape } from "./HrmModelLayout";
import { drawHrmArrows } from "./HrmArrows";
import { drawHrmSectionLabels } from "./HrmSectionLabels";
import { drawHrmBlockNames } from "./HrmAnnotations";
import { initHrmWalkthrough, IHrmWalkthrough, runHrmWalkthrough } from "./walkthrough/HrmWalkthrough";

export interface IHrmProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initHrmProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IHrmProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initHrmWalkthrough();
    let shape = defaultHrmShape;
    ...
    return {
        render, camera, shape,
        layout: genHrmModelLayout(shape),
...
export function runHrmProgram(view: IRenderView, state: IHrmProgramState) {
    ...
    state.layout = genHrmModelLayout(state.shape);
    ...
    if (state.inWalkthrough) runHrmWalkthrough(state, view);
    ...
    drawHrmArrows(state.render, state.layout);
    drawHrmSectionLabels(state.render, state.layout);
    drawHrmBlockNames(state.render, state.layout);
