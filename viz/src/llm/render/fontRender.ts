...
export async function fetchFontAtlasData(): Promise<IFontAtlasData> {
    ...
    imgEl.src = (process.env.NEXT_PUBLIC_BASE_PATH || '') + '/fonts/font-atlas.png';

    let fontDefP = fetch((process.env.NEXT_PUBLIC_BASE_PATH || '') + '/fonts/font-def.json', { credentials: 'include', mode: 'no-cors' }).then(r => r.json());
