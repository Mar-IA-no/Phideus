...
import ps from './T3TowerSidebar.module.scss';
import { IT3TowerProgramState } from './T3TowerProgram';
import { IT3TowerPhaseDef, jumpT3TowerPhase, phaseToT3TowerGroup, T3TowerPhase } from './walkthrough/T3TowerWalkthrough';
...
export let T3TowerProgramStateContext = createContext<IT3TowerProgramState>(null!);

export function useT3TowerProgramState() {
    let context = useContext(T3TowerProgramStateContext);
...
export const T3TowerSidebar: React.FC = () => {
    let progState = useT3TowerProgramState();
    ...
    function handleAdvanceClick() {
        if (wt.time >= wt.phaseLength) {
            ...
            let speed = 15;
            if (node) {
                if (node.end > wt.time) speed = (node.end - wt.time) * 2;
            }
            wt.speed = speed;
        }
        progState.markDirty();
    ...
    useEffect(() => {
        if (parasEl) parasEl.parentElement!.scrollTop = 0;
    }, [parasEl, wt.phase]);
...
const T3TowerPhaseMenu: React.FC = () => {
    let progState = useT3TowerProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: IT3TowerPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>T3 — Three-Tower Architecture</div>
...
const SectionHighlight: React.FC<{
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={x0} y={y0} width={x1 - x0} height={y1 - y0} fill="none" stroke="#2299aa" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
