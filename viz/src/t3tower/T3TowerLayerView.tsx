...
import s from './T3TowerLayerView.module.scss';
...
import { IT3TowerProgramState, initT3TowerProgramState, runT3TowerProgram } from './T3TowerProgram';
import { T3TowerProgramStateContext, T3TowerSidebar } from './T3TowerSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpT3TowerPhase } from './walkthrough/T3TowerWalkthrough';

class T3TowerCanvasRender {
    progState: IT3TowerProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initT3TowerProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runT3TowerProgram(view, progState);
...
export function T3TowerLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<T3TowerCanvasRender | null>(null);
    ...
    useEffect(() => {
        ...
        let render = new T3TowerCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <T3TowerProgramStateContext.Provider value={canvasRender.progState}>
            <T3TowerSidebar />
        </T3TowerProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <T3TowerProgramStateContext.Provider value={canvasRender.progState}>
            <T3TowerCanvasEventSurface />
        </T3TowerProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"t3tower-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const T3TowerCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(T3TowerProgramStateContext);
