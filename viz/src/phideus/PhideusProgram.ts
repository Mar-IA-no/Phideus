...
import { defaultPhideusShape, genPhideusModelLayout, IPhideusModelLayout, IPhideusModelShape } from "./PhideusModelLayout";
import { drawPhideusArrows } from "./PhideusArrows";
import { drawPhideusSectionLabels } from "./PhideusSectionLabels";
import { drawPhideusBlockNames } from "./PhideusAnnotations";
import { initPhideusWalkthrough, IPhideusWalkthrough, runPhideusWalkthrough } from "./walkthrough/PhideusWalkthrough";
...
export interface IPhideusProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initPhideusProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IPhideusProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initPhideusWalkthrough();
    let shape = defaultPhideusShape;
    ...
    return {
        ...
        camera,
        shape,
        layout: genPhideusModelLayout(shape),
...
export function runPhideusProgram(view: IRenderView, state: IPhideusProgramState) {
    ...
    state.layout = genPhideusModelLayout(state.shape);
    ...
    if (state.inWalkthrough) {
        runPhideusWalkthrough(state, view);
    }
    ...
    drawPhideusArrows(state.render, state.layout);
    drawPhideusSectionLabels(state.render, state.layout);
    drawPhideusBlockNames(state.render, state.layout);
