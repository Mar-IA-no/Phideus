...
import s from './PhideusLayerView.module.scss';
...
import { IPhideusProgramState, initPhideusProgramState, runPhideusProgram } from './PhideusProgram';
import { PhideusProgramStateContext, PhideusSidebar } from './PhideusSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpPhideusPhase } from './walkthrough/PhideusWalkthrough';

class PhideusCanvasRender {
    progState: IPhideusProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initPhideusProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runPhideusProgram(view, progState);
...
export function PhideusLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<PhideusCanvasRender | null>(null);
    ...
    useEffect(() => {
        ...
        let render = new PhideusCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <PhideusProgramStateContext.Provider value={canvasRender.progState}>
            <PhideusSidebar />
        </PhideusProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <PhideusProgramStateContext.Provider value={canvasRender.progState}>
            <PhideusCanvasEventSurface />
        </PhideusProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"phideus-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const PhideusCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(PhideusProgramStateContext);
