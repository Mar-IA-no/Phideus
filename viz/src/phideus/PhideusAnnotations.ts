...
import { PhideusDim, phideusDimColor, phideusDimText } from "./PhideusDimStyle";
...
/**
 * Draw |--- label ---| dimension annotation along a block edge.
 * Phideus version of blockDimension() from llm/Annotations.ts.
 */
export function phideusBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === PhideusDim.None) return;

    let text = phideusDimText(phDim);
    if (!text) return;

    let color = phideusDimColor(phDim).mul(t);
    ...
    // Camera-distance-based font scaling
    if (camera) {
...
function drawBlockNameLabel(render: IRenderState, blk: IBlkDef, opacity: number) {
    ...
    // Place label well above the block — clear of the top face
    let cy = blk.y - fontSize * 1.0 - 1.5;
    // Push forward in Z so it floats in front of the block
...
/**
 * Draw name labels for all visible blocks in the layout.
 */
export function drawPhideusBlockNames(render: IRenderState, layout: IPhideusModelLayout) {
