...
import ps from './DannSidebar.module.scss';
import { IDannProgramState } from './DannProgram';
import { IDannPhaseDef, jumpDannPhase, phaseToDannGroup, DannPhase } from './walkthrough/DannWalkthrough';
...
export let DannProgramStateContext = createContext<IDannProgramState>(null!);

export function useDannProgramState() {
    let context = useContext(DannProgramStateContext);
...
export const DannSidebar: React.FC = () => {
    let progState = useDannProgramState();
...
const DannPhaseMenu: React.FC = () => {
    let progState = useDannProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: IDannPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>DANN Architecture</div>
...
const SectionHighlight: React.FC<{ top: number; height: number; width: number }> = ({ top, height, width }) => {
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={pad} y={pad} width={svgW - 2 * pad} height={svgH - 2 * pad} fill="none" stroke="#996633" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
