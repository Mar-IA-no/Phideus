...
import s from './DannLayerView.module.scss';
...
import { IDannProgramState, initDannProgramState, runDannProgram } from './DannProgram';
import { DannProgramStateContext, DannSidebar } from './DannSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpDannPhase } from './walkthrough/DannWalkthrough';

class DannCanvasRender {
    progState: IDannProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initDannProgramState(canvasEl, fontAtlasData);
...
export function DannLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<DannCanvasRender | null>(null);
    ...
    useGlobalKeyboard(KeyboardOrder.MainPage, (ev: KeyboardEvent) => {
        ...
        if (ev.key === ' ') {
            if (wt.time >= wt.phaseLength) { jumpDannPhase(wt, 1); wt.time = 0; }
    ...
    useEffect(() => {
        ...
        let render = new DannCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <DannProgramStateContext.Provider value={canvasRender.progState}>
            <DannSidebar />
        </DannProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <DannProgramStateContext.Provider value={canvasRender.progState}>
            <DannCanvasEventSurface />
        </DannProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"dann-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const DannCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(DannProgramStateContext);
