...
import { defaultDannShape, genDannModelLayout, IDannModelLayout, IDannModelShape } from "./DannModelLayout";
import { drawDannArrows } from "./DannArrows";
import { drawDannSectionLabels } from "./DannSectionLabels";
import { drawDannBlockNames } from "./DannAnnotations";
import { initDannWalkthrough, IDannWalkthrough, runDannWalkthrough } from "./walkthrough/DannWalkthrough";

export interface IDannProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initDannProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IDannProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initDannWalkthrough();
    let shape = defaultDannShape;

    let camera: ICamera = {
        angle: new Vec3(290, 20, 10.0),
    ...
    return {
        render, camera, shape,
        layout: genDannModelLayout(shape),
...
export function runDannProgram(view: IRenderView, state: IDannProgramState) {
    ...
    state.layout = genDannModelLayout(state.shape);
    ...
    if (state.inWalkthrough) runDannWalkthrough(state, view);
    ...
    drawDannArrows(state.render, state.layout);
    drawDannSectionLabels(state.render, state.layout);
    drawDannBlockNames(state.render, state.layout);
