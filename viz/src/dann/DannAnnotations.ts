...
import { DannDim, dannDimColor, dannDimText } from "./DannDimStyle";
import { IDannModelLayout } from "./DannModelLayout";

export function dannBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === DannDim.None) return;

    let text = dannDimText(phDim);
    if (!text) return;

    let color = dannDimColor(phDim).mul(t);
...
function drawBlockNameLabel(render: IRenderState, blk: IBlkDef, opacity: number) {
    ...
    // GRL gets special brown color
    if (blk.special === 1) {
        color = new Vec4(0.6, 0.4, 0.2, opacity);
    }
...
export function drawDannBlockNames(render: IRenderState, layout: IDannModelLayout) {
