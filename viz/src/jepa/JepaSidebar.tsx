...
import ps from './JepaSidebar.module.scss';
import { IJepaProgramState } from './JepaProgram';
import { IJepaPhaseDef, jumpJepaPhase, phaseToJepaGroup, JepaPhase } from './walkthrough/JepaWalkthrough';
...
export let JepaProgramStateContext = createContext<IJepaProgramState>(null!);

export function useJepaProgramState() {
    let context = useContext(JepaProgramStateContext);
...
export const JepaSidebar: React.FC = () => {
    let progState = useJepaProgramState();
...
const JepaPhaseMenu: React.FC = () => {
    let progState = useJepaProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: IJepaPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>JEPA-Lite Architecture</div>
...
const SectionHighlight: React.FC<{ top: number; height: number; width: number }> = ({ top, height, width }) => {
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={pad} y={pad} width={svgW - 2 * pad} height={svgH - 2 * pad} fill="none" stroke="#6633cc" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
