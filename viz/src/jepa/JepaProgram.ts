...
import { defaultJepaShape, genJepaModelLayout, IJepaModelLayout, IJepaModelShape } from "./JepaModelLayout";
import { drawJepaArrows } from "./JepaArrows";
import { drawJepaSectionLabels } from "./JepaSectionLabels";
import { drawJepaBlockNames } from "./JepaAnnotations";
import { initJepaWalkthrough, IJepaWalkthrough, runJepaWalkthrough } from "./walkthrough/JepaWalkthrough";

export interface IJepaProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initJepaProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): IJepaProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initJepaWalkthrough();
    let shape = defaultJepaShape;
    ...
    return {
        render, camera, shape,
        layout: genJepaModelLayout(shape),
...
export function runJepaProgram(view: IRenderView, state: IJepaProgramState) {
    ...
    state.layout = genJepaModelLayout(state.shape);
    ...
    if (state.inWalkthrough) runJepaWalkthrough(state, view);
    ...
    drawJepaArrows(state.render, state.layout);
    drawJepaSectionLabels(state.render, state.layout);
    drawJepaBlockNames(state.render, state.layout);
