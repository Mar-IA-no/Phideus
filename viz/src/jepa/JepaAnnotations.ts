...
import { JepaDim, jepaDimColor, jepaDimText } from "./JepaDimStyle";
import { IJepaModelLayout } from "./JepaModelLayout";

export function jepaBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === JepaDim._None) return;

    let text = jepaDimText(phDim);
    if (!text) return;

    let color = jepaDimColor(phDim).mul(t);
...
function drawBlockNameLabel(render: IRenderState, blk: IBlkDef, opacity: number) {
    ...
    if (blk.special === 1) {
        color = new Vec4(0.4, 0.2, 0.8, opacity);  // purple for stop-gradient
    }
...
export function drawJepaBlockNames(render: IRenderState, layout: IJepaModelLayout) {
