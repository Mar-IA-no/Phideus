...
import s from './JepaLayerView.module.scss';
...
import { IJepaProgramState, initJepaProgramState, runJepaProgram } from './JepaProgram';
import { JepaProgramStateContext, JepaSidebar } from './JepaSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpJepaPhase } from './walkthrough/JepaWalkthrough';

class JepaCanvasRender {
    progState: IJepaProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initJepaProgramState(canvasEl, fontAtlasData);
...
export function JepaLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<JepaCanvasRender | null>(null);
    ...
    useGlobalKeyboard(KeyboardOrder.MainPage, (ev: KeyboardEvent) => {
        ...
        if (ev.key === ' ') {
            if (wt.time >= wt.phaseLength) { jumpJepaPhase(wt, 1); wt.time = 0; }
    ...
    useEffect(() => {
        ...
        let render = new JepaCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <JepaProgramStateContext.Provider value={canvasRender.progState}>
            <JepaSidebar />
        </JepaProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <JepaProgramStateContext.Provider value={canvasRender.progState}>
            <JepaCanvasEventSurface />
        </JepaProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"jepa-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const JepaCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(JepaProgramStateContext);
