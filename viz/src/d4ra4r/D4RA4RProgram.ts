...
import { defaultD4RA4RShape, genD4RA4RModelLayout, ID4RA4RModelLayout, ID4RA4RModelShape } from "./D4RA4RModelLayout";
import { drawD4RA4RArrows } from "./D4RA4RArrows";
import { drawD4RA4RSectionLabels } from "./D4RA4RSectionLabels";
import { drawD4RA4RBlockNames } from "./D4RA4RAnnotations";
import { initD4RA4RWalkthrough, ID4RA4RWalkthrough, runD4RA4RWalkthrough } from "./walkthrough/D4RA4RWalkthrough";

export interface ID4RA4RProgramState {
    render: IRenderState;
    camera: ICamera;
...
export function initD4RA4RProgramState(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData): ID4RA4RProgramState | null {
    let render = initRender(canvasEl, fontAtlasData);
    if (!render) return null;

    let walkthrough = initD4RA4RWalkthrough();
    let shape = defaultD4RA4RShape;
    ...
    return {
        render, camera, shape,
        layout: genD4RA4RModelLayout(shape),
...
export function runD4RA4RProgram(view: IRenderView, state: ID4RA4RProgramState) {
    ...
    state.layout = genD4RA4RModelLayout(state.shape);
    ...
    if (state.inWalkthrough) runD4RA4RWalkthrough(state, view);
    ...
    drawD4RA4RArrows(state.render, state.layout);
    drawD4RA4RSectionLabels(state.render, state.layout);
    drawD4RA4RBlockNames(state.render, state.layout);
