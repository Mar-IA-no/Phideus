...
import { D4RA4RDim, d4ra4rDimColor, d4ra4rDimText } from "./D4RA4RDimStyle";
import { ID4RA4RModelLayout } from "./D4RA4RModelLayout";

export function d4ra4rBlockDimension(
    render: IRenderState,
    layout: IModelLayout,
    blk: IBlkDef,
    dim: Dim,
    ...
    if (t < 0.01 || phDim === D4RA4RDim._None) return;

    let text = d4ra4rDimText(phDim);
    if (!text) return;

    let color = d4ra4rDimColor(phDim).mul(t);
...
function drawBlockNameLabel(render: IRenderState, blk: IBlkDef, opacity: number) {
    ...
    // Cross-attention blocks (special=1) → cyan
    if (blk.special === 1) {
        color = new Vec4(0.0, 0.6, 0.7, opacity);
    }
    // NoGrad DSP blocks → amber/orange
    else if (dimX === D4RA4RDim.NoGrad || dimX === D4RA4RDim.D_stft_bins || dimX === D4RA4RDim.D_bands || dimX === D4RA4RDim.D_semitone || dimX === D4RA4RDim.D_log_ratio) {
        color = new Vec4(0.85, 0.6, 0.25, opacity);
    }
    // TrainableXAtt blocks → cyan
    else if (dimX === D4RA4RDim.TrainableXAtt) {
        color = new Vec4(0.0, 0.6, 0.7, opacity);
    }
    else {
        switch (blk.t) {
...
export function drawD4RA4RBlockNames(render: IRenderState, layout: ID4RA4RModelLayout) {
