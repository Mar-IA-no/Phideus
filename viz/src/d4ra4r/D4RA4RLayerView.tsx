...
import s from './D4RA4RLayerView.module.scss';
...
import { ID4RA4RProgramState, initD4RA4RProgramState, runD4RA4RProgram } from './D4RA4RProgram';
import { D4RA4RProgramStateContext, D4RA4RSidebar } from './D4RA4RSidebar';
import { KeyboardManagerContext, KeyboardOrder, useGlobalKeyboard } from '@/src/utils/keyboard';
import { jumpD4RA4RPhase } from './walkthrough/D4RA4RWalkthrough';

class D4RA4RCanvasRender {
    progState: ID4RA4RProgramState;
    ...
    constructor(canvasEl: HTMLCanvasElement, fontAtlasData: IFontAtlasData) {
        let state = initD4RA4RProgramState(canvasEl, fontAtlasData);
    ...
    render(time: number, dt: number) {
        ...
        runD4RA4RProgram(view, progState);
...
export function D4RA4RLayerView() {
    let [canvasEl, setCanvasEl] = useState<HTMLCanvasElement | null>(null);
    let [canvasRender, setCanvasRender] = useState<D4RA4RCanvasRender | null>(null);
    ...
    useGlobalKeyboard(KeyboardOrder.MainPage, (ev: KeyboardEvent) => {
        ...
        if (ev.key === ' ') {
            if (wt.time >= wt.phaseLength) { jumpD4RA4RPhase(wt, 1); wt.time = 0; }
    ...
    useEffect(() => {
        ...
        let render = new D4RA4RCanvasRender(canvasEl, fontAtlasData);
    ...
    let sidebar = canvasRender && <div className={s.sidebar}>
        <D4RA4RProgramStateContext.Provider value={canvasRender.progState}>
            <D4RA4RSidebar />
        </D4RA4RProgramStateContext.Provider>
    </div>;

    let mainView = <div className={s.canvasWrap}>
        <canvas className={s.canvas} ref={setCanvasEl} />
        {canvasRender && <D4RA4RProgramStateContext.Provider value={canvasRender.progState}>
            <D4RA4RCanvasEventSurface />
        </D4RA4RProgramStateContext.Provider>}
    </div>;

    return <div className={s.view}>
        <Resizer id={"d4ra4r-sidebar"} className={"flex-1"} vertical={!layout.isDesktop} defaultFraction={0.35}>
...
const D4RA4RCanvasEventSurface: React.FC = () => {
    let [eventSurfaceEl, setEventSurfaceEl] = useState<HTMLDivElement | null>(null);
    let progState = React.useContext(D4RA4RProgramStateContext);
