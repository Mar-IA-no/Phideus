...
import ps from './D4RA4RSidebar.module.scss';
import { ID4RA4RProgramState } from './D4RA4RProgram';
import { ID4RA4RPhaseDef, jumpD4RA4RPhase, phaseToD4RA4RGroup, D4RA4RPhase } from './walkthrough/D4RA4RWalkthrough';
...
export let D4RA4RProgramStateContext = createContext<ID4RA4RProgramState>(null!);

export function useD4RA4RProgramState() {
    let context = useContext(D4RA4RProgramStateContext);
...
export const D4RA4RSidebar: React.FC = () => {
    let progState = useD4RA4RProgramState();
...
const D4RA4RPhaseMenu: React.FC = () => {
    let progState = useD4RA4RProgramState();
    let wt = progState.walkthrough;

    function handlePhaseClick(ev: React.MouseEvent, phase: ID4RA4RPhaseDef) {
    ...
    return <div className={ps.tocBackground}>
        <div className={ps.tocTitle}>d4r-a4r — Mixed Descriptors + Reverse</div>
...
const SectionHighlight: React.FC<{ top: number; height: number; width: number }> = ({ top, height, width }) => {
    ...
    return <div className={s.sectionHighlightWrap} style={{ top: top - rectPad, height: svgH, width: svgW, left: -rectPad }}>
        <svg viewBox={`0 0 ${svgW} ${svgH}`} className={s.sectionHighlight}>
            <rect x={pad} y={pad} width={svgW - 2 * pad} height={svgH - 2 * pad} fill="none" stroke="#6b9e3a" strokeWidth={strokeWidth} opacity={strokeWidth} rx={5} ry={5} />
        </svg>
    </div>;
};
